...
class dataMigrationExportAhgCsvAction extends AhgController
{
    ...
    protected function buildAhgCsv(array $records): string
    {
        ...
        $columns = [
            ...
            'ahgRelationships',
            // AHG Condition Fields
            'ahgConditionOverallRating',
            'ahgConditionSummary',
            'ahgConditionRecommendations',
            'ahgConditionPriority',
            'ahgConditionContext',
            'ahgConditionAssessmentDate',
            'ahgConditionNextCheckDate',
            'ahgConditionEnvironmentalNotes',
            'ahgConditionHandlingNotes',
            'ahgConditionDisplayNotes',
            'ahgConditionStorageNotes',
            'Filename',
    ...
    protected function transformData(array $rows, array $headers, array $mapping): array
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($headers as $index => $header) {
                if ((strpos($header, 'ahg') === 0 || strpos($header, '_digitalObject') === 0) && isset($row[$index])) {
            ...
            foreach ($mapping as $fieldConfig) {
                ...
                if (!empty($ahgField) && $value !== '') {
                    if (!isset($record[$ahgField]) || $record[$ahgField] === '') {
                        $record[$ahgField] = $value;
                    } elseif ($concatenate) {
                        $symbol = ($concatSymbol === '\n' || $concatSymbol === "\\n") ? "\n" : $concatSymbol;
                        $record[$ahgField] .= $symbol . $value;
                    }
                }
            ...
            // Map internal _digitalObject fields to export columns
