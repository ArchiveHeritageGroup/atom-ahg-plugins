...
<div class="container-fluid py-3">
  ...
  <div class="row mb-3">
    <div class="col">
      ...
      <span class="badge bg-info ms-2">Target: <?php echo htmlspecialchars($targetType) ?></span>
    </div>
  </div>
        ...
        <table class="table table-striped table-hover table-sm mb-0" id="mappingTable">
          ...
          <tbody>
            ...
            function getFieldCategory($field) {
              $f = strtolower($field);
              if (strpos($f, "security") !== false || strpos($f, "access") !== false && strpos($f, "accession") === false && strpos($f, "accesspoint") === false) return "security";
            ...
            <tr data-row="<?php echo $i ?>">
              ...
              <td style="width:14%" class="ahg-column">
                <select name="fields[<?php echo $i ?>][ahg_field]" class="form-select form-select-sm ahg-field-select">
                  ...
                  <?php foreach ($targetFields as $key => $label): ?>
                    <?php if (strpos($key, "ahgProvenance") === 0): ?>
                    <option value="<?php echo $key ?>" <?php echo (isset($row["ahg_field"]) && $row["ahg_field"] === $key) ? "selected" : "" ?>>
                      <?php echo htmlspecialchars(str_replace("AHG: ", "", $label)) ?>
                    </option>
                    <?php endif ?>
                  <?php endforeach ?>
                  </optgroup>
                  ...
                  <?php foreach ($targetFields as $key => $label): ?>
                    <?php if (strpos($key, "ahgCondition") === 0): ?>
                    <option value="<?php echo $key ?>" <?php echo (isset($row["ahg_field"]) && $row["ahg_field"] === $key) ? "selected" : "" ?>>
                      <?php echo htmlspecialchars(str_replace("AHG: ", "", $label)) ?>
                    </option>
                    <?php endif ?>
                  ...
                  <optgroup label="Rights &amp; Security">
                  <?php foreach ($targetFields as $key => $label): ?>
                    <?php if (strpos($key, "ahgRights") === 0 || strpos($key, "ahgAccess") === 0 || strpos($key, "ahgCopyright") === 0 || strpos($key, "ahgSecurity") === 0 || strpos($key, "ahgRelationships") === 0): ?>
                    <option value="<?php echo $key ?>" <?php echo (isset($row["ahg_field"]) && $row["ahg_field"] === $key) ? "selected" : "" ?>>
                      <?php echo htmlspecialchars(str_replace("AHG: ", "", $label)) ?>
                    </option>
                    <?php endif ?>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Auto-load default mapping for selected target type
  var targetType = "<?php echo htmlspecialchars($targetType) ?>";
  var defaultMappings = {
    <?php
    $defaultMaps = \Illuminate\Database\Capsule\Manager::table("atom_data_mapping")
      ->where("is_default", 1)
      ->get();
    $mapPairs = [];
    foreach ($defaultMaps as $dm) {
      $mapPairs[] = "'" . addslashes($dm->target_type) . "': " . $dm->id;
    }
    echo implode(", ", $mapPairs);
    ?>
  };
  
  // If there is a default mapping for this target type, load it automatically
  if (defaultMappings[targetType]) {
    var defaultId = defaultMappings[targetType];
    console.log("Auto-loading default mapping for " + targetType + ": ID " + defaultId);
    fetch("<?php echo url_for(["module" => "dataMigration", "action" => "loadMapping"]) ?>?id=" + defaultId)
      .then(r => r.json())
      .then(data => {
        if (data.success && data.mapping && data.mapping.fields) {
          applyMapping(data.mapping.fields);
          document.getElementById("currentMappingId").value = defaultId;
          console.log("Applied default mapping: " + data.name);
        }
      })
      .catch(err => console.error("Failed to load default mapping:", err));
  }
  ...
  window.toggleAhgFields = function() {
    var mode = document.getElementById("outputMode").value;
    var showAhg = (mode === "ahg_csv" || mode === "ahg_import");

    // Show/hide the AHG Extended column (header and cells)
    document.querySelectorAll(".ahg-column").forEach(function(el) {
      el.style.display = showAhg ? "" : "none";
    });
    ...
    newRow.innerHTML = `
      ...
      <td style="width:14%">
        <select name="fields[${rowIndex}][atom_field]" class="form-select form-select-sm atom-field-select">
          ...
          <optgroup label="─── AHG Extended Fields ───" class="ahg-fields-group" ${showAhg ? "" : "style=display:none"}>
            <?php foreach ($targetFields as $key => $label): ?>
              <?php if (strpos($key, "ahg") === 0 || ($key === "Filename" || (strpos($key, "digitalObject") === 0 && $key !== "digitalObjectPath" && $key !== "digitalObjectURI"))): ?>
  ...
  document.getElementById("confirmSaveMapping").addEventListener("click", function() {
    ...
    .then(function(data) {
      ...
      if (data.success) {
        ...
        saveModal.hide();
        setTimeout(function() {
          // Reload and auto-load the saved mapping
          var url = new URL(window.location.href);
          url.searchParams.set("load_mapping", data.id);
...
function applyMapping(fields) {
  ...
  // Detect if mapping contains AHG extended fields and set toggle
  var hasAhgFields = fields.some(function(f) {
    return f.ahg_field && f.ahg_field !== "";
  });
  console.log("Has AHG fields:", hasAhgFields);
  if (hasAhgFields) {
    var outputMode = document.getElementById("outputMode");
    if (outputMode && outputMode.value !== "ahg_csv" && outputMode.value !== "ahg_import") {
      outputMode.value = "ahg_csv";
      document.getElementById("outputModeInput").value = "ahg_csv";
      toggleAhgFields();
      console.log("Switched to AHG Extended mode");
    }
  }
  ...
  document.querySelectorAll('#mappingTable tbody tr').forEach(function(row) {
    ...
    if (m) {
      ...
      // Apply ahg_field (AHG Extended)
      var ahgField = m.ahg_field || '';
      var ahgSelect = row.querySelector('.ahg-field-select');
      if (ahgSelect && ahgField) {
        ahgSelect.value = ahgField;
        console.log('Setting AHG field for', source, 'to', ahgField);
      }
      // Apply constant
...
document.getElementById('backgroundJobBtn')?.addEventListener('click', function() {
  ...
  <?php if (!empty($autoLoadMappingId)): ?>
  (function() {
    var mappingId = <?php echo json_encode($autoLoadMappingId) ?>;
    console.log("Auto-loading mapping ID:", mappingId);
    fetch("<?php echo url_for(["module" => "dataMigration", "action" => "loadMapping"]) ?>?id=" + mappingId)
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data.success && data.mapping && data.mapping.fields) {
          applyMapping(data.mapping.fields);
          showToast("Loaded mapping: " + data.name, "success");
          // Update the selected mapping display
          document.getElementById("selectedMappingId").value = mappingId;
          var nameEl = document.getElementById("mappingName");
          if (nameEl) nameEl.value = data.name;
        }
      })
      .catch(function(e) { console.error("Auto-load failed:", e); });
  })();
  <?php endif; ?>
