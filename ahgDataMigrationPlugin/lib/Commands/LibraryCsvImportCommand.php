...
/**
 * Library (MARC/RDA) CSV import command.
 *
 * Imports CSV files following MARC/RDA standard for library materials.
 */
class LibraryCsvImportCommand extends BaseCommand
{
    ...
    Import library CSV data following the MARC/RDA standard into AtoM.

    Supports column mapping, validation-only mode, update/merge of existing
    records, publication date handling, and optional mapping profiles.

    Examples:
      php bin/atom sector:library-csv-import /path/to/file.csv
      php bin/atom sector:library-csv-import /path/to/file.csv --validate-only
      php bin/atom sector:library-csv-import /path/to/file.csv --repository=my-library --culture=af
      php bin/atom sector:library-csv-import /path/to/file.csv --update=identifier --update-mode=merge
      php bin/atom sector:library-csv-import /path/to/file.csv --limit=100 --skip=10
    EOF;
    ...
    protected function handle(): int
    {
        ...
        $this->info(sprintf('Processing %s for sector: library', basename($filename)));
    ...
    protected function createEvents(int $objectId, array $data): void
    {
        ...
        if ($author && $author !== $publisher) {
            $authorEvent = new \QubitEvent();
            $authorEvent->objectId = $objectId;
            $authorEvent->typeId = \QubitTerm::CREATION_ID;
            $authorEvent->date = $pubDate;
            $authorEvent->culture = $this->culture;

            $authorActor = $this->findOrCreateActor($author);
            if ($authorActor) {
                $authorEvent->actorId = $authorActor->id;
            }
    ...
    {
        try {
            $exists = DB::table('library_metadata')
