...
/**
 * Service #7: Authority Merge/Split Service (#207)
 *
 ...
 * Supports optional approval gating via ahgWorkflowPlugin.
 */
class AuthorityMergeService
{
    ...
    /**
     * Transfer all relations from secondary to primary actor.
     */
    protected function transferRelations(int $primaryId, int $secondaryId): int
    {
        $count = 0;

        // Update subject_id references
        $count += DB::table('relation')
            ->where('subject_id', $secondaryId)
            ->where('object_id', '!=', $primaryId) // Avoid self-relations
            ->update(['subject_id' => $primaryId]);

        // Update object_id references
        $count += DB::table('relation')
    ...
    /**
     * Transfer resource links (events) from secondary to primary.
     */
    ...
    /**
     * Transfer external identifiers from secondary to primary.
     */
    protected function transferIdentifiers(int $primaryId, int $secondaryId): int
    {
        ...
        foreach ($secondaryIds as $ident) {
            // Check if primary already has this type
    ...
    /**
     * Create a slug redirect so old URLs still work.
     */
    protected function createSlugRedirect(int $primaryId, int $secondaryId): void
    {
        ...
        if ($oldSlug) {
            // Update slug to point to primary
            DB::table('slug')
                ->where('object_id', $secondaryId)
                ->update(['object_id' => $primaryId]);
        }
    ...
    /**
     * Get actor detail for comparison.
     */
