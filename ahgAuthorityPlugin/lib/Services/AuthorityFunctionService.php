...
/**
 * Service #10: Authority Function Service (#201)
 *
...
class AuthorityFunctionService
{
     ...
     * Looks in information_object where level_of_description = "Function" or uses ahgFunctionManagePlugin.
     */
    public function searchFunctions(string $query, int $limit = 10): array
    {
        ...
        if (file_exists($funcServiceFile)) {
            ...
            if (class_exists('\\AhgFunctionManage\\Services\\FunctionBrowseService')) {
                $svc = new \AhgFunctionManage\Services\FunctionBrowseService();
                if (method_exists($svc, 'search')) {
                    return $svc->search($query, $limit);
                }
            }
        }

        // Fallback: search in information_object with function class
