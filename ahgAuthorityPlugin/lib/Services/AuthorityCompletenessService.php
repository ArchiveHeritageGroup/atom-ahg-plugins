...
/**
 * Service #4: Authority Completeness Service (#206)
 *
...
class AuthorityCompletenessService
{
    ...
    public const FIELD_WEIGHTS = [
        'authorized_name'   => 15,  // 5.1.2 Authorized form of name
        'entity_type'       => 5,   // 5.1.1 Type of entity
        'dates_existence'   => 10,  // 5.2.1 Dates of existence
        'history'           => 10,  // 5.2.2 History
        'places'            => 5,   // 5.2.3 Places
        'legal_status'      => 3,   // 5.2.4 Legal status
        'functions'         => 5,   // 5.2.5 Functions
        'mandates'          => 3,   // 5.2.6 Mandates/Sources of authority
        'internal_struct'   => 3,   // 5.2.7 Internal structures
        'general_context'   => 3,   // 5.2.8 General context
        'description_id'    => 3,   // 5.4.1 Description identifier
        'sources'           => 3,   // 5.4.8 Sources
        'maintenance_notes' => 2,   // 5.4.9 Maintenance notes
        'external_ids'      => 10,  // Has external identifiers (Wikidata, VIAF, etc.)
        'relations'         => 10,  // Has relations to other actors
        'resources'         => 5,   // Has linked resources (IOs)
        'contacts'          => 5,   // Has contact information
    ];
    ...
    public function calculateScore(int $actorId): array
    {
        ...
        $hasRelations = DB::table('relation')
            ...
            ->where('type_id', function ($q) {
                $q->select('id')->from('term')
                    ->whereIn('taxonomy_id', function ($q2) {
                        $q2->select('id')->from('taxonomy')
                            ->where('id', 160); // ACTOR_RELATION_TYPE taxonomy
                    })
                    ->limit(1);
            })
            ->exists();
        // Simpler check: any relation involving this actor
        $hasRelations = DB::table('relation')
            ->where(function ($q) use ($actorId) {
                $q->where('subject_id', $actorId)
                    ->orWhere('object_id', $actorId);
    ...
    /**
     * Batch calculate completeness for all actors (CLI task).
     */
    public function batchCalculate(int $limit = 0, ?callable $progressFn = null): int
    {
        ...
        foreach ($actorIds as $actorId) {
            ...
            if ($progressFn) {
                $progressFn($count, count($actorIds));
            }
        }
