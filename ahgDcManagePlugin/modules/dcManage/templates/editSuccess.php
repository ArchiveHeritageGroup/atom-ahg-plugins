...
<style <?php echo $cspNonce ? preg_replace('/^nonce=/', 'nonce="', $cspNonce).'"' : ''; ?>>
  .dcmana-width-80px-8db8 { width:80px; }
</style>
...
<?php slot('content'); ?>
  ...
  <?php $rawDcTypes = $sf_data->getRaw('dcTypeTerms'); ?>
  ...
  <form method="post" action="<?php echo $isNew ? url_for('@io_add_override') : url_for('@io_edit_override?slug=' . $rawIo['slug']); ?>" id="editForm">
    ...
    <div class="accordion mb-3" id="dcAccordion">
      ...
      <div class="accordion-item">
        ...
        <div id="elements-collapse" class="accordion-collapse collapse show" aria-labelledby="elements-heading">
          <div class="accordion-body">
            ...
            <!-- Names (actor + event type) -->
            <div class="mb-3">
              <label class="form-label"><?php echo __('Name(s)'); ?></label>
                    ...
                    <?php foreach ($rawEvents as $idx => $evt) { ?>
                      <tr>
                        ...
                        <td>
                          ...
                          <input type="hidden" name="events[<?php echo $idx; ?>][date]" value="">
                          <input type="hidden" name="events[<?php echo $idx; ?>][startDate]" value="">
                          <input type="hidden" name="events[<?php echo $idx; ?>][endDate]" value="">
                        </td>
            ...
            <!-- Date(s) — separate from Names in DC -->
            <div class="mb-3">
              <label class="form-label"><?php echo __('Date(s)'); ?></label>
              <table class="table table-sm" id="dates-table">
                <thead>
                  <tr>
                ...
                <tbody>
                  <?php if (!empty($rawEvents)) { ?>
                    <?php $dateIdx = 0; ?>
                    <?php foreach ($rawEvents as $evt) { ?>
                      <?php if (!empty($evt->date) || !empty($evt->start_date) || !empty($evt->end_date)) { ?>
                        <tr>
                          <td><input type="text" class="form-control form-control-sm" name="dcDates[<?php echo $dateIdx; ?>][date]" value="<?php echo esc_specialchars($evt->date ?? ''); ?>" placeholder="<?php echo __('e.g. ca. 1900'); ?>"></td>
                          <td><input type="date" class="form-control form-control-sm" name="dcDates[<?php echo $dateIdx; ?>][startDate]" value="<?php echo esc_specialchars($evt->start_date ?? ''); ?>"></td>
                          <td><input type="date" class="form-control form-control-sm" name="dcDates[<?php echo $dateIdx; ?>][endDate]" value="<?php echo esc_specialchars($evt->end_date ?? ''); ?>"></td>
                          <td><button type="button" class="btn btn-sm btn-outline-danger btn-remove-row"><?php echo __('Remove'); ?></button></td>
                        </tr>
                        <?php $dateIdx++; ?>
                      <?php } ?>
              ...
              <button type="button" class="btn btn-sm btn-outline-secondary" id="add-date-row"><?php echo __('Add date'); ?></button>
            </div>
            ...
            <div class="mb-3">
              ...
              <div class="form-text text-muted small"><?php echo __('An abstract, table of contents or description of the resource.'); ?></div>
            </div>
            ...
            <div class="mb-3">
              ...
              <?php $rawDcTypeAPs = $rawIo['dcTypes'] ?? []; ?>
              <div id="dctype-list">
                <?php if (!empty($rawDcTypeAPs)) { ?>
                  <?php foreach ($rawDcTypeAPs as $dt) { ?>
                    <div class="input-group input-group-sm mb-1">
                      <input type="text" class="form-control" value="<?php echo esc_specialchars($dt->term_name ?? ''); ?>" readonly>
                      <input type="hidden" name="dcTypeIds[]" value="<?php echo (int) $dt->term_id; ?>">
                      <button type="button" class="btn btn-outline-danger btn-remove-ap"><?php echo __('Remove'); ?></button>
                    </div>
                  <?php } ?>
                <?php } ?>
              ...
              <div class="input-group input-group-sm mt-1">
                <select class="form-select form-select-sm" id="dctype-select">
                  <option value=""><?php echo __('- Select type -'); ?></option>
                  <?php foreach ($rawDcTypes as $dt) { ?>
                    <option value="<?php echo $dt->id; ?>"><?php echo esc_specialchars($dt->name ?? ''); ?></option>
                  <?php } ?>
                </select>
                <button type="button" class="btn btn-outline-secondary" id="add-dctype-btn"><?php echo __('Add'); ?></button>
              </div>
              <div class="form-text text-muted small"><?php echo __('The nature or genre of the resource (DCMI Type Vocabulary).'); ?></div>
            </div>
            ...
            <!-- Format -->
            <div class="mb-3">
              <label for="extentAndMedium" class="form-label"><?php echo __('Format'); ?></label>
              ...
              <div class="form-text text-muted small"><?php echo __('The file format, physical medium, or dimensions of the resource.'); ?></div>
            </div>

            <!-- Source -->
            <div class="mb-3">
              <label for="locationOfOriginals" class="form-label"><?php echo __('Source'); ?></label>
              ...
              <div class="form-text text-muted small"><?php echo __('A related resource from which the described resource is derived.'); ?></div>
            </div>
            ...
            <!-- Repository (Relation - isLocatedAt) -->
            <div class="mb-3">
              <label for="repositoryName" class="form-label"><?php echo __('Relation (isLocatedAt)'); ?></label>
              ...
              <div class="form-text text-muted small"><?php echo __('The name of the organization which has custody of the resource.'); ?></div>
            </div>

            <!-- Coverage (Place access points) -->
            <div class="mb-3">
              <label class="form-label"><?php echo __('Coverage (spatial)'); ?></label>
            ...
            <!-- Rights -->
            <div class="mb-3">
              <label for="accessConditions" class="form-label"><?php echo __('Rights'); ?></label>
              ...
              <div class="form-text text-muted small"><?php echo __('Information about rights held in and over the resource.'); ?></div>
            </div>
                ...
                <table class="table table-sm" id="childlevels-table">
                  <thead>
                    <tr>
                      ...
                      <th class="dcmana-width-80px-8db8"></th>
                    </tr>
                  </thead>
                  <tbody></tbody>
                </table>
...
(function() {
    ...
    addEventBtn.addEventListener('click', function() {
      ...
      tr.innerHTML =
        ...
        '<td><input type="text" class="form-control form-control-sm actor-autocomplete" name="events[' + eventIdx + '][actorName]" placeholder="<?php echo __("Actor name"); ?>">' +
            '<input type="hidden" name="events[' + eventIdx + '][actorId]" value="0">' +
            '<input type="hidden" name="events[' + eventIdx + '][date]" value="">' +
            '<input type="hidden" name="events[' + eventIdx + '][startDate]" value="">' +
            '<input type="hidden" name="events[' + eventIdx + '][endDate]" value=""></td>' +
        '<td><button type="button" class="btn btn-sm btn-outline-danger btn-remove-row"><?php echo __("Remove"); ?></button></td>';
  ...
  // ── Date rows ───────────────────────────────────────────────────────
  var datesBody = document.querySelector('#dates-table tbody');
  var addDateBtn = document.getElementById('add-date-row');
  var dateIdx = datesBody ? datesBody.querySelectorAll('tr').length : 0;

  if (addDateBtn) {
    addDateBtn.addEventListener('click', function() {
      var tr = document.createElement('tr');
      tr.innerHTML =
        '<td><input type="text" class="form-control form-control-sm" name="dcDates[' + dateIdx + '][date]" placeholder="<?php echo __("e.g. ca. 1900"); ?>"></td>' +
        '<td><input type="date" class="form-control form-control-sm" name="dcDates[' + dateIdx + '][startDate]"></td>' +
        '<td><input type="date" class="form-control form-control-sm" name="dcDates[' + dateIdx + '][endDate]"></td>' +
        '<td><button type="button" class="btn btn-sm btn-outline-danger btn-remove-row"><?php echo __("Remove"); ?></button></td>';
  ...
  // ── DC Type add ─────────────────────────────────────────────────────
  var dcTypeList = document.getElementById('dctype-list');
  var dcTypeSelect = document.getElementById('dctype-select');
  var addDcTypeBtn = document.getElementById('add-dctype-btn');

  if (addDcTypeBtn && dcTypeSelect) {
    addDcTypeBtn.addEventListener('click', function() {
      var opt = dcTypeSelect.options[dcTypeSelect.selectedIndex];
      if (!opt || !opt.value) return;
      var div = document.createElement('div');
      div.className = 'input-group input-group-sm mb-1';
      div.innerHTML =
        '<input type="text" class="form-control" value="' + escHtml(opt.text) + '" readonly>' +
        '<input type="hidden" name="dcTypeIds[]" value="' + opt.value + '">' +
        '<button type="button" class="btn btn-outline-danger btn-remove-ap"><?php echo __("Remove"); ?></button>';
      dcTypeList.appendChild(div);
      dcTypeSelect.selectedIndex = 0;
    });
  }
