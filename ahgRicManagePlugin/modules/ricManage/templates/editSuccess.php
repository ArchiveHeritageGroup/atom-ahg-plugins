...
<?php slot('title'); ?>
  <div class="multiline-header d-flex flex-column mb-3">
    ...
    <span class="small text-muted"><?php echo __('Records in Context (RiC)'); ?></span>
    <?php if (!$isNew) { ?>
  ...
  <form method="post" action="<?php echo $isNew ? url_for('@io_add_override') : url_for('@io_edit_override?slug=' . $rawIo['slug']); ?>" id="editForm">
    ...
    <div class="accordion mb-3" id="isadAccordion">
      ...
      <?php
        ...
        $rawRicEntityTypes = $sf_data->getRaw('ricEntityTypes');
        $rawRicPropFields = $sf_data->getRaw('ricPropFields');
        $rawRicPreds = \AhgRicManage\Services\RicManageService::PROPERTY_PREDICATES;
      ?>
      <div class="accordion-item">
          ...
          <div class="accordion-body">
            ...
            <div class="mb-3">
              ...
              <select class="form-select" id="ricEntityType" name="ricEntityType">
                <?php foreach ($rawRicEntityTypes as $val => $label) { ?>
                  <option value="<?php echo esc_specialchars($val); ?>" <?php echo ($val === ($rawRicMeta['entity_type'] ?? 'Record')) ? 'selected' : ''; ?>><?php echo esc_specialchars($label); ?></option>
                <?php } ?>
              </select>
              <div class="form-text text-muted small"><?php echo __('How this record is typed in the Records in Context ontology.'); ?></div>
            </div>

            <?php foreach ($rawRicPropFields as $ricKey => $ricLabel) { ?>
              <div class="mb-3">
                <label for="ricProps_<?php echo esc_specialchars($ricKey); ?>" class="form-label"><?php echo esc_specialchars(__($ricLabel)); ?><?php if (!empty($rawRicPreds[$ricKey])) { ?> <span class="badge bg-light text-dark"><?php echo esc_specialchars($rawRicPreds[$ricKey]); ?></span><?php } ?></label>
                <textarea class="form-control" id="ricProps_<?php echo esc_specialchars($ricKey); ?>" name="ricProps[<?php echo esc_specialchars($ricKey); ?>]" rows="2"><?php echo esc_specialchars($rawRicMeta['properties'][$ricKey] ?? ''); ?></textarea>
              </div>
            <?php } ?>
...
(function() {
  ...
  var ricEditForm = document.getElementById('editForm');
  var ricObjId = <?php echo (int) ($rawIo['id'] ?? 0); ?>;
  var ricEtSel = document.getElementById('ricEntityType');
  if (ricEditForm && ricObjId && ricEtSel) {
    ricEditForm.addEventListener('submit', function (e) {
      if (ricEditForm.dataset.ricSaved === '1') { return; } // 2nd pass - let it go
      e.preventDefault();
      var fd = new FormData();
      fd.append('object_id', ricObjId);
      fd.append('entity_type', ricEtSel.value);
      ...
      var go = function () { ricEditForm.dataset.ricSaved = '1'; ricEditForm.submit(); };
