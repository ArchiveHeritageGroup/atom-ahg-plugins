...
DOI integration via DataCite — mint, deactivate, sync, queue processing, badge display, and verification

| | |
|---|---|
| Machine name | `ahgDoiPlugin` |
| Version | 1.0.0 |
...
cd tmp-fetch && git sparse-checkout set ahgDoiPlugin && cd ..
mv tmp-fetch/ahgDoiPlugin ./ahgDoiPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgDoiPlugin/database/install.sql
