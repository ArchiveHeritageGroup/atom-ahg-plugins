...
<div class="container-fluid py-4">
  <div class="row">
    ...
    <div class="col-lg-9 col-md-8">
      ...
      <?php elseif (empty($articleResults) && empty($sectionResults)): ?>
        <div class="alert alert-info">
          <i class="bi bi-info-circle me-1"></i>
          <?php echo __('No results found for "%1%". Try different keywords.', ['%1%' => htmlspecialchars($query)]) ?>
        </div>
      <?php else: ?>
        ...
        <?php if (!empty($articleResults)): ?>
          <h2 class="h5 mb-3">
            ...
            <span class="badge bg-primary ms-1"><?php echo count($articleResults) ?></span>
          </h2>
          <div class="list-group mb-4">
            <?php foreach ($articleResults as $result): ?>
              <a href="<?php echo url_for('@help_article_view?slug=' . urlencode($result['slug'])) ?>"
                ...
                <div class="d-flex justify-content-between align-items-start">
                  <div>
                    ...
                    <span class="badge bg-info me-1"><?php echo $result['category'] ?></span>
                    <?php if (!empty($result['subcategory'])): ?>
                      <span class="badge bg-light text-dark"><?php echo $result['subcategory'] ?></span>
                    <?php endif; ?>
        ...
        <?php if (!empty($sectionResults)): ?>
          <h2 class="h5 mb-3">
            ...
            <span class="badge bg-secondary ms-1"><?php echo count($sectionResults) ?></span>
          </h2>
          <div class="list-group mb-4">
            <?php foreach ($sectionResults as $result): ?>
              <a href="<?php echo url_for('@help_article_view?slug=' . urlencode($result['slug'])) ?>#<?php echo htmlspecialchars($result['anchor']) ?>"
                ...
                <div>
                  ...
                  <span class="badge bg-info"><?php echo $result['category'] ?></span>
                  <?php if (!empty($result['snippet'])): ?>
