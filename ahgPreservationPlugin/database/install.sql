...
-- ---------------------------------------------------------------------------
-- TIFF/PDF merge tables.
--
-- This plugin ships the tiffpdfmerge module, so it must ship that module's
-- schema too. The tables were declared only by ahgTiffPdfMergePlugin, so
-- installing preservation on its own gave a module whose every page 500'd with
-- "Table 'tiff_pdf_merge_job' doesn't exist".
--
-- IF NOT EXISTS throughout, so the two plugins can be installed together in
