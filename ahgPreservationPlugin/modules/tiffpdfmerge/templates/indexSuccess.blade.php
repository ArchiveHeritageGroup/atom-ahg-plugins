        ...
        <div class="col-lg-10">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted mb-0">
                        ...
                        Attaching to: <a href="{{ url_for(['module' => 'informationobject', 'slug' => $informationObject->slug]) }}">{{ $informationObject->title ?? $informationObject->slug }}</a>
                    </p>
                    @endif
                </div>
            ...
            <div class="card shadow-sm">
                    ...
                    <div class="mb-4 pb-4 border-bottom">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="tpmJobName" value="Merged Document {{ date('Y-m-d') }}">
                            </div>
                            <div class="col-md-3">
                ...
                <div class="card-footer bg-light">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            @if($informationObject)
                            <a href="{{ url_for(['module' => 'informationobject', 'slug' => $informationObject->slug]) }}" class="btn btn-outline-secondary">
                                <i class="fas fa-arrow-left me-1"></i>Back to Record
                            </a>
                            @else
                            <a href="{{ url_for(['module' => 'informationobject', 'action' => 'browse']) }}" class="btn btn-outline-secondary">
            ...
            <div class="card mt-4">
                ...
                <div class="list-group list-group-flush">
                    @foreach($pendingJobs as $pJob)
                    <div class="list-group-item d-flex justify-content-between align-items-center">
                        <div>
                            <strong>{{ $pJob->job_name }}</strong><br>
                            <small class="text-muted">{{ $pJob->file_count }} files &bull; {{ $pJob->created_at }}</small>
                        </div>
                        <span class="badge bg-{{ $pJob->status === 'pending' ? 'warning' : ($pJob->status === 'processing' ? 'info' : ($pJob->status === 'completed' ? 'success' : 'danger')) }}">
                            {{ ucfirst($pJob->status) }}
                        </span>
                    </div>
                    @endforeach
...
(function() {
    ...
    async function createJob() {
        try {
            const response = await fetch('{{ url_for(['module' => 'tiffpdfmerge', 'action' => 'create']) }}', {
                method: 'POST',
    ...
    async function processFiles(files) {
        ...
        for (const file of validFiles) {
            ...
            try {
                ...
                const res = await fetch('{{ url_for(['module' => 'tiffpdfmerge', 'action' => 'upload']) }}', { method: 'POST', body: fd });
    ...
    window.removeFile = async function(id) {
        ...
        try {
            await fetch('{{ url_for(['module' => 'tiffpdfmerge', 'action' => 'removeFile']) }}', {
    ...
    async function createPdf() {
        ...
        btn.disabled = true;
        try {
            const res = await fetch('{{ url_for(['module' => 'tiffpdfmerge', 'action' => 'process']) }}', {
                method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    ...
    async function clearAll() {
        ...
        if (currentJob) {
            try { await fetch('{{ url_for(['module' => 'tiffpdfmerge', 'action' => 'delete']) }}', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams({ job_id: currentJob }) }); } catch (e) {}
        }
        uploadedFiles = [];
        updateFileList();
        updateButtons();
