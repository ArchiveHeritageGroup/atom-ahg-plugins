@section('title', $schedule ? __('Edit Schedule') : __('New Schedule'))

<div class="d-flex justify-content-between align-items-center mb-4">
    <h1>
        <i class="fas fa-{{ $schedule ? 'edit' : 'plus' }}"></i>
        {{ $schedule ? __('Edit Schedule') : __('New Schedule') }}
    </h1>
    <a href="{{ url_for(['module' => 'preservation', 'action' => 'scheduler']) }}" class="btn btn-outline-secondary">
                ...
                <form method="post">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select class="form-select" id="workflow_type" name="workflow_type" required>
                                @foreach($workflowTypes as $value => $label)
                                    <option value="{{ $value }}"
                                            {{ ($schedule->workflow_type ?? '') === $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="2"
                                  placeholder="Optional description of this schedule">{{ $schedule->description ?? '' }}</textarea>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="batch_limit" name="batch_limit"
                                   value="{{ $schedule->batch_limit ?? 100 }}" min="1" max="10000">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" class="form-control" id="timeout_minutes" name="timeout_minutes"
                                   value="{{ $schedule->timeout_minutes ?? 60 }}" min="1" max="480">
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" id="notify_on_failure" name="notify_on_failure"
                                       {{ ($schedule->notify_on_failure ?? 1) ? 'checked' : '' }}>
                    ...
                    <div class="d-flex justify-content-between">
                        <a href="{{ url_for(['module' => 'preservation', 'action' => 'scheduler']) }}"
                           class="btn btn-outline-secondary">
                            <i class="fas fa-times me-1"></i>Cancel
                        </a>
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-1"></i>{{ $schedule ? 'Update Schedule' : 'Create Schedule' }}
                        </button>
                    </div>
                </form>
            </div>
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item">
                        <div class="d-flex justify-content-between align-items-center">
                            <small>
                                {{ date('Y-m-d H:i', strtotime($run->started_at)) }}
                                <br>
        ...
        <div class="card mt-4">
            ...
            <div class="card-body">
                <dl class="row mb-0">
                    ...
                    <dd class="col-6">{{ number_format($schedule->total_runs) }}</dd>
                    <dt class="col-6">Total Processed</dt>
                    <dd class="col-6">{{ number_format($schedule->total_processed) }}</dd>
                    <dt class="col-6">Created</dt>
                    <dd class="col-6"><small>{{ $schedule->created_at ? date('Y-m-d', strtotime($schedule->created_at)) : '-' }}</small></dd>
