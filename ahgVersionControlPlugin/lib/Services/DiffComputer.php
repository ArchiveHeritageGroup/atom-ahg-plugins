 ...
 * for rendering in the version-history UI (Phase G).
 *
 * Output shape (all sections are present; empty arrays when no changes):
 *
 *   [
 *     'scalar_changes'          => [ ['field' => 'identifier', 'old' => '..', 'new' => '..'], ... ],
 *     'i18n_changes'            => [ ['culture' => 'en', 'field' => 'title', 'old' => '..', 'new' => '..', 'long_text_diff' => '...html...'], ... ],
 *     'access_points_added'     => [ ['term_id' => 5, 'start_date' => null, 'end_date' => null], ... ],
 *     'access_points_removed'   => [ ... ],
 *     'events_added'            => [ ... ],
 *     'events_removed'          => [ ... ],
 *     'relations_added'         => [ ... ],
 *     'relations_removed'       => [ ... ],
 *     'physical_objects_added'  => [ ... ],
 *     'physical_objects_removed'=> [ ... ],
 *     'custom_fields_changes'   => [ ['field_definition_id' => 7, 'old' => '..', 'new' => '..'], ... ],
 *   ]
 *
 * For text fields longer than LONG_TEXT_THRESHOLD characters the entry gains a
 * `long_text_diff` key containing an inline HTML diff with <ins>/<del> spans.
 * Output is sanitised — only text content plus the two whitelisted tags.
 *
 * Noise filter: dropping lft / rgt / oai_local_identifier from scalar_changes
 * because those drift on every tree operation and would otherwise dominate the
 * diff. They're still part of the snapshot for restore correctness.
 *
 * @phase E
 */
class DiffComputer
{
    /** Character threshold above which a text field gets an inline word-diff. */
    public const LONG_TEXT_THRESHOLD = 200;

    /** Base columns excluded from scalar_changes (system / tree noise). */
    private const BASE_NOISE_FIELDS = ['lft', 'rgt', 'oai_local_identifier'];

    /**
     * Public entry point: structured diff between two snapshots.
     *
        ...
        foreach ($cultures as $culture) {
            ...
            if ($oldRow === null && $newRow !== null) {
                foreach ($newRow as $field => $val) {
                    ...
                    $row = ['culture' => $culture, 'field' => $field, 'old' => null, 'new' => $val, 'change_kind' => 'culture_added'];
                    $out[] = $this->maybeAttachLongTextDiff($row);
                }
                continue;
            }
            // Whole culture removed
            if ($oldRow !== null && $newRow === null) {
                foreach ($oldRow as $field => $val) {
                    if ($field === 'culture' || $field === 'id' || $val === null || $val === '') {
                        continue;
                    }
                    $row = ['culture' => $culture, 'field' => $field, 'old' => $val, 'new' => null, 'change_kind' => 'culture_removed'];
                    $out[] = $this->maybeAttachLongTextDiff($row);
                }
                continue;
            }
            // Per-field diff inside the culture
            ...
            foreach ($fields as $field) {
                ...
                $row = ['culture' => $culture, 'field' => $field, 'old' => $oldVal, 'new' => $newVal];
                $out[] = $this->maybeAttachLongTextDiff($row);
            }
        }
        return $out;
    ...
    /**
     * Symmetric set diff between two collections of associative rows.
     * Uses canonical JSON for each row as the membership key so element order
     * within the row doesn't cause false positives.
     *
     * @param array<int,array<string,mixed>> $old
     * @param array<int,array<string,mixed>> $new
    ...
    /**
     * Custom fields are diffed by `(field_definition_id, sequence)` so that
     * the same field changing value is reported as a CHANGE rather than as a
     * remove + add pair.
     *
     * @param array<int,array<string,mixed>> $old
     * @param array<int,array<string,mixed>> $new
    ...
    /**
     * Attach an inline HTML word-diff when either old or new is a long string.
     *
    ...
    /**
     * Inline word-level diff. Returns HTML with <ins>added words</ins> and
     * <del>removed words</del> on a backing of plain escaped text.
     *
     * Algorithm: standard LCS table over the tokenised word lists, then a
     * reverse walk producing the inline output. O(n*m) memory and time —
     * fine for typical AtoM field sizes; long fields (>5 KB) are chunked.
     */
    private function inlineWordDiff(string $old, string $new): string
    {
        ...
        // Reverse walk → ordered ops: ['eq'|'del'|'ins', token]
        $ops = [];
        $i = $n; $j = $m;
        while ($i > 0 && $j > 0) {
            if ($a[$i - 1] === $b[$j - 1]) {
        ...
        // Coalesce consecutive same-kind ops and render
        $out = '';
        $kind = null;
        $buf = '';
    ...
    /**
     * Tokenise into words + whitespace separators preserved as their own tokens.
     * This way the rendered diff keeps natural spacing.
     *
    ...
    /**
     * Canonical JSON for set-diff membership keys (mirror of VersionWriter helper).
     */
