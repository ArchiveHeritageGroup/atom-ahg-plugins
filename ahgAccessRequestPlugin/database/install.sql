...
CREATE TABLE IF NOT EXISTS `access_request` (
  ...
  `justification` text,
  `urgency` VARCHAR(39) COMMENT 'low, normal, high, critical' DEFAULT 'normal',
  `status` VARCHAR(57) COMMENT 'pending, approved, denied, cancelled, expired' DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS `access_request_log` (
  ...
  `action` VARCHAR(77) COMMENT 'created, updated, approved, denied, cancelled, expired, escalated' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `access_request_scope` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `request_id` int unsigned NOT NULL,
  `object_type` VARCHAR(49) COMMENT 'information_object, repository, actor' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `security_access_request` (
  ...
  `request_type` VARCHAR(81) COMMENT 'view, download, print, clearance_upgrade, compartment_access, renewal' NOT NULL,
  `justification` text NOT NULL,
  `duration_hours` int DEFAULT NULL,
  `priority` VARCHAR(37) COMMENT 'normal, urgent, immediate' DEFAULT 'normal',
  `status` VARCHAR(57) COMMENT 'pending, approved, denied, expired, cancelled' DEFAULT 'pending',
  `reviewed_by` int unsigned DEFAULT NULL,
