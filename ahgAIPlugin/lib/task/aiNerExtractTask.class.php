 ...
 */
class aiNerExtractTask extends sfBaseTask
{
    protected function configure()
    {
        ...
        $this->namespace = 'ai';
        $this->name = 'ner-extract';
        $this->briefDescription = 'Extract named entities from archival records';
        $this->detailedDescription = <<<EOD
The [ai:ner-extract|INFO] task extracts named entities from records.

Examples:
  [php symfony ai:ner-extract --all --limit=100|INFO]
  [php symfony ai:ner-extract --all --with-pdf --limit=100|INFO]
  [php symfony ai:ner-extract --object=12345|INFO]
EOD;
    }
    ...
    protected function execute($arguments = [], $options = [])
    {
        ...
        $this->logSection('ai', 'Starting NER extraction task');
        if ($options['with-pdf']) {
            $this->logSection('ai', 'PDF extraction enabled');
        }

        $nerSettings = $this->getSettings('ner');
        if (($nerSettings['enabled'] ?? '1') !== '1') {
            $this->logSection('ai', 'NER is disabled in settings', null, 'ERROR');
            return 1;
        }
        ...
        foreach ($objectIds as $objectId) {
            ...
            try {
                if ($options['queue']) {
                    ...
                    $this->logSection('ai', "Queued: $objectId");
                } else {
