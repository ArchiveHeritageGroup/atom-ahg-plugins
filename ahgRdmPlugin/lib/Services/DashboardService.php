...
/**
 * RDM dashboard — reverse port of Heratio ahg-rdm DashboardService (heratio#1337
 * Feature 3 + #1345 filters). The RDM unit's at-a-glance operational + strategic
 * view: headline KPIs, POPIA/disposition/method breakdowns, finding composition,
 * deposit trend, per-faculty posture, and the human-gate backlog.
 *
 * Read-only aggregation over the rdm_* tables (+ research_project for faculty).
 * The compliance scoreboard stays as the per-dataset drill-down; this is the
 * roll-up above it. Everything is guarded so it renders without the DMP slice.
 *
 * Filters: from / to (deposit date) + institution — resolved to a single set of
 * matching dataset ids that scopes every aggregate, so filter logic lives in one
 * place. No filter => null id-set => unscoped (full view).
 */
class DashboardService
{
    ...
    {
        $hasDmp = $this->columnExists('rdm_dataset', 'dmp_id');
        ...
        // Finding composition — by PII type and by detection method (AI-vs-rule split).
     ...
     * date-range filter (trend context, not a filtered count).
     */
    private function depositsByMonth(?string $institution): array
    {
        ...
        $raw = $q
            ...
            ->where('d.created_at', '>=', date('Y-m-01', strtotime('first day of -11 months')))
