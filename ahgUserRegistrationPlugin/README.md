...
cd tmp-fetch && git sparse-checkout set ahgUserRegistrationPlugin && cd ..
mv tmp-fetch/ahgUserRegistrationPlugin ./ahgUserRegistrationPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgUserRegistrationPlugin/database/install.sql
