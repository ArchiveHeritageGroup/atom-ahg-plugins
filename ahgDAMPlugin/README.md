...
cd tmp-fetch && git sparse-checkout set ahgDAMPlugin && cd ..
mv tmp-fetch/ahgDAMPlugin ./ahgDAMPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgDAMPlugin/database/install.sql
