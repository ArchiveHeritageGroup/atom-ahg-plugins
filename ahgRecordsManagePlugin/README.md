...
Records-management file plan / classification scheme — a nested-set tree of classification nodes (function/series/file) with retention and disposal-action metadata. PSIS-parity port of the Heratio ahg-records-manage FilePlanService (#118). Email capture and full disposal/retention execution are follow-ups.

| | |
|---|---|
| Machine name | `ahgRecordsManagePlugin` |
| Version | 0.1.0 |
...
cd tmp-fetch && git sparse-checkout set ahgRecordsManagePlugin && cd ..
mv tmp-fetch/ahgRecordsManagePlugin ./ahgRecordsManagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgRecordsManagePlugin/database/install.sql
