...
Duplicate detection and merging for archival records — configurable rules, scan jobs, compare view, merge workflow, and reports

| | |
|---|---|
| Machine name | `ahgDedupePlugin` |
| Version | 1.0.0 |
...
cd tmp-fetch && git sparse-checkout set ahgDedupePlugin && cd ..
mv tmp-fetch/ahgDedupePlugin ./ahgDedupePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgDedupePlugin/database/install.sql
