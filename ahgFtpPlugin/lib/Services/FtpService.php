...
namespace AhgFtpPlugin\Services;
...
class FtpService
{
    ...
    /**
     * Create instance from ahg_settings.
     */
     ...
     *
     * @return array ['success' => bool, 'files' => [['name', 'size', 'modified'], ...]]
     */
    ...
    /**
     * Delete every file in the upload folder. Reuses listFiles()/deleteFile()
     * so it works for local, SFTP and FTP. Returns the count deleted.
     */
    ...
    protected function testFtp(): array
    {
        ...
        // Try listing remote path to verify it exists
    ...
    protected function ftpListFiles(): array
    {
        ...
        foreach ($rawList as $line) {
            ...
            // Parse Unix-style: -rw-r--r-- 1 user group 12345 Jan  1 12:00 filename
    ...
    /**
     * Build sshpass prefix for SFTP commands.
     */
    ...
    /**
     * Common SSH options (disable host key checking for internal use).
     */
...
SFTPEOF";
    ...
    protected function sftpListFiles(): array
    {
        ...
        foreach ($output as $line) {
            $line = trim($line);
            // Skip non-listing lines (sftp prompts, blank, directories)
    ...
    /**
     * Sanitize filename — basename only, reject traversal attempts.
     */
