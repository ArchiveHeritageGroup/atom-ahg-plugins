...
| | |
|---|---|
| Machine name | `ahgRequestToPublishPlugin` |
| Version | 1.0.3 |
...
cd tmp-fetch && git sparse-checkout set ahgRequestToPublishPlugin && cd ..
mv tmp-fetch/ahgRequestToPublishPlugin ./ahgRequestToPublishPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgRequestToPublishPlugin/database/install.sql
