...
Archaeological site, find and stratigraphic context recording - single-context recording with Harris Matrix, for excavation archives

| | |
|---|---|
| Machine name | `ahgArchaeologyPlugin` |
| Version | 0.1.0 |
| Category | heritage |
...
- `archaeology_object`
- `archaeology_context`
- `archaeology_context_relationship`
...
cd tmp-fetch && git sparse-checkout set ahgArchaeologyPlugin && cd ..
mv tmp-fetch/ahgArchaeologyPlugin ./ahgArchaeologyPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgArchaeologyPlugin/database/install.sql
