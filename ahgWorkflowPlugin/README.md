...
Configurable approval workflow system for archival submissions with role-based task routing, pool claiming, and email notifications

| | |
|---|---|
| Machine name | `ahgWorkflowPlugin` |
| Version | 1.0.2 |
| Category | workflow |
...
cd tmp-fetch && git sparse-checkout set ahgWorkflowPlugin && cd ..
mv tmp-fetch/ahgWorkflowPlugin ./ahgWorkflowPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgWorkflowPlugin/database/install.sql
