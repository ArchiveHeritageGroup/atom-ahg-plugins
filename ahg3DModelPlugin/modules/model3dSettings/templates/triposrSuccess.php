<?php
/**
 * TripoSR Settings Template
 * Dedicated page for TripoSR image-to-3D configuration
 */
$settings = $sf_data->getRaw('settings');
$health = $sf_data->getRaw('health') ?? ['status' => 'unknown'];
$stats = $sf_data->getRaw('stats');
$recentJobs = $sf_data->getRaw('recentJobs');

$isOnline = ($health['status'] ?? '') === 'ok';

function getTripoSetting($settings, $key, $default = '') {
    return isset($settings[$key]) ? $settings[$key]->setting_value : $default;
}

function isTripoSettingEnabled($settings, $key) {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div>
        <a href="<?php echo url_for(['module' => 'model3dSettings', 'action' => 'index']) ?>" class="btn btn-outline-secondary me-2">
            <i class="fas fa-arrow-left me-1"></i>Back to 3D Settings
        </a>
        <a href="<?php echo url_for(['module' => 'model3d', 'action' => 'index']) ?>" class="btn btn-outline-primary">
...
<!-- Status Banner -->
<div class="alert <?php echo $isOnline ? 'alert-success' : 'alert-danger' ?> mb-4">
    <div class="d-flex align-items-center justify-content-between">
        <div>
            <i class="fas <?php echo $isOnline ? 'fa-check-circle' : 'fa-times-circle' ?> me-2"></i>
            <strong>TripoSR Service:</strong>
            <?php if ($isOnline): ?>
                Online - <?php echo $health['device'] ?? 'unknown' ?> (<?php echo $health['mode'] ?? 'local' ?> mode)
                <?php if ($health['cuda_available'] ?? false): ?>
                    <span class="badge bg-success ms-2">CUDA Available</span>
                <?php endif ?>
                <?php if ($health['model_loaded'] ?? false): ?>
                    <span class="badge bg-info ms-2">Model Loaded</span>
                <?php endif ?>
            <?php else: ?>
                Offline - <?php echo $health['message'] ?? 'Service not responding' ?>
            <?php endif ?>
        </div>
        <?php if ($isOnline && $health['remote_configured'] ?? false): ?>
...
<div class="row">
    ...
    <div class="col-md-3">
        <div class="card text-center mb-4">
            <div class="card-body">
                <div class="display-6 text-primary"><?php echo number_format($stats['total_jobs'] ?? 0) ?></div>
    ...
    <div class="col-md-3">
        <div class="card text-center mb-4">
            <div class="card-body">
                <div class="display-6 text-success"><?php echo number_format($stats['completed'] ?? 0) ?></div>
    ...
    <div class="col-md-3">
        <div class="card text-center mb-4">
            <div class="card-body">
                <div class="display-6 text-danger"><?php echo number_format($stats['failed'] ?? 0) ?></div>
    ...
    <div class="col-md-3">
        <div class="card text-center mb-4">
            <div class="card-body">
                <div class="display-6 text-warning"><?php echo number_format($stats['pending'] ?? 0) ?></div>
        ...
        <div class="col-md-8">
            ...
            <div class="card mb-4">
                <div class="card-header bg-light">
                    <h5 class="mb-0"><i class="fas fa-cog me-2"></i>Service Configuration</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-check mb-3">
                                <input class="form-check-input" type="checkbox" id="triposr_enabled" name="triposr_enabled" value="1"
                                       <?php echo isTripoSettingEnabled($settings, 'triposr_enabled') ? 'checked' : '' ?>>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="triposr_api_url"
                                       value="<?php echo esc_entities(getTripoSetting($settings, 'triposr_api_url', 'http://127.0.0.1:5050')) ?>">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="triposr_mode" id="triposr_mode">
                                    <option value="local" <?php echo getTripoSetting($settings, 'triposr_mode', 'local') == 'local' ? 'selected' : '' ?>>
                                        Local Processing (CPU/GPU)
                                    </option>
                                    <option value="remote" <?php echo getTripoSetting($settings, 'triposr_mode') == 'remote' ? 'selected' : '' ?>>
                                        Remote GPU Server
                                    </option>
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" name="triposr_timeout"
                                       value="<?php echo getTripoSetting($settings, 'triposr_timeout', '300') ?>"
                                       min="60" max="600">
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="mb-3">
                                ...
                                <input type="url" class="form-control" name="triposr_remote_url"
                                       value="<?php echo esc_entities(getTripoSetting($settings, 'triposr_remote_url')) ?>"
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <?php
                                $apiKey = getTripoSetting($settings, 'triposr_remote_api_key');
                                $maskedKey = $apiKey ? '***' : '';
                                ?>
            ...
            <div class="card mb-4">
                <div class="card-header bg-light">
                    <h5 class="mb-0"><i class="fas fa-sliders-h me-2"></i>Default Generation Options</h5>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-check mb-3">
                                <input class="form-check-input" type="checkbox" id="triposr_remove_bg" name="triposr_remove_bg" value="1"
                                       <?php echo getTripoSetting($settings, 'triposr_remove_bg', '1') === '1' ? 'checked' : '' ?>>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" name="triposr_foreground_ratio"
                                       value="<?php echo getTripoSetting($settings, 'triposr_foreground_ratio', '0.85') ?>"
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                <label class="form-label">Mesh Resolution</label>
                                <select class="form-select" name="triposr_mc_resolution">
                                    <option value="128" <?php echo getTripoSetting($settings, 'triposr_mc_resolution', '256') == '128' ? 'selected' : '' ?>>
                                        128 - Fast (lower quality)
                                    </option>
                                    <option value="256" <?php echo getTripoSetting($settings, 'triposr_mc_resolution', '256') == '256' ? 'selected' : '' ?>>
                                        256 - Balanced (recommended)
                                    </option>
                                    <option value="512" <?php echo getTripoSetting($settings, 'triposr_mc_resolution', '256') == '512' ? 'selected' : '' ?>>
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="triposr_bake_texture" name="triposr_bake_texture" value="1"
                               <?php echo isTripoSettingEnabled($settings, 'triposr_bake_texture') ? 'checked' : '' ?>>
                        <label class="form-check-label" for="triposr_bake_texture">
                            <strong>Bake Texture</strong>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        ?>
                                        <span class="badge bg-<?php echo $statusClass ?>"><?php echo $job->status ?></span>
                                    </td>
                                    <td>
                                        <?php echo $job->processing_time ? round($job->processing_time, 1) . 's' : '-' ?>
                                    </td>
                                </tr>
                                <?php endforeach ?>
                            </tbody>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="small">
                        ...
                        <code class="d-block mb-3">php symfony triposr:generate --image=/path/image.jpg</code>

                        <p class="mb-2"><strong>With object link:</strong></p>
                        <code class="d-block mb-3">php symfony triposr:generate --image=/path/image.jpg --object-id=123 --import</code>

                        <p class="mb-2"><strong>Check health:</strong></p>
                        <code class="d-block mb-3">php symfony triposr:generate --health</code>

                        <p class="mb-2"><strong>View statistics:</strong></p>
                        <code class="d-block">php symfony triposr:generate --stats</code>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const remoteCard = document.getElementById('remote_config_card');
