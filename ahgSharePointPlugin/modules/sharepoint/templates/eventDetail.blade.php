@php $title = sprintf(__('SharePoint event #%d'), (int) $event->id); @endphp
<h1>{{ $title }}</h1>
...
<dl class="row">
    ...
    <dt class="col-sm-3">{{ __('AtoM information_object') }}</dt>
    <dd class="col-sm-9">{{ $event->information_object_id ?? '—' }}</dd>
</dl>
...
<form method="post" class="mt-3">
    ...
    <a href="{{ url_for(['module' => 'sharepoint', 'action' => 'events']) }}" class="btn btn-link">{{ __('Back to event log') }}</a>
</form>
