@php $title = __('SharePoint inbound events'); @endphp
<h1>{{ $title }}</h1>
...
<table class="table table-sm table-striped">
    <thead>
        <tr>
            <th>{{ __('ID') }}</th><th>{{ __('Received') }}</th><th>{{ __('Drive') }}</th>
            <th>{{ __('Item') }}</th><th>{{ __('Change') }}</th><th>{{ __('Status') }}</th>
            <th>{{ __('Attempts') }}</th><th>{{ __('IO') }}</th><th></th>
        </tr>
    </thead>
    <tbody>
    @php
    ...
    @foreach ($events as $ev)
        <tr>
            ...
            <td><a href="{{ url_for(['module' => 'sharepoint', 'action' => 'eventDetail', 'id' => $ev->id]) }}" class="btn btn-sm btn-outline-secondary">{{ __('Detail') }}</a></td>
        </tr>
    @endforeach
    </tbody>
</table>
