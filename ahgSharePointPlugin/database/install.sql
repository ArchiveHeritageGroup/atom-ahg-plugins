-- =====================================================
-- ahgSharePointPlugin Database Schema
-- =====================================================
-- Microsoft 365 SharePoint integration.
-- Phase 1 tables: tenant, drive, mapping, sync_state.
-- Phase 2 tables: subscription, event.
-- All tables created up-front; Phase 2 tables remain unused until that phase ships.
-- Mirrored verbatim in heratio/packages/ahg-sharepoint/database/install.sql.

SET FOREIGN_KEY_CHECKS = 0;

-- -----------------------------------------------------
-- 1. sharepoint_tenant
-- Azure AD tenant + app-registration binding. One row per institution (most installs = 1).
-- -----------------------------------------------------
...
-- -----------------------------------------------------
-- 2. sharepoint_drive
-- Registered SP sites + document libraries available for ingest.
-- Site and drive collapsed; ingest always operates at drive level.
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS sharepoint_drive (
    ...
    default_repository_id INT DEFAULT NULL COMMENT 'AtoM repository for ingested records',
    default_parent_id INT DEFAULT NULL COMMENT 'AtoM info_object for placement',
...
-- Per-drive column mapping. Persistent profiles per drive/content-type.
-- Kept separate from ingest_mapping (which is keyed on session_id for per-run mappings).
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS sharepoint_mapping (
    ...
    target_field VARCHAR(255) NOT NULL COMMENT 'AtoM field name',
...
-- Delta token (per-drive cursor) for Graph delta queries.
-- -----------------------------------------------------
...
-- Graph webhook subscription state. Subscriptions expire and must be renewed.
-- -----------------------------------------------------
...
-- Inbound webhook events. Idempotency table.
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS sharepoint_event (
    ...
    last_error TEXT DEFAULT NULL,
    queue_job_id INT DEFAULT NULL COMMENT 'FK to ahg_queue_job (no constraint to avoid cross-plugin coupling)',
    ingest_job_id INT DEFAULT NULL COMMENT 'FK to ingest_job',
    information_object_id INT DEFAULT NULL COMMENT 'AtoM IO created/updated',
...
-- AAD object id -> AtoM user id. Auto-created on first push by default.
-- -----------------------------------------------------
