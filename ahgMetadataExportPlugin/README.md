...
Unified export framework supporting 10 metadata standards across GLAM sectors (Galleries, Libraries, Archives, Museums) and digital preservation.

| | |
|---|---|
| Machine name | `ahgMetadataExportPlugin` |
| Version | 1.0.0 |
| Category | export |
...
cd tmp-fetch && git sparse-checkout set ahgMetadataExportPlugin && cd ..
mv tmp-fetch/ahgMetadataExportPlugin ./ahgMetadataExportPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgMetadataExportPlugin/database/install.sql
