...
/**
 * Loads a digital object's sidecar metadata (digital_object_metadata,
 * dam_iptc_metadata, media_metadata) and projects it into the three C2PA 2.1
 * Standard Metadata Assertions: stds.exif / stds.iptc / stds.xmp.
 *
 * PII gate (#751): GPS keys are stripped from stds.exif and a _pii_redacted
 * marker set when ahg_pii_finding_embedded has a pending/escalated
 * gps_coordinate finding. Fail-open when the table is absent.
 * Ported from Heratio ahg-c2pa.
 *
...
final class StandardMetadataLoader
{
    public function loadExif(int $digitalObjectId): array
    {
        ...
        if ($this->hasPendingGpsFinding($digitalObjectId)) {
            ...
            unset($out['Exif/GPSLatitude'], $out['Exif/GPSLatitudeRef'], $out['Exif/GPSLongitude'], $out['Exif/GPSLongitudeRef']);
            if ($hadGps) {
    ...
    private function hasPendingGpsFinding(int $digitalObjectId): bool
    {
        if (!$this->tableExists('ahg_pii_finding_embedded')) {
            return false; // fail-open: #751 not deployed
        }
        try {
    ...
    public function loadIptc(int $digitalObjectId, ?int $objectId = null): array
    {
        ...
        if ($this->tableExists('dam_iptc_metadata')) {
            try {
                if ($objectId !== null) {
                    $row = (array) DB::table('dam_iptc_metadata')->where('object_id', $objectId)->first();
                }
                if (!$row) {
                    $row = (array) DB::table('dam_iptc_metadata')->where('object_id', $digitalObjectId)->first();
                }
            } catch (Throwable $e) {
                $row = null;
            }
        ...
        if (isset($row['keywords']) && is_string($row['keywords']) && trim($row['keywords']) !== '') {
            $parts = array_values(array_filter(array_map('trim', preg_split('/[,;]\s*/', (string) $row['keywords']) ?: []), fn ($s) => $s !== ''));
            if ($parts !== []) {
                $out['Keywords'] = $parts;
            }
        }
    ...
    public function loadXmp(int $digitalObjectId, ?int $objectId = null): array
    {
        ...
        if ($keywordsSrc !== null) {
            $parts = array_values(array_filter(array_map('trim', preg_split('/[,;]\s*/', (string) $keywordsSrc) ?: []), fn ($s) => $s !== ''));
            if ($parts !== []) {
    ...
    private static function nonEmptyString($v): bool
    {
