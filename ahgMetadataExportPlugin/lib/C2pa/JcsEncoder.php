...
/**
 * RFC 8785 JSON Canonicalization Scheme (JCS) encoder. Deterministic UTF-8
 * bytes for any JSON-serialisable value so two implementations agree on the
 * bytes to hash/sign. Ported from the Heratio inference-receipts reference.
 *
 *   - object keys sorted by UTF-16 code units (§3.2.3)
 *   - no insignificant whitespace
 *   - numbers per ECMA-262 (shortest round-trip)
 *   - strings escaped per JCS minimal rules
 *
...
final class JcsEncoder
{
    ...
    private static function serialize($value, int $depth): string
    {
        if ($depth > 512) {
