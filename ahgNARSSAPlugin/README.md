# NARSSA Transfer (South Africa)

South African National Archives and Records Service Act 1996 compliance — generates NARSSA-compliant transfer manifests (METS-EAD2002 XML + digital files + manifest.csv) for records reaching disposal-due via the ahgExtendedRightsPlugin disposal workflow.

| | |
|---|---|
| Machine name | `ahgNARSSAPlugin` |
| Version | 0.1.0 |
| Category | compliance |
...
cd tmp-fetch && git sparse-checkout set ahgNARSSAPlugin && cd ..
mv tmp-fetch/ahgNARSSAPlugin ./ahgNARSSAPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgNARSSAPlugin/database/install.sql
