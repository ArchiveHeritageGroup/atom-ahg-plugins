...
Chain of custody and provenance tracking for archival records, museum objects, and library materials, plus AI inference provenance recording and Ed25519 manifest signing

| | |
|---|---|
| Machine name | `ahgProvenancePlugin` |
| Version | 1.2.3 |
| Category | provenance |
...
cd tmp-fetch && git sparse-checkout set ahgProvenancePlugin && cd ..
mv tmp-fetch/ahgProvenancePlugin ./ahgProvenancePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgProvenancePlugin/database/install.sql
