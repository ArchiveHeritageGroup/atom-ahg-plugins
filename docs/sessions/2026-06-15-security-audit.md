...
- **FIXED — locked-plugin batch (Johan-authorized 2026-06-15):** nonced the genuine inline scripts in **ahgGalleryPlugin** (indexSuccess: 2 `type="importmap"` + 2 `type="module"` inline; the `type="module" src=` ones left as external), **ahgLibraryPlugin** (kbartVendor:201, z3950 editSuccess:176, opac:359 — one bare `<script>` each), **ahgSecurityClearancePlugin** (compliance index.blade:295 → `<script @cspNonce>`, the blade directive). All lint clean.
  - **Verified FALSE POSITIVES, no change:** ahgThemeB5Plugin `_show3D:20` (external model-viewer `src=`), ahgStorageManagePlugin `browseSuccess:4` (a *comment*), ahgCorePlugin `_tagManager:5` (the empty-nonce fallback branch — only runs when there's no nonce to add).
  - **Not done (optional hygiene, destructive):** deleting dead `heritage.bak/` + `*.class.php.bak` in ahgThemeB5Plugin — left in place (they never render → no CSP risk; deletion offers no security benefit). Offer-only.
