...
- **[DEFERRED — locked plugins, need explicit unlock]** ahgSecurityClearancePlugin `SecurityClearanceService.php:1280` (clearance code md5(uniqid(mt_rand))) and ahgCorePlugin `user/editAction.class.php:369` (API key openssl_random_pseudo_bytes) — both DO-NOT-MODIFY/double-locked. Same one-line random_bytes fix; awaiting go-ahead to touch.
