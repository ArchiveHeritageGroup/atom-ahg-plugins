...
CREATE TABLE IF NOT EXISTS `registry_institution` (
  ...
  `size` VARCHAR(42) COMMENT 'small, medium, large, national' DEFAULT NULL,
  `governance` VARCHAR(73) COMMENT 'public, private, ngo, academic, government, tribal, community' DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `registry_vendor` (
  ...
  `name` varchar(255) NOT NULL,
  `slug` varchar(255) NOT NULL,
  `vendor_type` VARCHAR(103) COMMENT 'developer, integrator, consultant, service_provider, hosting, digitization, training, other' NOT NULL,
  ...
  `established_year` int DEFAULT NULL,
  `team_size` VARCHAR(39) COMMENT 'solo, 2-5, 6-20, 21-50, 50+' DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `registry_instance` (
  ...
  `instance_type` VARCHAR(59) COMMENT 'production, staging, development, demo, offline' DEFAULT 'production',
  `software` varchar(100) DEFAULT 'heratio',
  `software_version` varchar(50) DEFAULT NULL,
  `hosting` VARCHAR(51) COMMENT 'self_hosted, cloud, vendor_hosted, saas' DEFAULT NULL,
  ...
  `status` VARCHAR(56) COMMENT 'online, offline, maintenance, decommissioned' DEFAULT 'online',
  `is_public` tinyint(1) DEFAULT 1,
  `description` text,
