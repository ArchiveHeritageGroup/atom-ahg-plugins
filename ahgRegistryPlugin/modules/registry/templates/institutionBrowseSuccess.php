...
<?php
  ...
  $sortKey = in_array($sortKey, ['name', 'created_at', 'instance_count', 'country'], true) ? $sortKey : 'name';
  ...
  $baseParams = [
    ...
    'size' => $sf_request->getParameter('size', ''),
    'governance' => $sf_request->getParameter('governance', ''),
    'uses_atom' => $sf_request->getParameter('uses_atom', ''),
    'sort' => $sortKey,
    'dir' => $sortDir,
    'view' => $viewMode,
  ];
  ...
  $sortOptions = [
    ...
    'instance_count|desc' => __('Most instances'),
...
<div class="row mb-4 align-items-center">
  ...
  <div class="col-auto d-flex align-items-center gap-2 flex-wrap">
    ...
    <a href="<?php echo url_for(['module' => 'registry', 'action' => 'map']); ?>" class="btn btn-outline-secondary btn-sm">
      ...
      <?php if ('list' === $viewMode): ?>
        <div class="list-group mb-3">
          <?php foreach ($myInstitutions as $myInst): ?>
            <?php include_partial('registry/institutionListItem', ['item' => $myInst, 'myInstitutionIds' => $myInstitutionIds ?? []]); ?>
          <?php endforeach; ?>
        </div>
      <?php else: ?>
      ...
      <?php if ('list' === $viewMode): ?>
        <div class="list-group">
          <?php foreach ($result['items'] as $inst): ?>
            <?php include_partial('registry/institutionListItem', ['item' => $inst, 'myInstitutionIds' => $myInstitutionIds ?? []]); ?>
          <?php endforeach; ?>
        </div>
      <?php else: ?>
