...
<div class="col">
  <div class="card h-100">
    ...
    <?php if (!empty($showFavoriteToggle)): ?>
      <?php
      ...
      <div class="card-footer bg-transparent border-0 pt-0 text-end" style="position:relative; z-index:2;">
        <form method="post" action="<?php echo url_for(['module' => 'registry', 'action' => 'favoriteToggle']); ?>" class="d-inline">
          ...
          <button type="submit" class="btn btn-sm btn-link p-0 text-decoration-none" title="<?php echo $_isFav ? __('Remove from favorites') : __('Add to favorites'); ?>">
            <i class="fa<?php echo $_isFav ? 's' : 'r'; ?> fa-star <?php echo $_isFav ? 'text-warning' : 'text-muted'; ?>"></i>
          </button>
        </form>
      </div>
    <?php endif; ?>
