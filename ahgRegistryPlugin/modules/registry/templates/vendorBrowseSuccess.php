        ...
        <nav aria-label="<?php echo __('Page navigation'); ?>" class="mt-4">
          <ul class="pagination justify-content-center">
            <li class="page-item<?php echo $page <= 1 ? ' disabled' : ''; ?>">
              <a class="page-link" href="<?php echo url_for(['module' => 'registry', 'action' => 'vendorBrowse', 'page' => $page - 1, 'q' => $sf_request->getParameter('q', ''), 'type' => $sf_request->getParameter('type', ''), 'country' => $sf_request->getParameter('country', '')]); ?>">&laquo;</a>
            </li>
            <?php for ($i = max(1, $page - 2); $i <= min($totalPages, $page + 2); $i++): ?>
              <li class="page-item<?php echo $i === $page ? ' active' : ''; ?>">
                <a class="page-link" href="<?php echo url_for(['module' => 'registry', 'action' => 'vendorBrowse', 'page' => $i, 'q' => $sf_request->getParameter('q', ''), 'type' => $sf_request->getParameter('type', ''), 'country' => $sf_request->getParameter('country', '')]); ?>"><?php echo $i; ?></a>
              </li>
            <?php endfor; ?>
            <li class="page-item<?php echo $page >= $totalPages ? ' disabled' : ''; ?>">
              <a class="page-link" href="<?php echo url_for(['module' => 'registry', 'action' => 'vendorBrowse', 'page' => $page + 1, 'q' => $sf_request->getParameter('q', ''), 'type' => $sf_request->getParameter('type', ''), 'country' => $sf_request->getParameter('country', '')]); ?>">&raquo;</a>
            </li>
          </ul>
        </nav>
      <?php endif; ?>
