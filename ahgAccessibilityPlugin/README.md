...
WCAG accessibility tooling for archival descriptions — human-authored image alternative text (WCAG 1.1.1), authoring UI, coverage dashboard and a consumer API.
...
cd tmp-fetch && git sparse-checkout set ahgAccessibilityPlugin && cd ..
mv tmp-fetch/ahgAccessibilityPlugin ./ahgAccessibilityPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgAccessibilityPlugin/database/install.sql
