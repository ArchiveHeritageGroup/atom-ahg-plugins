 ...
 * PSIS Symfony port of Heratio ahg-exhibition walkthrough.blade.php.
 *
 * Faithful replica of the Heratio three.js scene: room box from $roomDims,
 * walls / floor / ceiling, doors + window openings, placed objects (framed 2D
 * images / 3D models / spotlights / display cases honouring tilt_x/z, spotlight,
 * display_case, on_floor), camera navigation (WASD + mouse, mobile orbit) and the
 * guided-tour camera sequencing.
 *
 * Reads its data + i18n + (optional) endpoint URLs from window.EXH_WT, injected
 * by walkthroughSuccess.php as a nonce'd JSON block.
 *
 * Parity-phase-2 stubs (no PSIS backend endpoint ported): neural TTS, AI "describe"
 * + docent Q&A, related-object recommend, multi-user presence avatars, visit-event
 * logging and persisted wall annotations. Their UI controls render; the network
 * calls are guarded to no-op (browser speechSynthesis still narrates locally;
 * graffiti stays client-side for the session). Look for TODO(parity-phase-2).
 */
(function () {
  var CFG = window.EXH_WT || {};
  var I18N = CFG.i18n || {};
  // i18n lookup: returns the localized string for the English key, or the key itself.
  function T(k) { return (I18N && Object.prototype.hasOwnProperty.call(I18N, k)) ? I18N[k] : k; }

  function boot() {
    (function () {
      var BUILDING = CFG.building;
      ...
      if (typeof THREE === 'undefined' || !THREE.PointerLockControls) {
        room.innerHTML = '<div class="p-4 text-light">' + T('3D engine failed to load.') + '</div>';
        return;
      }
      if (!ROOMS) {
        ROOMS = [{ id: 0, name: '', w: 18, d: 14, h: 4, x_offset: 0, z_offset: -7, is_current: true,
          floorplan: CFG.floorplan, stops: CFG.stops, walls: CFG.walls }];
      }
      ...
      function edgeWall(rm, eIdx, ax, az, bx, bz, mat, ccx, ccz) {
        ...
        openings.forEach(function (dd) {
          ...
          if (dd.type && dd.type !== 'open') {
            ...
            var dnm = makeTextSprite('' + T('Door') + '', 0.2); dnm.position.set(mx + ginx / ginl * 0.14, doorH * 0.5, mz + ginz / ginl * 0.14); addToRoom(rm, dnm);   // "Door" floated off the panel so it does not clip
      ...
      (BUILDING && BUILDING.stairs ? BUILDING.stairs : []).forEach(function (st) {
        ...
        if (st.kind === 'elbow') {
          ...
          var eUp = makeTextSprite('' + T('STAIRS ↑') + '', 0.42); eUp.position.set(0, y0 + 1.4, -0.8); grp.add(eUp);
          var eDn = makeTextSprite('' + T('STAIRS ↓') + '', 0.42); eDn.position.set(ex, y1 + 1.4, legLen); grp.add(eDn);
        } else {
          ...
          var sUp = makeTextSprite('' + T('STAIRS ↑') + '', 0.42); sUp.position.set(0, y0 + 1.4, -legLen / 2 - 0.8); grp.add(sUp);
          var sDn = makeTextSprite('' + T('STAIRS ↓') + '', 0.42); sDn.position.set(0, y1 + 1.4, legLen / 2 + 0.8); grp.add(sDn);
        }
      ...
      function loadRelated(s) {
        ...
        if (!RECOMMEND_URL) return;   // TODO(parity-phase-2): recommend endpoint not ported
      ...
      var TTS_URL = null;   // #1168 neural TTS via the gateway
      ...
      function speakBrowser(text, onDone) {
        try {
          ...
          setNarrLabel('<i class="fas fa-volume-high me-1"></i>' + T('Reading description... (Esc to stop)') + '');
      ...
      function speakText(text, onDone) {
        stopNarrate();
        try {
          if (!TTS_URL) { speakBrowser(text, onDone); return; }   // TODO(parity-phase-2): neural TTS not ported; browser voice only
          setNarrLabel('<i class="fas fa-volume-high me-1"></i>' + T('Narrating... (Esc to stop)') + '');
      ...
      function narrate(s, force) {
        ...
        // No metadata (or force=G): AI-describe backend not ported (parity-phase-2). Read what we have.
        if (!CFG.describeUrl) { speakText((s.title ? s.title + '. ' : '') + (desc || T('No description available for this object.') + '')); return; }
        setNarrLabel('<i class="fas fa-wand-magic-sparkles me-1"></i>' + T('Generating AI description...') + '');
        fetch(CFG.describeUrl + s.information_object_id + '/describe' + (force ? '?fresh=1' : ''), { headers: { 'Accept': 'application/json' } })
          .then(function (r) { return r.json(); })
          .then(function (d) {
            var ai = (d && d.description) ? d.description : null;
            var text = ai || '' + T('No description available for this object.') + '';
      ...
      var ASK_TMPL = null;
      var ASK_CHIPS = ['' + T('Tell me about this') + '', '' + T('Who made it?') + '', '' + T('When is it from?') + '', '' + T('Why does it matter?') + ''];
      function askDocent(s, q) {
        ...
        if (ansEl) { ansEl.style.display = 'block'; ansEl.innerHTML = '<i class="fas fa-circle-notch fa-spin me-1"></i>' + T('The docent is thinking…') + ''; }
        if (btn) btn.disabled = true;
        if (!ASK_TMPL) { if (btn) btn.disabled = false; if (ansEl) ansEl.textContent = T('Sorry, the record does not let me answer that right now.') + ''; return; }   // TODO(parity-phase-2)
        fetch(ASK_TMPL.replace('__IO__', s.information_object_id) + '?q=' + encodeURIComponent(q), { headers: { 'Accept': 'application/json' } })
          .then(function (r) { return r.json(); })
          .then(function (d) {
            if (btn) btn.disabled = false;
            var a = (d && d.answer) ? d.answer : '' + T('Sorry, the record does not let me answer that right now.') + '';
            if (ansEl && currentStop === s) { ansEl.style.display = 'block'; ansEl.textContent = '🤖 ' + a; }
            if (d && d.answer) speakText(a);
          })
          .catch(function () { if (btn) btn.disabled = false; if (ansEl) ansEl.textContent = '' + T('Sorry, I could not answer that right now.') + ''; });
      }
      ...
      function docentTryNavigate(query) {
        ...
        speakText('' + T('Right this way.') + ' ' + (t.title || ''));
        return true;
      }
      (function () {
      ...
      function openPanel(s) {
        ...
        document.getElementById('inlayDesc').textContent = s.description || '' + T('No description available.') + '';
        buildAskChips(s);
        ...
        try { if (VISIT_EVENT && s && s.information_object_id) { fetch(VISIT_EVENT, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': WT_CSRF, 'Accept': 'application/json' }, body: JSON.stringify({ token: MY_TOKEN, type: 'object', object_id: s.information_object_id, room_id: (s._room ? s._room.id : null) }) }); } } catch (e) {}
        loadRelated(s);
      ...
      function miniFloorLabel(f) { return f === 0 ? '' + T('Ground') + '' : (f < 0 ? ('' + T('Basement') + '' + (f < -1 ? ' ' + (-f) : '')) : ('' + T('Floor') + ' ' + f)); }
      ...
      function fmtLive(lv) {
        if (!lv || lv.status === 'none' || !lv.readings) return '<div class="text-white-50">' + T('No live readings yet.') + '</div>';
        var r = lv.readings, parts = [];
        if (r.lux) parts.push('' + T('Light') + ': ' + Math.round(r.lux.value) + ' lux' + (lv.lux_target ? ' / ' + Math.round(lv.lux_target) : ''));
        if (r.temp_c) parts.push('' + T('Temp') + ': ' + r.temp_c.value + ' C');
      ...
      var PRESENCE_BEAT = null;
      var PRESENCE_LEAVE = null;
      var VISIT_EVENT = null;   // #1173
      var WT_CSRF = CFG.csrf;
      var CAN_DOCENT = false;
      ...
      var MY_NAME = sessionStorage.getItem('wt_name') || (CAN_DOCENT ? '' + T('Docent') + '' : '' + T('Visitor') + '');
      ...
      function applyPeers(peers) {
        ...
        var hl = document.getElementById('wtHereNowLbl'); if (hl) hl.textContent = (total === 1 ? '' + T('you are alone here') + '' : '' + T('here now') + '');
        var list = document.getElementById('wtPeopleList');
        if (list) { var html = '<div>• ' + T('You') + '' + (CAN_DOCENT && myTourActive ? ' ★' : '') + '</div>'; peers.forEach(function (p) { html += '<div>• ' + (p.role === 'docent' ? '★ ' : '') + (p.name || 'Visitor').replace(/[<>&]/g, '') + '</div>'; }); list.innerHTML = html; }
      }
      function applyTour(tour) {
        ...
        if (theirTour) {
          ...
          if (banner) { banner.textContent = (following ? '' + T('Following') + ' ' : '' + T('Guided tour live:') + ' ') + (tour.docent_name || 'Docent') + (tour.msg ? (' — ' + tour.msg) : ''); banner.style.display = 'block'; }
        } else {
          if (followBtn) followBtn.style.display = 'none';
          following = false; followTarget = null;
          if (banner) banner.style.display = 'none';
        }
        if (CAN_DOCENT && myTourActive && banner) { banner.textContent = '' + T('You are leading a tour') + '' + (myDocentMsg ? (' — ' + myDocentMsg) : ''); banner.style.display = 'block'; }
      }
      function wtBeat() {
        if (!PRESENCE_BEAT) return;   // TODO(parity-phase-2): multi-user presence not ported
      ...
      (function () {
        ...
        var fb = document.getElementById('wtFollowBtn'); if (fb) fb.addEventListener('click', function () { following = !following; fb.classList.toggle('btn-warning', !following); fb.classList.toggle('btn-secondary', following); fb.innerHTML = following ? '<i class="fas fa-xmark me-1"></i>' + T('Stop following') + '' : '<i class="fas fa-shoe-prints me-1"></i>' + T('Follow the docent') + ''; });
        var tb = document.getElementById('wtTourBtn'), dm = document.getElementById('wtDocentMsg');
        if (tb) tb.addEventListener('click', function () { myTourActive = !myTourActive; if (!myTourActive) { myFocus = 0; myDocentMsg = ''; if (dm) dm.value = ''; } tb.classList.toggle('btn-success', !myTourActive); tb.classList.toggle('btn-danger', myTourActive); tb.innerHTML = myTourActive ? '<i class="fas fa-stop me-1"></i>' + T('Stop tour') + '' : '<i class="fas fa-chalkboard-user me-1"></i>' + T('Start guided tour') + ''; if (dm) dm.style.display = myTourActive ? 'block' : 'none'; wtBeat(); });
        if (dm) dm.addEventListener('input', function () { myDocentMsg = (this.value || '').slice(0, 200); });
      })();
      setInterval(wtBeat, 450); wtBeat();
      window.addEventListener('pagehide', function () { if (!PRESENCE_LEAVE) return; try { var fd = new FormData(); fd.append('_token', WT_CSRF); fd.append('token', MY_TOKEN); navigator.sendBeacon(PRESENCE_LEAVE, fd); } catch (e) {} });
      ...
      var ANNOT_URL = null;
      var ANNOT_DEL_URL = null;
      var WT_AUTH = (!!CFG.auth);   // logged-in users persist graffiti to the DB; guests keep it client-side for the session only
      var WT_GRAFFITI_SS_KEY = ('wt_graffiti_' + CFG.slug);
      ...
      function placeGraffiti(e) {
        ...
        if (gh.length) {
          var sp = gh[0].object;
          if (window.confirm('' + T('Delete this graffiti?') + '')) {
            if (sp.userData.graffitiId && ANNOT_DEL_URL) { fetch(ANNOT_DEL_URL.replace('__ID__', sp.userData.graffitiId), { method: 'POST', headers: { 'X-CSRF-TOKEN': WT_CSRF, 'Accept': 'application/json' } }).catch(function () {}); }
        ...
        var hit = hits[0], p = hit.point, txt = window.prompt('' + T('Graffiti text (leave blank to cancel):') + '', '');
        ...
        if (!WT_AUTH || !ANNOT_URL) {   // no annotation backend (parity-phase-2): client-side session only
      ...
      function tourGoto(i) {
        ...
        else if (CFG.describeUrl) { fetch(CFG.describeUrl + s.io_id + '/describe', { headers: { 'Accept': 'application/json' } }).then(function (r) { return r.json(); }).then(function (d) { speakText((st ? st.title + '. ' : '') + ((d && d.description) || ''), advance); }).catch(advance); }
        else { var d2 = st ? (st.description || '') : ''; if (d2 || (st && st.title)) { speakText((st && st.title ? st.title + '. ' : '') + d2, advance); } else { advance(); } }   // no AI-describe backend (parity-phase-2)
      }
      ...
      if (isTouch && !TOURS.length && STOPS.length) {
        TOURS = [{ name: '' + T('Full tour') + '', stops: STOPS.map(function (s) { return { io_id: s.information_object_id, title: s.title || '', narration: '', dwell: 6 }; }) }];
      }
      ...
      if (isTouch && TOURS.length && blocker) {
        ...
        blocker.innerHTML =
          '<div class="text-center text-white" style="max-width:340px;padding:0 18px;">' +
            ...
            '<button type="button" id="wtMobTourGo" class="btn btn-success btn-lg rounded-pill shadow w-100"><i class="fas fa-play me-2"></i>' + T('Start tour') + '</button>' +
            '<div><button type="button" id="wtMobExplore" class="btn btn-sm btn-link text-white-50 mt-2">' + T('Explore freely instead') + '</button></div>' +
            '<div class="small text-white-50 mt-2"><i class="fas fa-desktop me-1"></i>' + T('On a PC or laptop you can roam the gallery freely.') + '</div>' +
          '</div>';
      ...
      function populateVoices() {
        ...
        sel.innerHTML = '<option value="">' + T('Default') + '</option>';
        ...
        sel.onchange = function () { var v = vs.filter(function (x) { return x.name === sel.value; })[0] || null; WT_VOICE = v; if (v) { sessionStorage.setItem('wt_voice', v.name); speakText('' + T('Voice selected.') + ''); } else { sessionStorage.removeItem('wt_voice'); } };
      }
      ...
      function stealAlarm(s) {
        ...
        if (txt) txt.textContent = '' + T('ALARM! Put') + ' ' + (s && s.title ? s.title : '' + T('that') + '') + ' ' + T('back!') + '';
  ...
  // three.js + loaders load as classic scripts before us; if the DOM is ready, go.
