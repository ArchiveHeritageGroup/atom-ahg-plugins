...
/**
 * FusekiPlaceAdapter - service for AtoM Heratio
 *
 * Candidate adapter that searches the AHG Fuseki dataset (/openric-model) for
 * RiC place nodes (rico:Place) whose name contains the query string.
 *
 * RiC models place names two ways and this adapter accepts both, plus generic
 * label fallbacks, via a UNION:
 *   - reified name objects: ?place rico:hasOrHadPlaceName|rico:hasOrHadName
 *       ?nameNode . ?nameNode rico:textualValue ?name
 *   - the direct rico:name datatype property: ?place rico:name ?name
 *   - rdfs:label / skos:prefLabel as a last resort
 *
 * As of this writing the /openric-model dataset holds only the RiC ontology
 * definition (no place instances), so search() returns [] cleanly. Once
 * place instances are loaded the same query will surface them with no code
...
class FusekiPlaceAdapter implements CandidateAdapterInterface
{
    ...
    /**
     * Query the Fuseki dataset for RiC place candidates whose name contains
...
SELECT DISTINCT ?s ?name WHERE {
  ?s a rico:Place .
  {
    ?s rico:hasOrHadPlaceName ?nameNode .
