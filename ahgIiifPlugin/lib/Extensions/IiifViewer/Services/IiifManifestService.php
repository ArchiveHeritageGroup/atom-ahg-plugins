...
class IiifManifestService
{
    ...
    private function getPdfPathWithRedaction(object $do): array
    {
        ...
        // Load redaction service via provider system
        try {
            $provider = Providers::piiRedaction();

            if (!$provider || !$provider->hasRedaction($objectId)) {
                return [
                    'path' => $originalPath,
                    'identifier' => $originalIdentifier,
                    'is_redacted' => false
            ...
            $result = $provider->getRedactedPdf($objectId, $originalPath);
