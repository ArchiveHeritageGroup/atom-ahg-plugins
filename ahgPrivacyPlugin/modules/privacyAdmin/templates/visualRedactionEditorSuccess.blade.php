        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'piiScan']) }}">PII Scanner</a></li>
                    <li class="breadcrumb-item"><a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'piiReview']) }}">Review Queue</a></li>
                    <li class="breadcrumb-item active">Visual Editor</li>
                </ol>
            </nav>
        </div>
        <div class="btn-group">
            <a href="{{ url_for(['module' => 'informationobject', 'slug' => $object->slug]) }}" class="btn btn-outline-secondary" target="_blank">
                <i class="fas fa-external-link-alt me-1"></i>View Record
            </a>
            <a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'piiReview']) }}" class="btn btn-outline-secondary">
    ...
    <div class="card bg-dark text-white mb-3">
        <div class="card-body py-3">
            <div class="row align-items-center">
                <div class="col-md-6">
                    <h5 class="mb-1">{{ $object->getTitle(['cultureFallback' => true]) ?? 'Untitled' }}</h5>
                    <small class="text-muted">{{ $object->identifier ?? $object->slug ?? '' }}</small>
                </div>
                <div class="col-md-6">
                    <div class="d-flex justify-content-end gap-4">
                        @if($docInfo)
                            <div class="text-end">
                                ...
                                <span>{!! $docInfo['is_pdf'] ? '<i class="fas fa-file-pdf text-danger"></i> PDF' : '<i class="fas fa-image text-info"></i> Image' !!}</span>
                            </div>
                            @if($docInfo['is_pdf'])
                                <div class="text-end">
                            ...
                            <div class="text-end">
                                ...
                                <span class="badge bg-danger">{{ count($regions) }}</span>
                            </div>
                        @else
            ...
            <div class="col-lg-3">
                <div class="card bg-dark text-white h-100">
                                ...
                                <div class="region-item p-2 border-bottom border-secondary" data-id="{{ $region->id }}">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <small class="text-muted ms-2">Page {{ $region->page_number }}</small>
                                            @if($region->label)
                                                <br><small>{{ $region->label }}</small>
                                            @endif
                                        </div>
                    ...
                    <div class="card-footer">
                        <button class="btn btn-outline-danger w-100" id="btn-clear-all" {{ empty($regions) ? 'disabled' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const objectId = {{ (int)$objectId }};
    const isPdf = {{ $docInfo['is_pdf'] ? 'true' : 'false' }};
    const pageCount = {{ $docInfo['page_count'] ?? 1 }};
    const documentUrl = '{{ addslashes($docInfo['url'] ?? '') }}';
    const existingRegions = {{ json_encode($regions) }};
    ...
    function initImageViewer() {
        ...
        const iiifUrl = '{{ $cantaloupeUrl }}/' + encodeURIComponent(imageId) + '/info.json';
    ...
    async function saveRedactions() {
        ...
        try {
            ...
            const response = await fetch('{{ url_for(['module' => 'privacyAdmin', 'action' => 'saveVisualRedaction']) }}', {
                method: 'POST',
    ...
    async function applyRedactions() {
        ...
        try {
            const response = await fetch('{{ url_for(['module' => 'privacyAdmin', 'action' => 'applyVisualRedactions']) }}', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
