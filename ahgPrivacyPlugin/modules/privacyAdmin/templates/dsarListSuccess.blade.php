...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'dsarAdd']) }}" class="btn btn-primary">
            ...
            <form method="get" class="row g-3">
                <div class="col-md-3">
                    <select name="status" class="form-select">
                        ...
                        <option value="received" {{ $sf_request->getParameter('status') === 'received' ? 'selected' : '' }}>{{ __('Received') }}</option>
                        <option value="in_progress" {{ $sf_request->getParameter('status') === 'in_progress' ? 'selected' : '' }}>{{ __('In Progress') }}</option>
                        <option value="completed" {{ $sf_request->getParameter('status') === 'completed' ? 'selected' : '' }}>{{ __('Completed') }}</option>
                        <option value="rejected" {{ $sf_request->getParameter('status') === 'rejected' ? 'selected' : '' }}>{{ __('Rejected') }}</option>
                    </select>
                </div>
                <div class="col-md-2">
                    <button type="submit" class="btn btn-secondary">{{ __('Filter') }}</button>
                    <a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'dsarList', 'overdue' => 1]) }}" class="btn btn-outline-danger">
                        {{ __('Overdue') }}
                    </a>
                </div>
            </form>
...
@endphp
                    <tr class="{{ $isOverdue ? 'table-danger' : '' }}">
                        <td>
                            <a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'dsarView', 'id' => $dsar->id]) }}">
                        ...
                        <td>
                            <a href="{{ url_for(['module' => 'privacyAdmin', 'action' => 'dsarView', 'id' => $dsar->id]) }}" class="btn btn-sm btn-outline-primary">
