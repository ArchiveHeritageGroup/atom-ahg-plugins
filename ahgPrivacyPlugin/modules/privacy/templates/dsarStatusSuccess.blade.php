...
<div class="container py-4">
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ url_for(['module' => 'privacy', 'action' => 'index']) }}">{{ __('Privacy') }}</a></li>
...
@endphp
    ...
    <div class="card">
        <div class="card-body">
            <form method="get" action="{{ url_for(['module' => 'privacy', 'action' => 'dsarStatus']) }}">
                <div class="row">
                    <div class="col-md-5 mb-3">
                        <label class="form-label">{{ __('Reference Number') }}</label>
                        <input type="text" name="reference" class="form-control" placeholder="DSAR-202501-0001" value="{{ $sf_request->getParameter('reference') }}">
                    </div>
                    <div class="col-md-5 mb-3">
                        <label class="form-label">{{ __('Email Address') }}</label>
                        <input type="email" name="email" class="form-control" value="{{ $sf_request->getParameter('email') }}">
                    </div>
