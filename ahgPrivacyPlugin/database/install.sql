...
CREATE TABLE IF NOT EXISTS `privacy_breach` (
  ...
  `jurisdiction` VARCHAR(37) COMMENT 'popia, gdpr, pipeda, ccpa' NOT NULL,
  `breach_type` VARCHAR(52) COMMENT 'confidentiality, integrity, availability' NOT NULL,
  `severity` VARCHAR(39) COMMENT 'low, medium, high, critical' NOT NULL DEFAULT 'medium',
  `status` VARCHAR(64) COMMENT 'detected, investigating, contained, resolved, closed' NOT NULL DEFAULT 'detected',
  ...
  `risk_to_rights` VARCHAR(44) COMMENT 'unlikely, possible, likely, high' DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `privacy_breach_notification` (
  ...
  `notification_type` VARCHAR(58) COMMENT 'regulator, data_subject, internal, third_party' NOT NULL,
  `recipient` VARCHAR(255) NOT NULL,
  `method` VARCHAR(51) COMMENT 'email, letter, portal, phone, in_person' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `privacy_complaint` (
  ...
  `status` VARCHAR(64) COMMENT 'received, investigating, resolved, escalated, closed' DEFAULT 'received',
  `assigned_to` INT DEFAULT NULL,
  `resolution` TEXT,
...
CREATE TABLE IF NOT EXISTS `privacy_consent` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `consent_type` VARCHAR(94) COMMENT 'processing, marketing, profiling, third_party, cookies, research, special_category' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `privacy_consent_log` (
  ...
  `action` VARCHAR(48) COMMENT 'granted, withdrawn, expired, renewed' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `privacy_data_inventory` (
  ...
  `name` VARCHAR(255) NOT NULL,
  `description` TEXT,
  `data_type` VARCHAR(97) COMMENT 'personal, special_category, children, criminal, financial, health, biometric, genetic' NOT NULL,
  `storage_location` VARCHAR(255) DEFAULT NULL,
  `storage_format` VARCHAR(35) COMMENT 'electronic, paper, both' NOT NULL DEFAULT 'electronic',
...
CREATE TABLE IF NOT EXISTS `privacy_dsar` (
  ...
  `reference_number` VARCHAR(50) NOT NULL,
  `jurisdiction` VARCHAR(37) COMMENT 'popia, gdpr, pipeda, ccpa' NOT NULL,
  `request_type` VARCHAR(97) COMMENT 'access, rectification, erasure, portability, restriction, objection, withdraw_consent' NOT NULL,
  ...
  `status` VARCHAR(89) COMMENT 'received, verified, in_progress, pending_info, completed, rejected, withdrawn' NOT NULL DEFAULT 'received',
  `priority` VARCHAR(37) COMMENT 'low, normal, high, urgent' NOT NULL DEFAULT 'normal',
  `received_date` DATE NOT NULL,
  `due_date` DATE NOT NULL,
  `completed_date` DATE DEFAULT NULL,
  `assigned_to` INT DEFAULT NULL,
  `outcome` VARCHAR(63) COMMENT 'granted, partially_granted, refused, not_applicable' DEFAULT NULL,
  `refusal_reason` TEXT,
...
CREATE TABLE IF NOT EXISTS `privacy_officer` (
  ...
  `jurisdiction` VARCHAR(42) COMMENT 'popia, gdpr, pipeda, ccpa, all' NOT NULL DEFAULT 'all',
...
CREATE TABLE IF NOT EXISTS `privacy_paia_request` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `reference_number` VARCHAR(50) NOT NULL,
  `paia_section` VARCHAR(70) COMMENT 'section_18, section_22, section_23, section_50, section_77' NOT NULL,
  `requestor_name` VARCHAR(255) NOT NULL,
  `requestor_email` VARCHAR(255) DEFAULT NULL,
  `requestor_phone` VARCHAR(50) DEFAULT NULL,
  `requestor_id_number` VARCHAR(100) DEFAULT NULL,
  ...
  `access_form` VARCHAR(31) COMMENT 'inspect, copy, both' NOT NULL DEFAULT 'copy',
  `status` VARCHAR(92) COMMENT 'received, processing, granted, partially_granted, refused, transferred, appealed' NOT NULL DEFAULT 'received',
  `outcome_reason` TEXT,
...
CREATE TABLE IF NOT EXISTS `privacy_retention_schedule` (
  ...
  `disposal_action` VARCHAR(47) COMMENT 'destroy, archive, anonymize, review' NOT NULL DEFAULT 'destroy',
  `jurisdiction` VARCHAR(42) COMMENT 'popia, gdpr, pipeda, ccpa, all' NOT NULL DEFAULT 'all',
  `is_active` TINYINT(1) NOT NULL DEFAULT 1,
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `privacy_visual_redaction` (
    ...
    `region_type` VARCHAR(40) COMMENT 'rectangle, polygon, freehand' NOT NULL DEFAULT 'rectangle',
    ...
    `source` VARCHAR(48) COMMENT 'manual, auto_ner, auto_pii, imported' NOT NULL DEFAULT 'manual',
    ...
    `status` VARCHAR(48) COMMENT 'pending, approved, applied, rejected' NOT NULL DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS `privacy_redaction_cache` (
    ...
    `file_type` VARCHAR(22) COMMENT 'pdf, image' NOT NULL DEFAULT 'pdf',
...
INSERT IGNORE INTO `privacy_jurisdiction_registry`
