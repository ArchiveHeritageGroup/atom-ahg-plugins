...
User browse and management using Laravel Query Builder
...
cd tmp-fetch && git sparse-checkout set ahgUserManagePlugin && cd ..
mv tmp-fetch/ahgUserManagePlugin ./ahgUserManagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgUserManagePlugin/database/install.sql
