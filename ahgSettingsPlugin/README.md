...
Extended settings management for AtoM with sections, metadata templates, and configuration tools

| | |
|---|---|
| Machine name | `ahgSettingsPlugin` |
| Version | 1.0.1 |
| Category | admin |
...
cd tmp-fetch && git sparse-checkout set ahgSettingsPlugin && cd ..
mv tmp-fetch/ahgSettingsPlugin ./ahgSettingsPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgSettingsPlugin/database/install.sql
