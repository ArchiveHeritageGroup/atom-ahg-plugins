...
<div class="row">
    ...
    <div class="col-lg-4 col-md-6 mb-4">
        <div class="card h-100 shadow-sm {{ $card['color'] !== 'primary' ? 'border-' . $card['color'] : '' }}">
            <div class="card-body text-center py-4">
                <div class="mb-3">
                    <i class="{{ $card['icon_prefix'] }} {{ $card['icon'] }} fa-3x text-{{ $card['color'] }}"></i>
                </div>
                <h5 class="card-title">{{ __($card['label']) }}</h5>
                <p class="card-text text-muted small">{{ __($card['description']) }}</p>
            </div>
            <div class="card-footer bg-white border-0 text-center pb-4">
                <a href="{{ $card['url'] }}" class="btn btn-{{ $card['color'] }}">
                    <i class="fas {{ $card['btn_icon'] }}"></i> {{ __($card['btn_text']) }}
                </a>
            </div>
        </div>
    </div>
