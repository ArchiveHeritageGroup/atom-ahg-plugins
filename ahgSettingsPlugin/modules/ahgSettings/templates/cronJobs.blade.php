...
<div class="row">
  ...
  <div class="col-md-9">
    ...
    <div class="alert alert-info mb-4">
      <h5 class="alert-heading"><i class="bi bi-info-circle me-2"></i>{{ __('Cron Syntax Reference') }}</h5>
      ...
      <hr>
      <small>
        <strong>AtoM Root:</strong> <code>{{ $atomRoot }}</code> &nbsp;|&nbsp;
    ...
    @foreach ($categories as $catKey => $category)
      <div class="card mb-4">
        <div class="card-header">
          ...
          <span class="badge bg-secondary ms-2">{{ count($category['jobs']) }}</span>
        </div>
        <div class="card-body p-0">
          <div class="accordion accordion-flush" id="accordion-{{ $catKey }}">
            @foreach ($category['jobs'] as $idx => $job)
              @php $collapseId = "collapse-{$catKey}-{$idx}"; @endphp
              <div class="accordion-item cron-card">
                <h2 class="accordion-header" id="heading-{{ $catKey }}-{{ $idx }}">
                  <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $collapseId }}" aria-expanded="false" aria-controls="{{ $collapseId }}">
                    ...
                    <span class="badge bg-light text-dark me-2">
                      ...
                      @endphp
                      <span class="{{ $durationClass }}">{{ $job['duration'] }}</span>
                    </span>
                  </button>
                </h2>
                <div id="{{ $collapseId }}" class="accordion-collapse collapse" aria-labelledby="heading-{{ $catKey }}-{{ $idx }}" data-bs-parent="#accordion-{{ $catKey }}">
                  <div class="accordion-body">
                    <p class="mb-3">{{ $job['description'] }}</p>

                    <h6><i class="bi bi-terminal me-1"></i> Command</h6>
                    <div class="command-box mb-3">
                      <code>{{ htmlspecialchars($job['command']) }}</code>
                      <i class="bi bi-clipboard copy-btn float-end" onclick="copyToClipboard('{{ htmlspecialchars(addslashes($job['command'])) }}')" title="Copy"></i>
                    </div>

                    @if (!empty($job['options']))
                      <h6><i class="bi bi-sliders me-1"></i> Options</h6>
                      <div class="mb-3">
                        @foreach ($job['options'] as $opt => $desc)
                          <div class="mb-1">
                            <span class="badge bg-secondary option-badge">{{ htmlspecialchars($opt) }}</span>
                            <span class="small text-muted ms-2">{!! $desc !!}</span>
                          </div>
                        @endforeach
                      </div>
                    @endif
                    ...
                    <p class="text-muted small mb-3">{{ $job['schedule'] }}</p>

                    <h6><i class="bi bi-code-square me-1"></i> Example Cron Entry</h6>
                    <div class="command-box">
                      <code>{!! nl2br(htmlspecialchars(str_replace('{root}', $atomRoot, $job['example']))) !!}</code>
                      <i class="bi bi-clipboard copy-btn float-end" onclick="copyToClipboard('{{ htmlspecialchars(addslashes(str_replace('{root}', $atomRoot, $job['example']))) }}')" title="Copy"></i>
                    </div>
                  </div>
                </div>
              </div>
