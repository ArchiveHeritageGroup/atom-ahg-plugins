...
class SettingsTemplateAction extends AhgController
{
    ...
    {
        $infoObjectTemplate = SettingService::getByNameAndScope('informationobject', 'default_template');
        $actorTemplate = SettingService::getByNameAndScope('actor', 'default_template');
        $repositoryTemplate = SettingService::getByNameAndScope('repository', 'default_template');
    ...
    protected function updateDefaultTemplateSettings($form)
    {
        if (null !== $newValue = $form->getValue('informationobject')) {
            $setting = SettingService::findAndSave(
                'informationobject',
                $newValue,
