...
class SettingsTreeviewAction extends AhgEditController
{
    ...
    public function execute($request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                $this->redirect(['module' => 'ahgSettings', 'action' => 'treeview']);
            }
        }
    }
    ...
    protected function addField($name)
    {
        switch ($name) {
            case 'type':
                $this->typeSetting = SettingService::getByName('treeview_type');
                $default = 'sidebar';
                $options = [
            ...
            case 'showBrowseHierarchyPage':
                $this->showBrowseHierarchyPageSetting = SettingService::getByName('treeview_show_browse_hierarchy_page');
                $default = 'no';
                $options = [
                    'no' => $this->i18n->__('No'),
            ...
            case 'allowFullWidthTreeviewCollapse':
                $this->allowFullWidthTreeviewCollapse = SettingService::getByName('treeview_allow_full_width_collapse');
                $default = 'no';
                $options = [
                    'no' => $this->i18n->__('No'),
                    'yes' => $this->i18n->__('Yes'),
            ...
            case 'ioSort':
                $this->ioSortSetting = SettingService::getByName('sort_treeview_informationobject');
                $default = 'none';
                $options = [
            ...
            case 'showIdentifier':
                $this->showIdentifierSetting = SettingService::getByName('treeview_show_identifier');
                $default = 'no';
                $options = [
                    'no' => $this->i18n->__('No'),
            ...
            case 'showLevelOfDescription':
                $this->showLevelOfDescriptionSetting = SettingService::getByName('treeview_show_level_of_description');
                $default = 'yes';
                $options = [
                    'no' => $this->i18n->__('No'),
                    'yes' => $this->i18n->__('Yes'),
            ...
            case 'showDates':
                $this->showDatesSetting = SettingService::getByName('treeview_show_dates');
                $default = 'no';
                $options = [
                    'no' => $this->i18n->__('No'),
                    'yes' => $this->i18n->__('Yes'),
            ...
            case 'fullItemsPerPage':
                $this->fullItemsPerPageSetting = SettingService::getByName('treeview_full_items_per_page');
