...
class SettingsCronJobsAction extends AhgController
{
    ...
    protected function getAllCronJobs(): array
    {
        return [
            ...
            [
                'name' => 'Library Overdue Check',
                'command' => 'php symfony library:overdue-check',
                'description' => 'Scans active checkouts past their due date. Reports overdue items and optionally flags patrons for notification.',
                'options' => [
                    '--days=N' => 'Minimum days overdue to report (default: 1)',
                    '--notify' => 'Flag patrons for overdue notification',
                ],
                'schedule' => 'Daily',
                'example' => '0 6 * * * cd {root} && php symfony library:overdue-check --notify >> /var/log/atom/library-overdue.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
                'name' => 'Library Process Fines',
                'command' => 'php symfony library:process-fines',
                'description' => 'Calculates and creates daily overdue fines based on loan rules. Respects grace periods and fine caps per material type and patron type.',
                'options' => [
                    '--dry-run' => 'Preview fines without creating them',
                ],
                'schedule' => 'Daily',
                'example' => '0 1 * * * cd {root} && php symfony library:process-fines >> /var/log/atom/library-fines.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
                'name' => 'Library Hold Expiry',
                'command' => 'php symfony library:hold-expiry',
                'description' => 'Expires unfulfilled holds past their expiry date. Promotes the next patron in the hold queue automatically.',
                'options' => [
                    '--dry-run' => 'Preview without expiring',
                ],
                'schedule' => 'Daily',
                'example' => '0 2 * * * cd {root} && php symfony library:hold-expiry >> /var/log/atom/library-holds.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
                'name' => 'Library Patron Expiry',
                'command' => 'php symfony library:patron-expiry',
                'description' => 'Flags patrons with expired memberships by setting borrowing_status to expired. Prevents further checkouts until renewed.',
                'options' => [
                    '--grace-days=N' => 'Grace period after expiry before flagging (default: 0)',
                    '--dry-run' => 'Preview without updating',
                ],
                'schedule' => 'Daily',
                'example' => '0 3 * * * cd {root} && php symfony library:patron-expiry --grace-days=7 >> /var/log/atom/library-patron.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
                'name' => 'Library Serial Expected Issues',
                'command' => 'php symfony library:serial-expected',
                'description' => 'Generates expected serial issue records for active subscriptions based on their frequency. Also reports missing/late issues.',
                'options' => [
                    '--months=N' => 'Generate expected issues for N months ahead (default: 3)',
                    '--dry-run' => 'Preview without creating',
                ],
                'schedule' => 'Weekly',
                'example' => '0 5 * * 1 cd {root} && php symfony library:serial-expected --months=3 >> /var/log/atom/library-serials.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
                'name' => 'Library ILL Overdue',
                'command' => 'php symfony library:ill-overdue',
                'description' => 'Reports overdue interlibrary loan items — both borrowed items we need to return and lent items partner libraries need to return.',
                'options' => [
                    '--days=N' => 'Minimum days overdue (default: 1)',
                ],
                'schedule' => 'Daily',
                'example' => '0 6 * * * cd {root} && php symfony library:ill-overdue >> /var/log/atom/library-ill.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
            ...
            [
                ...
                'options' => [
                    ...
                    '--repository-id=ID' => 'Repository ID (for scope=repository)',
                    '--mode=MODE' => 'Export mode: read_only, editable, archive',
                    ...
                    '--export-id=ID' => 'Process existing export job by ID',
                ],
                'schedule' => 'On demand or quarterly for distribution',
                'example' => 'cd {root} && php symfony portable:export --scope=all --mode=archive --zip --output=/tmp/archive-export.zip >> /var/log/atom/portable-export.log 2>&1',
                'duration' => 'Long (depends on collection size and images)',
                'category' => 'ahg',
            ],
            [
                'name' => 'Portable Verify',
                'command' => 'php symfony portable:verify',
                'description' => 'Verifies the integrity of an archive export package by checking SHA-256 checksums in manifest.json against all exported files. Exit code 0 = all OK, 1 = errors found.',
                'options' => [
                    '--path=PATH' => 'Path to export ZIP or extracted directory',
                ],
            ...
            [
                ...
                'command' => 'php symfony portable:cleanup',
                'description' => 'Removes expired export packages based on retention settings. Run periodically to free disk space.',
                'options' => [],
                'schedule' => 'Daily at 3am',
                'example' => '0 3 * * * cd {root} && php symfony portable:cleanup >> /var/log/atom/portable-cleanup.log 2>&1',
                'duration' => 'Short',
                'category' => 'ahg',
            ],
            [
