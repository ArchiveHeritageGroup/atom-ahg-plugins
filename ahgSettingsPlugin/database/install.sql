...
INSERT INTO numbering_scheme (name, sector, pattern, description, is_default, is_active) VALUES
...
-- ---------------------------------------------------------------------------
-- level_of_description_sector - which levels of description each GLAM sector
-- offers, and in what order.
--
-- Added 2026-08-17. It was queried by ahgCorePlugin, ahgDisplayPlugin,
-- ahgLibraryPlugin, ahgDAMPlugin and ahgThemeB5Plugin, and written by this
-- plugin's own Settings > Levels handler, but no install.sql anywhere created
-- it. On a clean install every page logged
--   "Levels query error: Base table or view not found: 1146
--    Table 'atom.level_of_description_sector' doesn't exist"
-- and the sector level lists silently fell back to empty.
--
-- Owned here because this is the plugin that writes it; everything else reads.
--
-- No foreign key to term: house rule, and the readers treat term_id as a
-- logical reference. The unique key is what stops a level being listed twice
-- for one sector, since the handler deletes by sector then re-inserts.
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS `level_of_description_sector` (
    `id`            BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `term_id`       INT NOT NULL COMMENT 'term.id of the level of description (logical FK)',
    `sector`        VARCHAR(50) NOT NULL COMMENT 'archive, library, museum, gallery, dam',
    `display_order` INT NOT NULL DEFAULT 0,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uniq_term_sector` (`term_id`, `sector`),
    KEY `idx_sector_order` (`sector`, `display_order`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
