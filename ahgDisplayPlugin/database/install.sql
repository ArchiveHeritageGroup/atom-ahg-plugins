...
CREATE TABLE IF NOT EXISTS `display_field` (
  ...
  `field_group` VARCHAR(68) COMMENT 'identity, description, context, access, technical, admin' DEFAULT 'description',
  `data_type` VARCHAR(101) COMMENT 'text, textarea, date, daterange, number, select, multiselect, relation, file, actor, term' DEFAULT 'text',
...
CREATE TABLE IF NOT EXISTS `display_profile` (
  ...
  `layout_mode` VARCHAR(74) COMMENT 'detail, hierarchy, grid, gallery, list, card, masonry, catalog' DEFAULT 'detail',
  `thumbnail_size` VARCHAR(50) COMMENT 'none, small, medium, large, hero, full' DEFAULT 'medium',
  `thumbnail_position` VARCHAR(48) COMMENT 'left, right, top, background, inline' DEFAULT 'left',
...
INSERT IGNORE INTO `display_profile_i18n` VALUES (4,'en','Collections Procedures full',NULL);
INSERT IGNORE INTO `display_profile_i18n` VALUES (5,'en','Collections Procedures card',NULL);
INSERT IGNORE INTO `display_profile_i18n` VALUES (6,'en','Collections Procedures catalog',NULL);
