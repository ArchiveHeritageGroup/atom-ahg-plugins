-- =============================================================================
-- arDisplayPlugin Safe Upgrade Script
-- =============================================================================
...
REPLACE INTO display_level_i18n (id, culture, name, description) 
SELECT id, 'en', CONCAT(UPPER(SUBSTRING(REPLACE(code, '_', ' '), 1, 1)), LOWER(SUBSTRING(REPLACE(code, '_', ' '), 2))), NULL
FROM display_level;
...
('isad_full', 'archive', 'detail', 'small', 'left', '["identifier","title","dates","level","extent","creator"]', '["scope_content","arrangement","appraisal"]', '["provenance","custodial_history","acquisition"]', '["access_conditions","reproduction","language","finding_aids"]', '["view","download","request","cite","print"]', 1, 1),
('isad_hierarchy', 'archive', 'hierarchy', 'none', 'left', '["identifier","title","dates","level"]', '["scope_content"]', '[]', '[]', '["view","expand"]', 0, 2),
('isad_list', 'archive', 'list', 'small', 'left', '["identifier","title","dates","level"]', '[]', '[]', '[]', '["view"]', 0, 3),
('spectrum_full', 'museum', 'detail', 'large', 'left', '["object_number","object_name","classification","materials","dimensions","technique"]', '["description","inscription","condition","completeness"]', '["production","provenance","acquisition","associations"]', '["access_conditions","reproduction"]', '["view","condition_report","movement","loan_request","print"]', 1, 10),
('spectrum_card', 'museum', 'card', 'medium', 'top', '["object_number","object_name","materials"]', '["description"]', '[]', '[]', '["view","add_to_exhibition"]', 0, 11),
('spectrum_catalog', 'museum', 'catalog', 'medium', 'left', '["object_number","object_name","materials","dimensions","date"]', '["description"]', '["provenance"]', '[]', '["view","print"]', 0, 12),
('gallery_full', 'gallery', 'gallery', 'hero', 'top', '["artist","title","date","medium","dimensions","edition_info"]', '["description","artist_statement"]', '["provenance","exhibition_history","bibliography"]', '["rights","reproduction"]', '["view","zoom","add_to_exhibition","license","print"]', 1, 20),
('gallery_wall', 'gallery', 'gallery', 'full', 'background', '["artist","title","date","medium"]', '[]', '[]', '[]', '["view","zoom","info"]', 0, 21),
('gallery_catalog', 'gallery', 'catalog', 'medium', 'left', '["artist","title","date","medium","dimensions"]', '["description"]', '["provenance","literature"]', '[]', '["view","print"]', 0, 22),
('book_full', 'library', 'detail', 'small', 'left', '["call_number","title","author","publisher","date","isbn","edition"]', '["abstract","subjects","table_of_contents"]', '["provenance","notes"]', '["access_conditions","location"]', '["view","request","cite","print"]', 1, 30),
('book_list', 'library', 'list', 'none', 'left', '["call_number","author","title","date"]', '[]', '[]', '[]', '["view","request"]', 0, 31),
('book_card', 'library', 'card', 'small', 'left', '["title","author","date"]', '["abstract"]', '[]', '[]', '["view"]', 0, 32),
('photo_full', 'dam', 'detail', 'large', 'top', '["asset_id","title","photographer","date_taken","location"]', '["caption","keywords"]', '["provenance","collection"]', '["rights","usage_terms","restrictions"]', '["view","zoom","download","add_to_lightbox","license","derivatives"]', 1, 40),
('photo_grid', 'dam', 'grid', 'medium', 'top', '["title","date"]', '[]', '[]', '[]', '["view","select","add_to_lightbox"]', 0, 41),
('photo_lightbox', 'dam', 'masonry', 'large', 'top', '["title"]', '[]', '[]', '[]', '["view","zoom","select","compare","download"]', 0, 42),
('search_result', 'universal', 'card', 'small', 'left', '["identifier","title","creator","date","level"]', '["description"]', '[]', '[]', '["view"]', 0, 100),
('print_record', 'universal', 'detail', 'medium', 'right', '["identifier","title","creator","date","level","extent"]', '["scope_content","description"]', '["provenance","acquisition"]', '["access_conditions","rights"]', '[]', 0, 101);

REPLACE INTO display_profile_i18n (id, culture, name)
...
REPLACE INTO display_field_i18n (id, culture, name)
SELECT id, 'en', CONCAT(UPPER(SUBSTRING(REPLACE(code, '_', ' '), 1, 1)), SUBSTRING(REPLACE(code, '_', ' '), 2))
FROM display_field;
...
REPLACE INTO display_collection_type_i18n (id, culture, name, description)
SELECT id, 'en', name, description FROM (
    SELECT (SELECT id FROM display_collection_type WHERE code = 'archive') as id, 'Archive' as name, 'Archival fonds and collections following ISAD(G)' as description
    UNION ALL SELECT (SELECT id FROM display_collection_type WHERE code = 'museum'), 'Museum', 'Museum objects following Spectrum'
    UNION ALL SELECT (SELECT id FROM display_collection_type WHERE code = 'gallery'), 'Gallery', 'Artworks and artist archives'
    UNION ALL SELECT (SELECT id FROM display_collection_type WHERE code = 'library'), 'Book Collection', 'Books, periodicals, and printed materials'
    UNION ALL SELECT (SELECT id FROM display_collection_type WHERE code = 'photo_archive'), 'Photo Archive', 'Photographs and digital assets'
    UNION ALL SELECT (SELECT id FROM display_collection_type WHERE code = 'audiovisual'), 'Audiovisual', 'Film, video, and audio recordings'
    UNION ALL SELECT (SELECT id FROM display_collection_type WHERE code = 'mixed'), 'Mixed Collection', 'Collections with mixed material types'
) AS t WHERE t.id IS NOT NULL;

SELECT 'arDisplayPlugin upgrade complete!' as status;
SELECT COUNT(*) as display_levels FROM display_level;
SELECT COUNT(*) as display_profiles FROM display_profile;
SELECT COUNT(*) as display_fields FROM display_field;
SELECT COUNT(*) as collection_types FROM display_collection_type;
