      ...
      <div class="accordion-body bg-secondary px-5 pt-4 pb-3">
        <div id="atom-slider-images" class="mb-0">
          <?php foreach ($thumbnails as $item) { ?>
            <?php
              $parentObj = isset($item->parent) ? $item->parent->object : null;
              $itemUrl = $parentObj ? url_for([$parentObj, 'module' => 'informationobject']) : '#';
              $itemTitle = $parentObj ? strip_markdown($parentObj) : __('Untitled');
              $altText = strip_markdown($item->getDigitalObjectAltText() ?: ($parentObj ?: __('Image')));
            ?>
            <a title="<?php echo $itemTitle; ?>" href="<?php echo $itemUrl; ?>">
              <?php echo image_tag($item->getFullPath(), ['class' => 'img-thumbnail mx-2', 'longdesc' => $itemUrl, 'alt' => $altText]); ?>
            </a>
          <?php } ?>
        </div>
        ...
        <div id="atom-slider-title">
          <?php foreach ($thumbnails as $item) { ?>
            <?php
              ...
              $itemTitle = $parentObj ? strip_markdown($parentObj) : __('Untitled');
            ?>
            <a href="<?php echo $itemUrl; ?>" class="text-white text-center mt-2 mb-1">
              <?php echo $itemTitle; ?>
            </a>
          <?php } ?>
        </div>
