...
Geospatial search and GeoJSON export for heritage records with coordinates
...
cd tmp-fetch && git sparse-checkout set ahgGISPlugin && cd ..
mv tmp-fetch/ahgGISPlugin ./ahgGISPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgGISPlugin/database/install.sql
