...
class arSpectrumExportService
{
    protected $eventService;

    public function __construct()
    {
        $this->eventService = new arSpectrumEventService();
    }
    ...
    public function exportObjectHistoryCSV($objectId, $procedureId = null): string
    {
        ...
        foreach ($events as $event) {
            $procedureName = arSpectrumEventService::$procedures[$event['procedure_id']]['name'] ?? $event['procedure_id'];
            $eventTypeName = arSpectrumEventService::$eventTypeLabels[$event['event_type']] ?? $event['event_type'];
            $statusFromName = arSpectrumEventService::$statusLabels[$event['status_from']]['label'] ?? $event['status_from'];
            $statusToName = arSpectrumEventService::$statusLabels[$event['status_to']]['label'] ?? $event['status_to'];
    ...
    public function exportRepositoryHistoryCSV($repositoryId = null, $procedureId = null, $dateFrom = null, $dateTo = null): string
    {
        ...
        // Data rows
        foreach ($events as $event) {
            $procedure = arSpectrumEventService::$procedures[$event->procedure_id] ?? [];
    ...
    public function exportOverdueCSV($repositoryId = null): string
    {
        ...
        foreach ($overdue as $item) {
            $procedureName = arSpectrumEventService::$procedures[$item['procedure_id']]['name'] ?? $item['procedure_id'];
                ...
                <div class="status-row">
                    ...
                    <div class="status-cell">
                        <span class="status-badge status-<?php echo $status['current_status']; ?>">
                            <?php echo arSpectrumEventService::$statusLabels[$status['current_status']]['label'] ?? $status['current_status']; ?>
                        </span>
                    </div>
            ...
            <?php foreach ($events as $event): ?>
                <div class="event-item">
                    <div class="event-header">
                        <?php echo arSpectrumEventService::$eventTypeLabels[$event['event_type']] ?? $event['event_type']; ?>
                        - <?php echo arSpectrumEventService::$procedures[$event['procedure_id']]['name'] ?? $event['procedure_id']; ?>
                    </div>
                    <div class="event-meta">
                        ...
                        <?php if (!empty($event['status_from']) && !empty($event['status_to'])): ?>
                            | Status: <?php echo arSpectrumEventService::$statusLabels[$event['status_from']]['label'] ?? $event['status_from']; ?>
                            → <?php echo arSpectrumEventService::$statusLabels[$event['status_to']]['label'] ?? $event['status_to']; ?>
                        <?php endif; ?>
                    </div>
    ...
    <div class="section">
        ...
        <?php foreach (array_slice($overdue, 0, 20) as $item): ?>
            <div class="overdue-item">
                ...
                - <?php echo arSpectrumEventService::$procedures[$item['procedure_id']]['name'] ?? $item['procedure_id']; ?>
                <br>
