...
class arSpectrumTemplateService
{
    ...
    protected function getDefaultProcedureConfig()
    {
        ...
        foreach (arSpectrumEventService::$procedures as $procId => $procedure) {
            $config[$procId] = [
                'enabled' => true,
    ...
    public function validateTemplate($template)
    {
        ...
        if (!isset($template['procedures']) || !is_array($template['procedures'])) {
            ...
            foreach ($template['procedures'] as $procId => $config) {
                if (!isset(arSpectrumEventService::$procedures[$procId])) {
    ...
    public function getProcedureConfig($repositoryId, $procedureId)
    {
        ...
        $baseConfig = arSpectrumEventService::$procedures[$procedureId] ?? null;
        if (!$baseConfig) {
            return null;
        }
