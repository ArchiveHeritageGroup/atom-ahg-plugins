    ...
    public function up(): void
    {
        ...
        if (!$schema->hasTable('spectrum_loan')) {
            ...
            $schema->create('spectrum_loan', function (Blueprint $table) {
                ...
                $table->string('loan_type', 41)->comment('incoming, outgoing');
                ...
                $table->string('status', 77)->default('requested')->comment('requested, approved, active, overdue, returned, cancelled');
        ...
        if (!$schema->hasTable('spectrum_label')) {
            ...
            $schema->create('spectrum_label', function (Blueprint $table) {
                $table->id();
                $table->integer('object_id')->unsigned();
                $table->string('label_type', 70)->comment('object, storage, exhibition, loan, qr, barcode');
