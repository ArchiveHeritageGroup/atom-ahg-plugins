...
<section id="action-icons">
  <ul class="list-unstyled">
    <li>
      <a class="atom-icon-link" href="{{ url_for(['module' => 'informationobject', 'action' => 'index', 'slug' => $resource->slug]) }}">
        <i class="fas fa-fw fa-arrow-left me-1" aria-hidden="true"></i>{{ __('Back to record') }}
      </a>
    </li>
  </ul>
</section>
...
// Generated locally as data URIs rather than fetched from barcodeapi.org and
// api.qrserver.com (#260). Every dropdown option is pre-rendered, keyed by the
// option's value, so switching source is a local swap.
$barcodeUris = [];
                ...
                <div class="label-preview" id="labelContent" style="max-width: 300px;">
                    ...
                    <div id="barcodeSection" class="mb-2">
                        <img id="barcodeImg" class="barcode-img"
                             src="{{ $defaultBarcodeUri }}"
                             data-barcode-uris="{{ json_encode($barcodeUris) }}"
                             alt="{{ __('Barcode') }}">
...
function updateBarcodeSource() {
    ...
    // Rendered server-side, so this is a local swap - nothing leaves the instance (#260).
    var uris = {};
    try { uris = JSON.parse(img.getAttribute('data-barcode-uris') || '{}'); } catch (e) { uris = {}; }
    if (uris[value]) { img.src = uris[value]; }
