...
<div class="spectrum-condition-photos">
    ...
    <div class="page-header d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">
                ...
                ({{ date('d M Y', strtotime($conditionCheck['check_date'])) }})
            </p>
        </div>
        <div class="btn-group">
            <a href="{{ url_for(['module' => 'spectrum', 'action' => 'index', 'slug' => $resource->slug]) }}" class="btn btn-secondary">
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ url_for(['module' => 'spectrum', 'action' => 'conditionPhotos', 'slug' => $resource->slug]) . '?photo_action=upload&condition_id=' . $conditionCheckId }}"
                  method="post"
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="photographer" id="photographer" class="form-control"
                                   value="{{ $sf_user->getAttribute('username') }}">
                        </div>
                    ...
                    <div class="btn-group btn-group-sm" id="photo-type-filter">
                        ...
                        @foreach($photosByType as $type => $typePhotos)
                            <button type="button" class="btn btn-outline-primary" data-type="{{ $type }}">
                                {{ __($photoTypes[$type]) }} ({{ count($typePhotos) }})
                            </button>
                        @endforeach
                    </div>
                </div>
                            ...
                            <div class="photo-card">
                                <div class="photo-image">
                                    ...
                                    <span class="badge badge-{{ $photo['photo_type'] }} photo-type-badge">
                                        {{ __($photoTypes[$photo['photo_type']]) }}
                                    </span>
                                </div>
                                    ...
                                    <div class="photo-meta">
                                        <small class="text-muted">
                                            ...
                                            @if($photo['photographer'])
                                                | <i class="fas fa-user"></i> {!! htmlspecialchars($photo['photographer']) !!}
                                            @endif
                                        </small>
                                    </div>
                                </div>
                    ...
                    <div class="col-md-6 mb-4">
                        <div class="comparison-card">
                            @if($comparison['comparison_title'])
                                <h6>{!! htmlspecialchars($comparison['comparison_title']) !!}</h6>
                            @endif
                            ...
                            @if($comparison['comparison_notes'])
                                <p class="mt-2 small text-muted">{!! htmlspecialchars($comparison['comparison_notes']) !!}</p>
                            @endif
                        </div>
                    </div>
                @endforeach
...
function initAnnotator(photo) {
    ...
    try {
        currentAnnotator = new ConditionAnnotator('annotator-container', {
            ...
            readonly: false,
            showToolbar: true,
            saveUrl: '{{ url_for(["module" => "spectrum", "action" => "saveAnnotations"]) }}',
            getUrl: '{{ url_for(["module" => "spectrum", "action" => "getAnnotations"]) }}'
        });
...
document.getElementById('export-annotations-btn').addEventListener('click', function() {
    if (currentPhotoId) {
        window.open('{{ url_for(["module" => "spectrum", "action" => "exportAnnotatedPhoto"]) }}?photo_id=' + currentPhotoId + '&format=png', '_blank');
    }
});
