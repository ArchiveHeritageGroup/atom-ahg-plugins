...
High-performance archival institution browse using Laravel Query Builder and direct ES queries
...
cd tmp-fetch && git sparse-checkout set ahgRepositoryManagePlugin && cd ..
mv tmp-fetch/ahgRepositoryManagePlugin ./ahgRepositoryManagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgRepositoryManagePlugin/database/install.sql
