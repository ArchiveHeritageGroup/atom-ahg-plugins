...
Static page management — list, edit, and delete AtoM static pages via Laravel Query Builder
...
cd tmp-fetch && git sparse-checkout set ahgStaticPagePlugin && cd ..
mv tmp-fetch/ahgStaticPagePlugin ./ahgStaticPagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgStaticPagePlugin/database/install.sql
