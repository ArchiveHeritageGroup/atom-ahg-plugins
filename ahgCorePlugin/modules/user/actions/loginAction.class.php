    ...
    public function execute($request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                $lockedOut = false;
                try {
                    if (\AtomFramework\Core\Security\LoginSecurityService::isLockedOut($email, $ip)) {
                        $remaining = \AtomFramework\Core\Security\LoginSecurityService::lockoutRemaining($email);
                ...
                if (!$lockedOut) {
                    if ($this->context->user->authenticate($email, $this->form->getValue('password'))) {
