...
| | |
|---|---|
| Machine name | `ahgTiffPdfMergePlugin` |
| Version | 1.0.0 |
...
cd tmp-fetch && git sparse-checkout set ahgTiffPdfMergePlugin && cd ..
mv tmp-fetch/ahgTiffPdfMergePlugin ./ahgTiffPdfMergePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgTiffPdfMergePlugin/database/install.sql
