...
Configurable metadata entry forms per repository — drag-drop form builder, template library, field assignments, draft saving, and import/export

| | |
|---|---|
| Machine name | `ahgFormsPlugin` |
| Version | 1.0.2 |
| Category | data-entry |
...
cd tmp-fetch && git sparse-checkout set ahgFormsPlugin && cd ..
mv tmp-fetch/ahgFormsPlugin ./ahgFormsPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgFormsPlugin/database/install.sql
