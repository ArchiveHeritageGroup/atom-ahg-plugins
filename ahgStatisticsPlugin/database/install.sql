...
CREATE TABLE IF NOT EXISTS `ahg_usage_event` (
    ...
    `event_type` VARCHAR(46) COMMENT 'view, download, search, login, api' NOT NULL DEFAULT 'view',
...
CREATE TABLE IF NOT EXISTS `ahg_bot_list` (
    ...
    `category` VARCHAR(67) COMMENT 'search_engine, social, monitoring, crawler, spam, other' NOT NULL DEFAULT 'crawler',
...
CREATE TABLE IF NOT EXISTS `ahg_statistics_config` (
    ...
    `setting_value` TEXT,
    `setting_type` VARCHAR(42) COMMENT 'string, integer, boolean, json' NOT NULL DEFAULT 'string',
    `description` TEXT,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
