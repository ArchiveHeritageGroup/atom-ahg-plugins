# IPSAS Heritage Accounting

International Public Sector Accounting Standards (IPSAS 45) heritage asset accounting — asset register, valuations, impairments, insurance, depreciation, disposals, and financial year reporting
...
cd tmp-fetch && git sparse-checkout set ahgIPSASPlugin && cd ..
mv tmp-fetch/ahgIPSASPlugin ./ahgIPSASPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgIPSASPlugin/database/install.sql
