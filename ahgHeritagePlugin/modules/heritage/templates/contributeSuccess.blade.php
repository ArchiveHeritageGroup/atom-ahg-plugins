...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
        ...
        <p class="card-text small text-muted">
            {{ substr(strip_tags($item->scope_and_content), 0, 200) }}...
        </p>
        @endif
        <a href="{{ url_for(['module' => 'informationobject', 'slug' => $slug]) }}"
    ...
    <ul class="list-group list-group-flush">
        ...
        <li class="list-group-item">
            <div class="d-flex align-items-center mb-1">
                <i class="fas {{ $contrib['type']['icon'] }} text-{{ $contrib['type']['color'] }} me-2"></i>
                <small class="fw-bold">{{ $contrib['type']['name'] }}</small>
            </div>
            <small class="text-muted">
                by {{ $contrib['contributor']['display_name'] }}
            </small>
        </li>
        @endforeach
    </ul>
    ...
    <div class="card-body">
        ...
        <div class="text-center py-5">
            ...
            <div class="d-flex justify-content-center gap-2">
                <a href="{{ url_for(['module' => 'heritage', 'action' => 'contributorLogin']) }}" class="btn btn-primary">
        ...
        <div class="mb-4">
            <p class="text-muted mb-0">
                Contributing as <strong>{{ $contributorName }}</strong>
                <a href="{{ url_for(['module' => 'heritage', 'action' => 'contributorLogout']) }}" class="small ms-2">
                    (Sign out)
                </a>
            </p>
        </div>
        ...
        <ul class="nav nav-pills mb-4" id="contributionTabs" role="tablist">
            ...
            <li class="nav-item" role="presentation">
                <button class="nav-link {{ $selectedType === $opp['code'] ? 'active' : '' }} {{ !$opp['available'] ? 'disabled' : '' }}"
                        id="tab-{{ $opp['code'] }}"
                        data-bs-toggle="pill"
                        data-bs-target="#form-{{ $opp['code'] }}"
                        type="button"
                        role="tab"
                        {!! !$opp['available'] ? 'disabled title="' . e($opp['reason']) . '"' : '' !!}>
                    <i class="fas {{ $opp['icon'] }} me-1"></i>
                    {{ $opp['name'] }}
                    @if ($opp['existing_count'] > 0)
                    <span class="badge bg-secondary ms-1">{{ $opp['existing_count'] }}</span>
                    @endif
                </button>
            </li>
            @endforeach
        ...
        <div class="tab-content" id="contributionTabsContent">
            @foreach ($opportunities as $opp)
            <div class="tab-pane fade {{ $selectedType === $opp['code'] ? 'show active' : '' }}"
                 id="form-{{ $opp['code'] }}"
                 role="tabpanel">

                @if (!$opp['available'])
                <div class="alert alert-warning">
                    <i class="fas fa-info-circle me-2"></i>{{ $opp['reason'] }}
                </div>
                @else

                <form id="contribution-form-{{ $opp['code'] }}" class="contribution-form">
                    ...
                    <div class="mb-3">
                        ...
                        <p class="small text-success">
                            <i class="fas fa-gift me-1"></i>Earn {{ $opp['points_value'] }} points for this contribution
                        </p>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="border rounded p-3 bg-light small">
                            {!! nl2br(e(substr($item->scope_and_content, 0, 500))) !!}
                        </div>
                    </div>
                    @endif
                    <div class="mb-3">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <a href="{{ url_for(['module' => 'informationobject', 'slug' => $slug]) }}" class="btn btn-outline-secondary">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-body text-center py-5">
                ...
                <div class="d-flex justify-content-center gap-2">
                    <a href="{{ url_for(['module' => 'heritage', 'action' => 'myContributions']) }}" class="btn btn-primary">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.contribution-form').forEach(form => {
        form.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('{{ url_for(['module' => 'heritage', 'action' => 'apiSubmitContribution']) }}', {
                    method: 'POST',
                    headers: {
    ...
    if (tagInput) {
        ...
        tagInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(async () => {
                const response = await fetch(`{{ url_for(['module' => 'heritage', 'action' => 'apiSuggestTags']) }}?q=${encodeURIComponent(query)}`);
                const result = await response.json();
