...
@section('sidebar')
@include('heritage/adminSidebar', ['active' => 'featured'])
@endsection
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
        <form action="{{ url_for(['module' => 'heritage', 'action' => 'adminFeaturedCollections']) }}" method="post">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4" id="archival_select_wrapper" style="display: none;">
                    ...
                    <select class="form-select" id="archival_source_id" name="source_id_archival">
                        ...
                        @foreach ($archivalCollections as $c)
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
        ...
        <span class="badge bg-primary">{{ count($featured) }} collections</span>
    </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @if ($item->source_slug)
                            <br><small class="text-muted">{{ $item->source_slug }}</small>
                            @endif
                        </td>
                        <td>
                        ...
                        <td>
                            <form action="{{ url_for(['module' => 'heritage', 'action' => 'adminFeaturedCollections']) }}" method="post" class="d-inline">
                                <input type="hidden" name="featured_action" value="toggle">
                                <input type="hidden" name="featured_id" value="{{ $item->id }}">
                                <button type="submit" class="btn btn-sm btn-outline-secondary" title="{{ $item->is_enabled ? 'Disable' : 'Enable' }}">
                                    <i class="fas {{ $item->is_enabled ? 'fa-pause' : 'fa-play' }}"></i>
                                </button>
                            </form>
                            <form action="{{ url_for(['module' => 'heritage', 'action' => 'adminFeaturedCollections']) }}" method="post" class="d-inline" onsubmit="return confirm('Remove this collection from featured?');">
