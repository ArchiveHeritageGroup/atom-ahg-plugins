...
@section('sidebar')
@include('heritage/adminSidebar', ['active' => 'popia'])
...
<div class="card border-0 shadow-sm mt-4">
    ...
    <div class="card-body">
        <div class="d-flex justify-content-between mb-2">
            ...
            <span class="badge bg-warning">{{ $stats['unresolved'] ?? 0 }}</span>
        </div>
        <div class="d-flex justify-content-between mb-2">
            <span>Critical</span>
        ...
        <div class="d-flex justify-content-between mb-2">
            <span>High</span>
            <span class="badge bg-warning">{{ $stats['high'] ?? 0 }}</span>
        </div>
        <div class="d-flex justify-content-between">
            <span>Resolved (This Month)</span>
            <span class="badge bg-success">{{ $stats['resolved_this_month'] ?? 0 }}</span>
        </div>
    </div>
</div>
@endsection
        ...
        <form method="get" class="row g-3">
            <div class="col-md-4">
                <select class="form-select" name="severity">
                    ...
                    <option value="critical" {{ $sf_request->getParameter('severity') === 'critical' ? 'selected' : '' }}>Critical</option>
                    <option value="high" {{ $sf_request->getParameter('severity') === 'high' ? 'selected' : '' }}>High</option>
                    <option value="medium" {{ $sf_request->getParameter('severity') === 'medium' ? 'selected' : '' }}>Medium</option>
                    <option value="low" {{ $sf_request->getParameter('severity') === 'low' ? 'selected' : '' }}>Low</option>
                </select>
            </div>
            <div class="col-md-4">
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-transparent d-flex justify-content-between align-items-center">
        ...
        <span class="badge bg-warning">{{ number_format($total) }} flags</span>
    </div>
        ...
        <div class="list-group list-group-flush">
            ...
            <div class="list-group-item">
                <div class="row align-items-center">
                    <div class="col-md-1">
                        <span class="badge bg-{{ $color }} text-uppercase">{{ $flag->severity }}</span>
                    </div>
                    <div class="col-md-5">
                        <a href="{{ url_for(['module' => 'informationobject', 'slug' => $flag->slug]) }}" target="_blank">
                            {{ $flag->object_title ?? $flag->slug ?? 'Item' }}
                        </a>
                        <br>
                        <small class="text-muted">{{ ucfirst(str_replace('_', ' ', $flag->flag_type)) }}</small>
                    </div>
                    <div class="col-md-4">
                        @if ($flag->description)
                        <small>{{ substr($flag->description, 0, 100) }}</small>
                        @endif
                    </div>
