...
<section class="heritage-recent py-5 bg-light">
    <div class="container-xxl">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <a href="{{ url_for(['module' => 'heritage', 'action' => 'search', 'sort' => 'newest']) }}" class="btn btn-link text-decoration-none">
                   ...
                   style="width: 180px; scroll-snap-align: start;">
                    ...
                    <div class="heritage-recent-thumb rounded overflow-hidden mb-2" style="height: 180px;">
                        ...
                        <img src="{{ $item['thumbnail'] }}"
                             ...
                             loading="lazy"
                             onerror="this.parentElement.innerHTML='<div class=\'w-100 h-100 bg-{{ $bgColor }} bg-opacity-25 d-flex align-items-center justify-content-center\'><i class=\'bi {{ $iconClass }} display-4 text-{{ $bgColor }}\'></i></div>';">
                        @else
                        <div class="w-100 h-100 bg-{{ $bgColor }} bg-opacity-25 d-flex align-items-center justify-content-center">
                            <i class="fas {{ $iconClass }} display-4 text-{{ $bgColor }}"></i>
                        </div>
                        @endif
                    </div>

                    <!-- Title -->
                    <p class="small text-dark mb-0 text-truncate" title="{{ $item['title'] }}">
                        {{ $item['title'] }}
                    </p>
                </a>
                @endforeach
