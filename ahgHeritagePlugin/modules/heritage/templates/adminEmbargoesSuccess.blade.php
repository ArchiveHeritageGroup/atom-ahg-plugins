...
@section('sidebar')
@include('heritage/adminSidebar', ['active' => 'embargoes'])
...
<div class="card border-0 shadow-sm mt-4">
    ...
    <ul class="list-group list-group-flush">
        @foreach (array_slice($expiringEmbargoes, 0, 5) as $exp)
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <a href="{{ url_for(['module' => 'informationobject', 'slug' => $embargo->slug]) }}" target="_blank">
                                {{ $embargo->title ?? $embargo->slug ?? 'Item' }}
                            </a>
                        </td>
                        <td>
                            <span class="badge bg-{{ $color }}">{{ ucfirst(str_replace('_', ' ', $embargo->embargo_type)) }}</span>
                        </td>
                        <td>
                            @if ($embargo->end_date)
                            {{ date('Y-m-d', strtotime($embargo->end_date)) }}
                            @else
                            <span class="text-muted">Indefinite</span>
                            @endif
                        </td>
                        <td>
                            @if ($embargo->auto_release)
