        ...
        <div class="row mb-4">
            <div class="col">
                ...
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="btn-group">
                        <a href="{{ url_for(['module' => 'heritage', 'action' => 'search']) }}" class="btn btn-outline-secondary">
                            <i class="bi bi-search me-1"></i> Search
                        </a>
                        <a href="{{ url_for(['module' => 'heritage', 'action' => 'explore']) }}" class="btn btn-outline-secondary">
                    ...
                    <div class="card-body py-2">
                        <div class="row text-center small">
                            <div class="col">
                                ...
                                <strong id="stat-nodes">{{ number_format($stats['total_nodes'] ?? 0) }}</strong>
                            </div>
                            <div class="col">
                                <span class="text-muted">Edges:</span>
                                <strong id="stat-edges">{{ number_format($stats['total_edges'] ?? 0) }}</strong>
                            </div>
                            <div class="col">
                                <span class="text-muted">Avg connections:</span>
                                <strong id="stat-avg-connections">{{ number_format($stats['avg_connections_per_node'] ?? 0, 1) }}</strong>
                            </div>
                        </div>
                    </div>
                </div>
...
(function() {
    ...
    const dataUrl = '{{ url_for(['module' => 'heritage', 'action' => 'graphData']) }}';
    ...
    function renderGraph(data) {
        ...
        node.append('text')
            ...
            .attr('x', d => (d.size || 10) + 5).attr('y', 4)
            .attr('font-size', '11px').attr('fill', '#333');

        node.on('click', (event, d) => { event.stopPropagation(); showEntityPanel(d); });
    ...
    function showEntityPanel(entity) {
        ...
        content.innerHTML = `
            ...
            <div class="d-grid gap-2">
                <a href="{{ url_for(['module' => 'heritage', 'action' => 'search']) }}?ner_${entity.type}[]=${encodeURIComponent(entity.label)}" class="btn btn-sm btn-primary">
                    <i class="bi bi-search me-1"></i> View Records
                </a>
                <a href="{{ url_for(['module' => 'heritage', 'action' => 'entity']) }}/${entity.type}/${encodeURIComponent(entity.label)}" class="btn btn-sm btn-outline-secondary">
