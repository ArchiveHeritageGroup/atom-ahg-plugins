 ...
 * Entity Detail Page.
 *
 * Displays detailed information about a single entity from the knowledge graph.
 *
 * @var object $entity         Entity data from graph node
 * @var array  $relatedEntities Related entities
    ...
    <div class="container-xxl">
        ...
        <nav aria-label="breadcrumb" class="mb-3">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="<?php echo url_for(['module' => 'heritage', 'action' => 'graph']); ?>">Knowledge Graph</a></li>
                <li class="breadcrumb-item active"><?php echo esc_specialchars($entityData->canonical_value); ?></li>
            </ol>
        </nav>
            ...
            <div class="col-lg-8">
                            ...
                            <div class="flex-grow-1">
                                ...
                                <div class="d-flex flex-wrap gap-2 mb-3">
                                    ...
                                    <span class="badge bg-light text-dark fs-6">
                                        <i class="bi bi-file-earmark me-1"></i> <?php echo number_format($entityData->occurrence_count); ?> records
                                    </span>
                                    <?php if ($entityData->confidence_avg >= 0.9): ?>
                                    <span class="badge bg-success fs-6"><i class="bi bi-check-circle me-1"></i> High Confidence</span>
                                    <?php elseif ($entityData->confidence_avg >= 0.7): ?>
                                    <span class="badge bg-warning text-dark fs-6"><i class="bi bi-exclamation-circle me-1"></i> Medium Confidence</span>
                                    <?php endif; ?>
                                </div>

                                <?php if (!empty($entityData->description)): ?>
                                <p class="lead mb-0"><?php echo esc_specialchars($entityData->description); ?></p>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        <a href="<?php echo url_for(['module' => 'heritage', 'action' => 'search', 'ner_' . $entityData->entity_type . '[]' => $entityData->canonical_value]); ?>"
                    ...
                    <div class="card-body p-0">
                        ...
                        <div class="list-group list-group-flush">
                            <?php foreach (array_slice($objectsArray, 0, 10) as $obj): ?>
                            <a href="<?php echo url_for(['module' => 'informationobject', 'slug' => $obj->slug]); ?>"
                               class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                <div>
                                    <h6 class="mb-1"><?php echo esc_specialchars($obj->title ?? 'Untitled'); ?></h6>
                                    <small class="text-muted">
                                        <?php if ($obj->mention_count > 1): ?>
                                        Mentioned <?php echo $obj->mention_count; ?> times
                                        <?php endif; ?>
                                    </small>
                                </div>
                                <span class="badge bg-primary rounded-pill"><?php echo round($obj->confidence * 100); ?>%</span>
                            </a>
                            <?php endforeach; ?>
                        </div>
                        <?php else: ?>
                ...
                <!-- Related Entities -->
                <?php if (!empty($relatedEntitiesArray)): ?>
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            <?php foreach ($relatedEntitiesArray as $related): ?>
                            <div class="col-md-6">
                                <div class="card h-100 border">
                                    <div class="card-body py-2">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <span class="badge bg-light text-muted ms-auto"><?php echo $related['co_occurrences']; ?></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-lg-4">
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body d-grid gap-2">
                        ...
                        <a href="<?php echo url_for(['module' => 'heritage', 'action' => 'graph', 'focus' => $entityData->id]); ?>"
                ...
                <div class="card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <dl class="row mb-0">
                            ...
                            <dd class="col-7"><?php echo ucfirst(esc_specialchars($entityData->entity_type)); ?></dd>

                            <dt class="col-5 text-muted">Occurrences</dt>
                            <dd class="col-7"><?php echo number_format($entityData->occurrence_count); ?></dd>

                            <dt class="col-5 text-muted">Avg. Confidence</dt>
                            <dd class="col-7"><?php echo round($entityData->confidence_avg * 100); ?>%</dd>

                            <?php if (!empty($entityData->first_seen_at)): ?>
                            <dt class="col-5 text-muted">First Seen</dt>
                            <dd class="col-7"><?php echo date('M j, Y', strtotime($entityData->first_seen_at)); ?></dd>
                            <?php endif; ?>

                            <?php if (!empty($entityData->last_seen_at)): ?>
                            <dt class="col-5 text-muted">Last Updated</dt>
                            <dd class="col-7"><?php echo date('M j, Y', strtotime($entityData->last_seen_at)); ?></dd>
                            <?php endif; ?>
                        </dl>
                    </div>
                </div>
                ...
                <!-- External Links -->
                <?php if (!empty($entityData->wikidata_id) || !empty($entityData->viaf_id) || !empty($entityData->actor_id)): ?>
                <div class="card shadow-sm mb-4">
                    <div class="card-header">
                        <h5 class="mb-0">External Links</h5>
                    </div>
                    <div class="card-body d-grid gap-2">
                        <?php if (!empty($entityData->actor_id)): ?>
                        <a href="<?php echo url_for(['module' => 'actor', 'slug' => $entityData->actor_id]); ?>" class="btn btn-outline-primary btn-sm">
                            <i class="bi bi-person me-1"></i> Authority Record
                        </a>
                        <?php endif; ?>
                        <?php if (!empty($entityData->wikidata_id)): ?>
                        <a href="https://www.wikidata.org/wiki/<?php echo esc_specialchars($entityData->wikidata_id); ?>" target="_blank" class="btn btn-outline-secondary btn-sm">
                            <i class="bi bi-box-arrow-up-right me-1"></i> Wikidata
                        </a>
                        <?php endif; ?>
                        <?php if (!empty($entityData->viaf_id)): ?>
                        <a href="https://viaf.org/viaf/<?php echo esc_specialchars($entityData->viaf_id); ?>" target="_blank" class="btn btn-outline-secondary btn-sm">
