...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body text-center">
        ...
        <span class="badge bg-{{ match($contributor['trust_level'] ?? 'new') {
            ...
            {{ ucfirst($contributor['trust_level'] ?? 'new') }} Contributor
        </span>
        @if (!empty($contributor['bio']))
        <p class="text-muted small mt-3 mb-0">{!! nl2br(e($contributor['bio'])) !!}</p>
        @endif
        <p class="text-muted small mt-3 mb-0">
            Member since {{ date('F Y', strtotime($contributor['created_at'] ?? 'now')) }}
        </p>
    </div>
</div>
...
<div class="card border-0 shadow-sm mb-4">
    ...
    <div class="card-body">
        <div class="row g-3 text-center">
            <div class="col-4">
                <div class="h4 text-primary mb-0">{{ number_format($contributor['approved_contributions'] ?? 0) }}</div>
                <small class="text-muted">Approved</small>
            </div>
            <div class="col-4">
                <div class="h4 text-success mb-0">{{ number_format($contributor['points'] ?? 0) }}</div>
                <small class="text-muted">Points</small>
            </div>
            <div class="col-4">
                <div class="h4 text-info mb-0">{{ count($badges) }}</div>
...
<div class="card border-0 shadow-sm">
    ...
    <ul class="list-group list-group-flush">
        ...
        <li class="list-group-item d-flex align-items-center">
            <i class="fas {{ $badge['icon'] ?? 'fas fa-award' }} fs-4 text-{{ $badge['color'] ?? 'primary' }} me-3"></i>
            <div>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4 col-6">
                <div class="d-flex align-items-center">
                    <i class="fas {{ $stat['icon'] }} fs-3 text-primary me-3"></i>
                    <div>
                        <div class="h5 mb-0">{{ number_format($stat['count']) }}</div>
...
<div class="card border-0 shadow-sm">
    ...
    <div class="list-group list-group-flush">
        ...
        <a href="{{ url_for(['module' => 'informationobject', 'slug' => $contrib['item_slug']]) }}"
           class="list-group-item list-group-item-action">
            <div class="d-flex align-items-center">
                <i class="fas {{ $contrib['type_icon'] }} fs-4 text-primary me-3"></i>
                <div class="flex-grow-1">
                    <h6 class="mb-0">{{ $contrib['item_title'] ?? 'Untitled' }}</h6>
                    <small class="text-muted">
                        {!! $contrib['type_name'] !!} &middot;
...
<div class="mt-4">
    <a href="{{ url_for(['module' => 'heritage', 'action' => 'leaderboard']) }}" class="btn btn-outline-primary">
