# AHG Email Delivery Plugin

Email deliverability: bounce capture + suppression list + send-time gate (#145, parity with Heratio EmailBounceController/EmailSuppressionGate)
...
cd tmp-fetch && git sparse-checkout set ahgEmailDeliveryPlugin && cd ..
mv tmp-fetch/ahgEmailDeliveryPlugin ./ahgEmailDeliveryPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgEmailDeliveryPlugin/database/install.sql
