# AHG Redaction Plugin

Manual visual redaction of digital objects: draw regions over a PDF or image, save them, and generate a redacted copy. The original file is never modified.

| | |
|---|---|
| Machine name | `ahgRedactionPlugin` |
| Version | 1.0.0 |
| Category | compliance |
...
cd tmp-fetch && git sparse-checkout set ahgRedactionPlugin && cd ..
mv tmp-fetch/ahgRedactionPlugin ./ahgRedactionPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgRedactionPlugin/database/install.sql
