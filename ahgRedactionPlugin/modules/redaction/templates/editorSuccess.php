        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="<?php echo url_for(['module' => 'informationobject', 'slug' => $object->slug]); ?>"><?php echo esc_entities($object->getTitle(['cultureFallback' => true]) ?: $object->slug); ?></a></li>
                    <li class="breadcrumb-item active"><?php echo __('Visual redaction'); ?></li>
                </ol>
            </nav>
        </div>
        <div class="btn-group">
            <a href="<?php echo url_for(['module' => 'informationobject', 'slug' => $object->slug]); ?>" class="btn btn-outline-secondary">
...
<script <?php $n = sfConfig::get('csp_nonce', ''); echo $n ? preg_replace('/^nonce=/', 'nonce="', $n).'"' : ''; ?>>
// Token for the mutating endpoints. The framework rejects any non-GET request
// without it, and this page saves and applies over fetch().
const AHG_REDACTION_CSRF = '<?php echo htmlspecialchars($csrfToken ?? '', ENT_QUOTES); ?>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function saveRedactions() {
        ...
        try {
            ...
            const response = await fetch('<?php echo url_for(['module' => 'redaction', 'action' => 'save']); ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': AHG_REDACTION_CSRF },
    ...
    async function applyRedactions() {
        ...
        try {
            const response = await fetch('<?php echo url_for(['module' => 'redaction', 'action' => 'apply']); ?>', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': AHG_REDACTION_CSRF },
