<?php
/**
 * PSIS / AtoM-AHG - one C2PA assertion (a single self-contained provenance claim).
 *
 * Symfony 1.4 / PHP 8.3 port of Heratio's AhgC2pa\Manifest\Assertion. Behaviour
 * and on-wire bytes are identical; the only change is that the plugin loads this
 * class via require_once (no PSR-4 autoload for AtoM plugin classes) and depends
 * on the framework-installed ahg/inference-receipts library for JCS encoding.
 *
...
final class Assertion
{
    ...
    /**
     * Canonical (RFC 8785 JCS) byte form of the assertion data.
     */
        ...
        return new self('c2pa.actions.v2', [
            'actions' => [[
                ...
                'softwareAgent' => $parameters['softwareAgent'] ?? [
                    'name'    => 'AtoM-AHG',
                    'version' => $parameters['frameworkVersion'] ?? 'unknown',
                ],
                'parameters' => array_diff_key($parameters, array_flip(['softwareAgent', 'frameworkVersion'])),
            ]],
        ]);
    }
