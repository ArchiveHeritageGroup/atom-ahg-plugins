...
Donor browse and management using Laravel Query Builder
...
cd tmp-fetch && git sparse-checkout set ahgDonorManagePlugin && cd ..
mv tmp-fetch/ahgDonorManagePlugin ./ahgDonorManagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgDonorManagePlugin/database/install.sql
