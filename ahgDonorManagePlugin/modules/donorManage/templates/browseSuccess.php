    ...
    <table class="table table-bordered mb-0">
      ...
      <tbody>
        <?php foreach ($sf_data->getRaw('pager')->getResults() as $doc) { ?>
          <tr>
            <td>
              <?php echo link_to(render_title($doc['name']), '@donor_view_override?slug=' . $doc['slug']); ?>
            </td>
            <?php if ('alphabetic' != $sf_request->sort) { ?>
              <td>
...
<?php slot('after-content'); ?>
  ...
  <section class="actions mb-3">
    <?php echo link_to(__('Add new'), '@donor_add_override', ['class' => 'btn atom-btn-outline-light']); ?>
  </section>
