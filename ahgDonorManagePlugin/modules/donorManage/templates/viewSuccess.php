...
<div class="section border-bottom" id="basicInfo">
  ...
  <?php echo render_b5_section_heading(
      __('Basic info'),
      $canEdit,
      '@donor_edit_override?slug=' . $donor['slug'],
...
<?php slot('after-content'); ?>
  <?php if ($canEdit || $canDelete || $canCreate) { ?>
    <ul class="actions mb-3 nav gap-2">
      <?php if ($canEdit) { ?>
        <li><?php echo link_to(__('Edit'), '@donor_edit_override?slug=' . $donor['slug'], ['class' => 'btn atom-btn-outline-light']); ?></li>
      <?php } ?>
      <?php if ($canDelete) { ?>
        <li><?php echo link_to(__('Delete'), '@donor_delete_override?slug=' . $donor['slug'], ['class' => 'btn atom-btn-outline-danger']); ?></li>
      <?php } ?>
      <?php if ($canCreate) { ?>
        <li><?php echo link_to(__('Add new'), '@donor_add_override', ['class' => 'btn atom-btn-outline-light']); ?></li>
      <?php } ?>
    </ul>
  <?php } ?>
<?php end_slot(); ?>
