# AHG Web Annotations Plugin

Standalone W3C Web Annotation Data Model + Protocol backend (#146, parity with Heratio ahg-annotations)
...
cd tmp-fetch && git sparse-checkout set ahgAnnotationsPlugin && cd ..
mv tmp-fetch/ahgAnnotationsPlugin ./ahgAnnotationsPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgAnnotationsPlugin/database/install.sql
