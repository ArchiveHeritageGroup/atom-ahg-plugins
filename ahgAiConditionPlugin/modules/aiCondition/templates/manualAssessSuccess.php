...
<div class="sidebar-content">
    <div class="card mb-3">
        <div class="card-header bg-primary text-white py-2">
            <h6 class="mb-0"><i class="fas fa-clipboard-check me-1"></i><?php echo __('Manual Assessment') ?></h6>
        </div>
        <div class="card-body py-2 small">
            <p class="text-muted mb-2"><?php echo __('Record a condition assessment manually without AI. Fill in the condition grade, damages, and recommendations based on physical inspection.') ?></p>
            <a href="<?php echo url_for(['module' => 'aiCondition', 'action' => 'browse']) ?>" class="btn btn-sm btn-outline-secondary w-100">
                <i class="fas fa-arrow-left me-1"></i><?php echo __('Back to Browse') ?>
            </a>
        </div>
    </div>
</div>
...
<?php slot('title') ?>
<h1 class="h3 mb-0"><i class="fas fa-clipboard-check me-2"></i><?php echo __('Manual Condition Assessment') ?></h1>
<p class="text-muted small mb-3"><?php echo __('Record a manual condition assessment without AI') ?></p>
<?php end_slot() ?>
...
<div class="card">
    <div class="card-body">
        <form id="manualAssessForm" enctype="multipart/form-data">
            ...
            <!-- Condition Grade -->
            <div class="mb-3">
                <label class="form-label"><?php echo __('Condition Grade') ?> <span class="text-danger">*</span></label>
                <select class="form-select form-select-sm" id="conditionGrade" name="condition_grade" required>
                    <option value=""><?php echo __('-- Select grade --') ?></option>
                    <?php foreach (['excellent', 'good', 'fair', 'poor', 'critical'] as $g): ?>
                    <option value="<?php echo $g ?>"><?php echo ucfirst($g) ?></option>
                    <?php endforeach ?>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control form-control-sm" id="recommendations" name="recommendations" rows="3" placeholder="<?php echo __('Enter conservation recommendations...') ?>"></textarea>
            </div>

            <!-- Image Upload (optional) -->
            <div class="mb-3">
                <label class="form-label"><?php echo __('Reference Photo (optional)') ?></label>
...
document.getElementById('manualAssessForm').addEventListener('submit', function(e) {
    ...
    if (!grade) { alert('<?php echo __('Please select a condition grade') ?>'); return; }
    ...
    fetch('<?php echo url_for(['module' => 'aiCondition', 'action' => 'apiManualSave']) ?>', {
        method: 'POST',
        body: formData
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
        ...
        if (data.success && data.assessment_id) {
            window.location.href = '<?php echo url_for(['module' => 'aiCondition', 'action' => 'view', 'id' => '']) ?>' + data.assessment_id;
        } else {
    ...
    .catch(function() {
        ...
        var alertEl = document.getElementById('submitAlert');
        alertEl.style.display = '';
        alertEl.innerHTML = '<div class="alert alert-danger py-1 small"><i class="fas fa-times me-1"></i><?php echo __('Network error') ?></div>';
    });
});
