...
<style <?php echo $cspNonce ? preg_replace('/^nonce=/', 'nonce="', $cspNonce).'"' : ''; ?>>
  .galler-display-none-224b { display: none; }
  .galler-max-width-350px-white-space--4d57 { max-width:350px;white-space:normal; }
</style>
<?php use_helper('Date'); ?>
...
<div class="sidebar-content">
  ...
  <section id="template-selector" class="sidebar-section">
    <h4><?php echo __('Artwork Template'); ?></h4>
  ...
  <section id="cco-reference" class="sidebar-section">
    <h4><?php echo __('Standards Reference'); ?></h4>
    <p class="small"><?php echo __('This form follows CCO/CDWA standards for artwork cataloguing.'); ?></p>
    ...
    <a href="https://www.getty.edu/research/publications/electronic_publications/cdwa/" target="_blank" class="btn btn-sm btn-cco-guide mt-1">
      <i class="fa fa-external-link"></i> <?php echo __('CDWA'); ?>
    </a>
  </section>
...
#informationobject_adminInfo label, #informationobject_adminInfo .form-label { font-size: 13px !important; }
#informationobject_adminInfo select, #informationobject_adminInfo input { font-size: 13px !important; }
/* Reduce font sizes */
.cco-field label { font-size: 14px !important; }
.field-input input, .field-input select, .field-input textarea { font-size: 13px !important; }
.category-description { font-size: 12px !important; }
.help-text { font-size: 12px !important; }
/* Gallery Form Styling */
.sidebar-section {
  background: #fff;
  border-radius: 8px;
  padding: 15px;
...
/* Accordion Sections */
.gallery-cataloguing-form .accordion-item {
  border: none;
  margin-bottom: 10px;
  border-radius: 8px;
  overflow: hidden;
...
.gallery-cataloguing-form .accordion-button:focus {
  box-shadow: none;
}

.gallery-cataloguing-form .accordion-body {
  padding: 20px;
  background: #fff;
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  $("#repository").select2({
    theme: "bootstrap-5",
    placeholder: "Search gallery/repository...",
    allowClear: true,
    width: "100%"
  });
});
...
<div class="alert alert-danger">
  ...
  <p class="mt-2">
    <button type="submit" name="saveAnyway" value="1" form="gallery-form" class="btn btn-sm btn-warning">
...
<?php echo $form->renderFormTag(url_for(['module' => 'gallery', 'action' => ($resource->slug ?? null) ? 'edit' : 'add', 'slug' => $resource->slug ?? null]), [
  'id' => 'gallery-form',
  'class' => 'gallery-cataloguing-form'
]); ?>
      ...
      <div class="accordion-body">
        ...
        <?php foreach ($category['fields'] as $fieldName => $fieldDef): ?>
          ...
          <div class="cco-field level-<?php echo $level; ?>" data-field="<?php echo $fieldName; ?>">
            ...
            <div class="field-help" id="help-<?php echo $fieldName; ?>" class="galler-display-none-224b">
...
<?php if ($resourceId): ?>
  <?php $qubitResource = QubitInformationObject::getById($resourceId); ?>
...
(function() {
  ...
  // Getty Vocabulary Autocomplete with Tom Select
  ...
  function initGalleryAutocomplete() {
    if (typeof TomSelect === 'undefined') return;
    document.querySelectorAll(".gallery-autocomplete").forEach(function(input) {
      if (input.tomselect) return; // Already initialized
      ...
      new TomSelect(input, {
        ...
        load: function(query, callback) {
          var results = [];
          fetch("/index.php/museum/vocabulary?field=" + encodeURIComponent(fieldName) + "&query=" + encodeURIComponent(query))
            ...
            .then(function(localData) {
              ...
              if (query.length >= 2) {
                return fetch("/index.php/museum/getty?vocabulary=" + gettyVocab + "&query=" + encodeURIComponent(query) + "&limit=10");
              }
              return Promise.resolve(null);
            })
            .then(function(response) { if (response) return response.json(); return null; })
            .then(function(gettyData) {
              if (gettyData && gettyData.success && gettyData.results) {
                gettyData.results.forEach(function(item) {
                  var exists = results.some(function(r) { return r.label.toLowerCase() === item.label.toLowerCase(); });
                  if (!exists) { item.source = "getty"; item.sourceLabel = "Getty " + gettyVocab.toUpperCase(); results.push(item); }
                });
              }
              callback(results);
            })
            .catch(function(err) { console.error("Vocabulary load error:", err); callback(results); });
        },
        render: {
          option: function(item, escape) {
            var html = '<div class="vocab-option"><div class="d-flex justify-content-between align-items-start"><span class="vocab-label">' + escape(item.label) + '</span>';
            if (item.source === "local") { html += '<span class="badge bg-success ms-2">' + escape(item.sourceLabel) + '</span>'; }
            else if (item.source === "getty") { html += '<span class="badge bg-info ms-2">' + escape(item.sourceLabel) + '</span>'; }
            html += '</div>';
            if (item.broader) { html += '<div class="text-muted small">' + escape(item.broader) + '</div>'; }
            if (item.scopeNote) { html += '<div class="text-muted small galler-max-width-350px-white-space--4d57" >' + escape(item.scopeNote.substring(0, 80)) + (item.scopeNote.length > 80 ? '...' : '') + '</div>'; }
            html += '</div>';
            return html;
          },
          item: function(item, escape) { return '<div>' + escape(item.label) + '</div>'; },
          no_results: function(data, escape) { return '<div class="no-results p-2 text-muted">No results - type to add custom value</div>'; }
        }
      });
    });
  }
