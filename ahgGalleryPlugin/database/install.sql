...
CREATE TABLE IF NOT EXISTS `gallery_artist` (
  ...
  `artist_type` VARCHAR(53) COMMENT 'individual, collective, studio, anonymous' DEFAULT 'individual',
...
CREATE TABLE IF NOT EXISTS `gallery_artist_bibliography` (
  ...
  `entry_type` VARCHAR(84) COMMENT 'book, catalog, article, review, interview, thesis, website, video, other' DEFAULT 'article',
...
CREATE TABLE IF NOT EXISTS `gallery_artist_exhibition_history` (
  `id` int NOT NULL AUTO_INCREMENT,
  `artist_id` int NOT NULL,
  `exhibition_type` VARCHAR(51) COMMENT 'solo, group, duo, retrospective, survey' DEFAULT 'group',
...
CREATE TABLE IF NOT EXISTS `gallery_loan` (
  ...
  `loan_type` VARCHAR(30) COMMENT 'incoming, outgoing' NOT NULL,
  `status` VARCHAR(123) COMMENT 'inquiry, requested, approved, agreed, in_transit_out, on_loan, in_transit_return, returned, cancelled, declined' DEFAULT 'inquiry',
...
CREATE TABLE IF NOT EXISTS `gallery_facility_report` (
  `id` int NOT NULL AUTO_INCREMENT,
  `loan_id` int NOT NULL,
  `report_type` VARCHAR(30) COMMENT 'incoming, outgoing' NOT NULL,
...
CREATE TABLE IF NOT EXISTS `gallery_valuation` (
  `id` int NOT NULL AUTO_INCREMENT,
  `object_id` int NOT NULL,
  `valuation_type` VARCHAR(79) COMMENT 'insurance, market, replacement, auction_estimate, probate, donation' DEFAULT 'insurance',
...
CREATE TABLE IF NOT EXISTS `gallery_insurance_policy` (
  ...
  `policy_type` VARCHAR(77) COMMENT 'all_risk, named_perils, transit, exhibition, permanent_collection' DEFAULT 'all_risk',
