...
- `gallery_exhibition_checklist`
- `gallery_exhibition_object`
- `gallery_venue`
- `gallery_artist`
...
cd tmp-fetch && git sparse-checkout set ahgGalleryPlugin && cd ..
mv tmp-fetch/ahgGalleryPlugin ./ahgGalleryPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgGalleryPlugin/database/install.sql
