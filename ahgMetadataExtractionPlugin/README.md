...
Universal metadata extraction from digital objects using ExifTool, with automatic field mapping and preservation metadata support

| | |
|---|---|
| Machine name | `ahgMetadataExtractionPlugin` |
| Version | 1.0.0 |
...
- Field mapping to AtoM descriptive fields
...
cd tmp-fetch && git sparse-checkout set ahgMetadataExtractionPlugin && cd ..
mv tmp-fetch/ahgMetadataExtractionPlugin ./ahgMetadataExtractionPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgMetadataExtractionPlugin/database/install.sql
