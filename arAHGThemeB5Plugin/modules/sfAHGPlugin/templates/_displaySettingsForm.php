    ...
    <div class="row g-3 mb-4">
        <?php foreach ($allSettings as $setting): ?>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card h-100 display-module-card <?php echo $isLocked ? 'border-warning' : ''; ?>"
                     data-module="<?php echo $module; ?>">
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-start mb-2">
                            <div>
                                <h6 class="card-title mb-1">
                                    <i class="bi <?php echo $moduleInfo['icon']; ?> me-1"></i>
                                    <?php echo $moduleInfo['label']; ?>
                                </h6>
                        ...
                        <div class="current-mode mb-2">
                            <span class="badge bg-<?php echo $modeInfo['color']; ?> px-3 py-2">
                                <i class="bi <?php echo $modeInfo['icon']; ?> me-1"></i>
                                <?php echo $modeInfo['name']; ?>
                            </span>
                            <span class="text-muted small ms-2">
                                <?php echo $setting['items_per_page'] ?? 30; ?>/page
                            </span>
                        </div>
                        
                        <!-- Quick Mode Switcher -->
                        <div class="btn-group btn-group-sm w-100" role="group">
                            <?php foreach ($allModes as $mode => $info): ?>
                                <?php 
                                $isAvailable = in_array($mode, $availableModes);
                                $isActive = $setting['display_mode'] === $mode;
                                ?>
                                <button type="button" 
                                        class="btn btn-outline-<?php echo $info['color']; ?> quick-mode-btn
                                               <?php echo $isActive ? 'active' : ''; ?>
                                               <?php echo !$isAvailable ? 'disabled opacity-25' : ''; ?>"
                                        data-module="<?php echo $module; ?>"
                                        data-mode="<?php echo $mode; ?>"
                                        title="<?php echo $info['name']; ?><?php echo !$isAvailable ? ' (disabled)' : ''; ?>">
            ...
            <div class="modal-body">
                <form id="editDisplayForm">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <label class="form-label"><?php echo __('Default Display Mode'); ?></label>
                            <div class="btn-group-vertical w-100" id="editModeGroup">
                                <?php foreach ($allModes as $mode => $info): ?>
                                    <input type="radio" class="btn-check" name="display_mode" 
                                           id="edit_mode_<?php echo $mode; ?>" value="<?php echo $mode; ?>">
                                    <label class="btn btn-outline-<?php echo $info['color']; ?> text-start" 
                                           for="edit_mode_<?php echo $mode; ?>">
                        ...
                        <!-- Available Modes -->
                        <div class="col-md-6">
                            <label class="form-label"><?php echo __('Available Modes'); ?></label>
                            ...
                            <?php foreach ($allModes as $mode => $info): ?>
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input available-mode-edit" 
                                           name="available_modes[]" value="<?php echo $mode; ?>"
                                           id="edit_avail_<?php echo $mode; ?>">
                                    <label class="form-check-label" for="edit_avail_<?php echo $mode; ?>">
                        ...
                        <!-- Sort Default -->
                        <div class="col-md-4">
                            <label class="form-label"><?php echo __('Default Sort'); ?></label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Store settings data
    <?php foreach ($allSettings as $setting): ?>
    moduleSettings['<?php echo $setting['module']; ?>'] = <?php echo json_encode($setting); ?>;
    <?php endforeach; ?>
