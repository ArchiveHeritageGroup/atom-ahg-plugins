...
<style <?php echo $cspNonce ? preg_replace('/^nonce=/', 'nonce="', $cspNonce).'"' : ''; ?>>
  .inform-width-30-59c8 { width:30%; }
  .inform-width-80px-8db8 { width:80px; }
</style>
...
<?php slot('content'); ?>
  ...
  <form method="post" action="<?php echo $isNew ? url_for('@io_add_override') : url_for('@io_edit_override?slug=' . $rawIo['slug']); ?>" id="editForm">
              ...
              <table class="table table-sm" id="altids-table">
                <thead>
                  <tr>
                    ...
                    <th class="inform-width-80px-8db8"></th>
                  </tr>
                </thead>
                <tbody>
        ...
        <div id="notes-collapse" class="accordion-collapse collapse" aria-labelledby="notes-heading">
          <div class="accordion-body">
            ...
            <table class="table table-sm" id="notes-table">
              <thead>
                <tr>
                  <th class="inform-width-30-59c8"><?php echo __('Type'); ?></th>
    ...
    <?php // Admin-defined custom / "Additional Fields" (ahgCustomFieldsPlugin).
          ...
          // Only for an existing record (values key off the object id). Restored
          // here because record editing now routes to this AHG form instead of
          // the base ISAD edit form that used to include it. ?>
    <?php if (!$isNew && !empty($rawIo['id'])
