...
| | |
|---|---|
| Machine name | `ahgLandingPagePlugin` |
| Version | 1.0.0 |
| Category | admin |
...
cd tmp-fetch && git sparse-checkout set ahgLandingPagePlugin && cd ..
mv tmp-fetch/ahgLandingPagePlugin ./ahgLandingPagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgLandingPagePlugin/database/install.sql
