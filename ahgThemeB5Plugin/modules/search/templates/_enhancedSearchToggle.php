...
$userPref = $sf_user->getAttribute('enhanced_search', $isEnabled ? '1' : '0');
?>

<?php if ($isEnabled): ?>
<div class="enhanced-search-toggle d-inline-flex align-items-center ms-2" 
     title="<?php echo __('Smart search uses synonyms and related terms for better results'); ?>">
    <div class="form-check form-switch mb-0">
        <input class="form-check-input" type="checkbox" role="switch" 
               id="enhancedSearchToggle" 
               <?php echo $userPref === '1' ? 'checked' : ''; ?>>
        <label class="form-check-label small text-muted" for="enhancedSearchToggle">
            <i class="bi bi-stars me-1"></i><?php echo __('Smart'); ?>
        </label>
    </div>
</div>
