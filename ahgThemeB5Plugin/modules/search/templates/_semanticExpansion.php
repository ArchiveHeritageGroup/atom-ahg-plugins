...
<div class="semantic-expansion-info alert alert-info alert-dismissible fade show mb-3" role="alert">
  <div class="d-flex align-items-start">
    ...
    <div class="flex-grow-1">
      <strong><?php echo __('Semantic Search Active'); ?></strong>
      <p class="mb-1 small">
        <?php echo __('Your search has been expanded with related terms:'); ?>
      </p>

      <?php if (!empty($expansionInfo['terms'])): ?>
        <div class="expansion-terms">
          <?php foreach ($expansionInfo['terms'] as $originalTerm => $synonyms): ?>
            <div class="term-expansion mb-1">
              <code class="bg-light px-1 rounded"><?php echo htmlspecialchars($originalTerm); ?></code>
              <i class="fas fa-arrow-right mx-2 text-muted small" aria-hidden="true"></i>
              <span class="synonyms">
                <?php
                $synonymList = array_map(function ($syn) {
                    return '<span class="badge bg-secondary">' . htmlspecialchars($syn) . '</span>';
                }, $synonyms);
                echo implode(' ', $synonymList);
                ?>
              </span>
            </div>
          <?php endforeach; ?>
      ...
      <p class="mb-0 mt-2 small text-muted">
        ...
        <?php echo __('Disable semantic search in the search options to search for exact terms only.'); ?>
      </p>
    </div>
  </div>
