  ...
  <div class="multiline-header d-flex align-items-center mb-3">
    ...
    <div class="d-flex flex-column">
      ...
      <span class="small" id="heading-label">
        <?php echo render_title($resource) ?: $resource->slug; ?>
      </span>
    </div>
  </div>
<?php end_slot(); ?>
...
<?php slot('before-content'); ?>
  <div class="d-flex flex-wrap gap-2 mb-3">
    <?php echo get_component('search', 'inlineSearch', [
        'label' => __('Search %1%', ['%1%' => strtolower(render_title($resource) ?: $resource->slug)]),
        'landmarkLabel' => __(render_title($resource) ?: $resource->slug),
        'route' => url_for([
            'module' => 'taxonomy',
            'action' => 'index',
            'slug' => $resource->slug,
  ...
  <div class="table-responsive mb-3">
    <table class="table table-bordered mb-0">
      <thead>
        <tr>
          <th>
            <?php echo __('%1% term', ['%1%' => render_title($resource) ?: $resource->slug]); ?>
          </th><th>
      ...
      <tbody>
        <?php foreach ($pager->getResults() as $hit) { ?>
          ...
          <tr>
            ...
            <?php if ($addIoCountColumn) { ?>
              <td><?php echo countRelatedInformationObjects($hit->getId()); ?></td>
            <?php } ?>
            <?php if ($addActorCountColumn) { ?>
...
<?php slot('after-content'); ?>
  ...
  <?php if (\AtomExtensions\Services\AclService::check($resource, 'createTerm')) { ?>
    <section class="actions mb-3">
      <?php echo link_to(__('Add new'), ['module' => 'term', 'action' => 'add', 'taxonomy' => url_for(['module' => 'taxonomy', 'slug' => $resource->slug])], ['class' => 'btn atom-btn-outline-light']); ?>
    </section>
  <?php } ?>
...
<?php
/**
 * Count information objects related to a term via object_term_relation
 */
function countRelatedInformationObjects(int $termId): int
{
