...
<?php if (isset($latitude, $longitude) && $mapApiKey = sfConfig::get('app_google_maps_api_key')) { ?>
  <style <?php $n = sfConfig::get('csp_nonce', ''); echo $n ? preg_replace('/^nonce=/', 'nonce="', $n).'"' : ''; ?>></style>
...
<?php if (\AtomExtensions\Services\AclService::check($resource, ['update', 'delete', 'create'])) { ?>

  <?php slot('after-content'); ?>

    <ul class="actions mb-3 nav gap-2">
      <?php if (\AtomExtensions\Services\AclService::check($resource, 'update') || \AtomExtensions\Services\AclService::check($resource, 'translate')) { ?>
        <li><?php echo link_to(__('Edit'), [$resource, 'module' => 'repository', 'action' => 'edit'], ['class' => 'btn atom-btn-outline-light']); ?></li>
      <?php } ?>
      <?php if (\AtomExtensions\Services\AclService::check($resource, 'delete')) { ?>
        <li><?php echo link_to(__('Delete'), [$resource, 'module' => 'repository', 'action' => 'delete'], ['class' => 'btn atom-btn-outline-danger']); ?></li>
      <?php } ?>
      <?php if (\AtomExtensions\Services\AclService::check($resource, 'create')) { ?>
        <li><?php echo link_to(__('Add new'), ['module' => 'repository', 'action' => 'add'], ['class' => 'btn atom-btn-outline-light']); ?></li>
      <?php } ?>
      <?php if (\AtomExtensions\Services\AclService::check(QubitInformationObject::getRoot(), 'create')) { ?>
