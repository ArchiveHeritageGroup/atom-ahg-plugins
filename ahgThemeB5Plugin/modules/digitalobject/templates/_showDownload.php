...
<?php if (QubitTerm::REFERENCE_ID == $usageType) { ?>
  ...
  <?php if (isset($link)) { ?>
    <?php if ($hasPiiRedaction): ?>
      <?php echo link_to(image_tag($representation->getFullPath(), ['alt' => __('Open redacted %1%', ['%1%' => sfConfig::get('app_ui_label_digitalobject')]), 'class' => 'img-thumbnail']), $link, ['target' => '_blank']); ?>
      <div class="mt-1"><span class="badge bg-warning text-dark"><i class="fas fa-shield-alt me-1"></i><?php echo __('PII Redacted'); ?></span></div>
    <?php else: ?>
...
<?php } else { ?>
  ...
  <?php } else { ?>
    ...
    <div class="digitalObject text-center" style="width: 120px;">
      ...
      <?php if ($hasPiiRedaction): ?>
        <div class="mt-1"><span class="badge bg-warning text-dark small"><i class="fas fa-shield-alt me-1"></i><?php echo __('PII Redacted'); ?></span></div>
      <?php endif; ?>
