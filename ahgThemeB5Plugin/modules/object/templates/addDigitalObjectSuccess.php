...
<?php slot('content'); ?>
  ...
  <?php } else { ?>
        ...
        <div class="accordion-item">
          <h2 class="accordion-header" id="merge-heading">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#merge-collapse" aria-expanded="false" aria-controls="merge-collapse">
              <i class="fas fa-layer-group me-2"></i><?php echo __('Merge images to PDF'); ?>
            </button>
          </h2>
          <div id="merge-collapse" class="accordion-collapse collapse" aria-labelledby="merge-heading">
            <div class="accordion-body">
              ...
              <div class="row g-3 mb-3">
                <div class="col-md-4">
                  <label for="tpmJobName" class="form-label"><?php echo __('Document Name'); ?></label>
                  <input type="text" class="form-control" id="tpmJobName" value="<?php echo htmlspecialchars($resource->identifier ? $resource->identifier . ' - Merged' : 'Merged Document ' . date('Y-m-d')); ?>">
                </div>
                <div class="col-md-3">
                  <label for="tpmPdfStandard" class="form-label"><?php echo __('PDF Standard'); ?></label>
                ...
                <div class="col-md-2">
                  <label for="tpmDpi" class="form-label"><?php echo __('DPI'); ?></label>
                ...
                <div class="col-md-3">
                  <label for="tpmQuality" class="form-label"><?php echo __('Quality'); ?></label>
              ...
              <div class="d-flex justify-content-end gap-2">
                ...
                <button type="button" class="btn btn-primary" id="tpmCreateBtn" disabled>
                  <i class="fas fa-file-pdf me-1"></i><?php echo __('Create & Link PDF'); ?>
                </button>
              </div>
            </div>
          </div>
    ...
    (function() {
        ...
        var ioSlug = <?php echo json_encode($resource->slug ?? ''); ?>;
        var ioId = <?php echo json_encode($resource->id); ?>;
        var createUrl = <?php echo json_encode(url_for(['module' => 'tiffpdfmerge', 'action' => 'create'])); ?>;
        var uploadUrl = <?php echo json_encode(url_for(['module' => 'tiffpdfmerge', 'action' => 'upload'])); ?>;
        var reorderUrl = <?php echo json_encode(url_for(['module' => 'tiffpdfmerge', 'action' => 'reorder'])); ?>;
        var removeUrl = <?php echo json_encode(url_for(['module' => 'tiffpdfmerge', 'action' => 'removeFile'])); ?>;
        var processUrl = <?php echo json_encode(url_for(['module' => 'tiffpdfmerge', 'action' => 'process'])); ?>;
        var deleteUrl = <?php echo json_encode(url_for(['module' => 'tiffpdfmerge', 'action' => 'delete'])); ?>;
        var recordUrl = <?php echo json_encode(url_for([$resource, 'module' => 'informationobject'])); ?>;
