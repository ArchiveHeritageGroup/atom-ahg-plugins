...
class arFaceDetectionService
{
     ...
     */
    public function matchToAuthorities($faces, $imagePath)
    {
        $matched = [];
     ...
     * @return bool Success
     */
    public function indexAuthorityFace($imagePath, $authorityId, $faceCoords = null)
    {
     ...
     */
    protected function detectWithOpenCV($imagePath)
    {
     ...
     */
    protected function generateOpenCVScript()
    {
        return <<<'PYTHON'
#!/usr/bin/env python3
import sys
...
PYTHON;
     ...
     */
    protected function detectWithOpenCVFallback($imagePath)
    {
     ...
     */
    protected function clusterSkinRegions($regions, $imageWidth, $imageHeight)
    {
        if (count($regions) < 10) {
            return [];
        }
     ...
     */
    protected function detectWithAwsRekognition($imagePath)
    {
     ...
     */
    protected function detectWithAzure($imagePath)
    {
     ...
     */
    protected function detectWithGoogle($imagePath)
    {
     ...
     */
    protected function parseGoogleLandmarks($landmarks)
    {
        $result = [];
     ...
     */
    protected function extractFaceImage($imagePath, $face)
    {
        $imageInfo = @getimagesize($imagePath);
        if (!$imageInfo) {
            return null;
        ...
        if ($this->config['save_face_crops']) {
            $outputDir = $this->getUploadsDir() . '/' . $this->config['face_crop_path'];
            if (!is_dir($outputDir)) {
     ...
     */
    protected function searchAuthorityFaces($faceImagePath, $faceData)
    {
        $matches = [];
        ...
        try {
            $rows = DB::table('actor_face_index as afi')
                ->join('actor as a', 'afi.actor_id', '=', 'a.id')
                ->leftJoin('actor_i18n as ai', function ($join) {
                    $join->on('a.id', '=', 'ai.id')
                        ->where('ai.culture', '=', 'en');
                })
                ->where('afi.is_active', 1)
                ->select('afi.*', 'a.id as actor_id', 'ai.authorized_form_of_name')
                ->get();
     ...
     */
    protected function compareFaces($faceImage1, $faceImage2)
    {
     ...
     */
    protected function calculateHistogram($img)
    {
     ...
     * Calculate histogram correlation
     */
    protected function correlateHistograms($hist1, $hist2)
    {
     ...
     * Store face encoding in database
     */
    protected function storeFaceEncoding($authorityId, $faceImagePath, $faceCoords)
    {
     ...
     */
    protected function ensureFaceIndexTable()
    {
        try {
            DB::statement("CREATE TABLE IF NOT EXISTS actor_face_index (
     ...
     */
    public function linkFacesToInformationObject($faces, $informationObjectId)
    {
        $linked = 0;
