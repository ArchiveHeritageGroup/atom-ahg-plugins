...
Background jobs browse, delete, and report management using Laravel Query Builder
...
cd tmp-fetch && git sparse-checkout set ahgJobsManagePlugin && cd ..
mv tmp-fetch/ahgJobsManagePlugin ./ahgJobsManagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgJobsManagePlugin/database/install.sql
