...
CREATE TABLE IF NOT EXISTS `ahg_contract` (
    ...
    `counterparty_type` ENUM('vendor','institution','individual','government','other') DEFAULT 'vendor',
    ...
    `status` ENUM('draft','pending_review','pending_signature','active','suspended','expired','terminated','renewed') DEFAULT 'draft',
    ...
    `currency` VARCHAR(3) DEFAULT 'ZAR',
    `payment_terms` TEXT,
    `payment_frequency` ENUM('once','monthly','quarterly','annually','on_delivery') DEFAULT NULL,
    `scope_of_work` TEXT,
    `deliverables` TEXT,
    `general_terms` TEXT,
...
CREATE TABLE IF NOT EXISTS `ahg_contract_document` (
    ...
    `document_type` ENUM('signed_contract','draft','amendment','addendum','schedule','annexure','correspondence','quote','invoice','certificate','insurance','legal_opinion','other') NOT NULL,
    `filename` VARCHAR(255) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ahg_contract_reminder` (
    `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
    `contract_id` INT NOT NULL,
    `reminder_type` ENUM('expiry_warning','review_due','renewal_required','payment_due','deliverable_due','compliance_check','insurance_expiry','audit','custom') NOT NULL,
    `subject` VARCHAR(255) NOT NULL,
    `description` TEXT,
    `reminder_date` DATE NOT NULL,
    `advance_days` INT DEFAULT 30,
    `is_recurring` TINYINT(1) DEFAULT 0,
    `recurrence_pattern` ENUM('daily','weekly','monthly','quarterly','yearly') DEFAULT NULL,
    `recurrence_end_date` DATE DEFAULT NULL,
    `priority` ENUM('low','normal','high','urgent') DEFAULT 'normal',
    `notify_email` TINYINT(1) DEFAULT 1,
    `notification_recipients` TEXT,
    `status` ENUM('active','snoozed','completed','cancelled') DEFAULT 'active',
...
CREATE TABLE IF NOT EXISTS `ahg_contract_history` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `contract_id` INT NOT NULL,
    `action` ENUM('created','updated','status_changed','approved','renewed','terminated','document_added','document_removed','reminder_sent','note_added') NOT NULL,
