...
<div class="modal fade" id="addItemModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" action="@url('ahg_vend_transaction_item_add', ['id' => $transaction->id])" id="addItemForm">
                ...
                if (data.length > 0) {
                    ...
                    data.forEach(function(item) {
                        html += '<div class="autocomplete-item" onclick="selectGlamItem(' + JSON.stringify(item).replace(/"/g, '&quot;') + ')">'
                            + '<div class="item-title">' + escapeHtmlModal(item.value) + '</div>'
                            + '<div class="item-meta">'
                            + (item.identifier ? '<code>' + escapeHtmlModal(item.identifier) + '</code> &bull; ' : '')
                            + (item.level ? '<span class="badge bg-secondary">' + escapeHtmlModal(item.level) + '</span>' : '')
                            + '</div></div>';
                    });
