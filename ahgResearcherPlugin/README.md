# Researcher Collection Upload

Researcher collection upload and approval workflow — online submissions and offline exchange import with two-step archivist review
...
cd tmp-fetch && git sparse-checkout set ahgResearcherPlugin && cd ..
mv tmp-fetch/ahgResearcherPlugin ./ahgResearcherPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgResearcherPlugin/database/install.sql
