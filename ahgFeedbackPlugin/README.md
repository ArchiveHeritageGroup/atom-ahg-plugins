...
User feedback and suggestions management for archival records

| | |
|---|---|
| Machine name | `ahgFeedbackPlugin` |
| Version | 1.0.6 |
...
cd tmp-fetch && git sparse-checkout set ahgFeedbackPlugin && cd ..
mv tmp-fetch/ahgFeedbackPlugin ./ahgFeedbackPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgFeedbackPlugin/database/install.sql
