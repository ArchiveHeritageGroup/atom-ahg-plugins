...
OCFL v1.1 (Oxford Common File Layout) preservation storage: storage-root management, content-addressed object versioning with deterministic inventory.json + SHA-512 digests, per-version directories, fixity verification and tar export.
...
cd tmp-fetch && git sparse-checkout set ahgOcflPlugin && cd ..
mv tmp-fetch/ahgOcflPlugin ./ahgOcflPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgOcflPlugin/database/install.sql
