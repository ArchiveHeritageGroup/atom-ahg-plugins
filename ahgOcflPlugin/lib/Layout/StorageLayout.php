 ...
 *   - hashed-n-tuple sha256(id) split into three 3-char chunks.
...
final class StorageLayout
{
    ...
    public function __construct(string $layout = self::FLAT_ID)
    {
        if (!in_array($layout, [self::FLAT_ID, self::PAIRTREE, self::HASHED_N_TUPLE], true)) {
            throw new InvalidArgumentException("Unknown OCFL storage layout '{$layout}'");
        }
        $this->layout = $layout;
    }
    ...
    public function pathFor(string $objectId): string
    {
        ...
        switch ($this->layout) {
            case self::PAIRTREE:
                return $this->pairtree($id);
            case self::HASHED_N_TUPLE:
                return $this->hashedNTuple($id);
            case self::FLAT_ID:
            default:
                return $this->safeName($id);
        }
    }
    ...
    public function descriptor(): array
    {
        switch ($this->layout) {
            case self::PAIRTREE:
                $desc = 'Pairtree (Namaste) - two-character pairs of the url-encoded id';
                break;
            case self::HASHED_N_TUPLE:
                $desc = 'Hashed n-tuple - sha256(id) split into 3 x 3-char segments';
                break;
            case self::FLAT_ID:
            default:
                $desc = 'Flat object id (one directory per object, name = url-encoded id)';
                break;
        }
