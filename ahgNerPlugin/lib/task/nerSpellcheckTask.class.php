 ...
 */
class nerSpellcheckTask extends sfBaseTask
{
...
The [ner:spellcheck|INFO] task checks spelling in metadata fields.

Examples:
  [php symfony ner:spellcheck --all --limit=100|INFO]
  [php symfony ner:spellcheck --object=12345|INFO]
EOD;
    }
    ...
    protected function execute($arguments = [], $options = [])
    {
        ...
        $this->logSection('spellcheck', 'Starting spell check task');

        $settings = $this->getSettings();
        if (($settings['spellcheck_enabled'] ?? '0') !== '1') {
            $this->logSection('spellcheck', 'Spell check disabled', null, 'ERROR');
            return 1;
        }

        $language = $options['language'] ?: ($settings['spellcheck_language'] ?? 'en_US');
        ...
        if (empty($objectIds)) {
            $this->logSection('spellcheck', 'No objects to check');
            return 0;
        }

        $this->logSection('spellcheck', sprintf('Checking %d objects (lang: %s)', count($objectIds), $language));

        if ($options['dry-run']) {
            foreach ($objectIds as $id) $this->logSection('spellcheck', "Would check: $id");
            return 0;
        }
        ...
        foreach ($objectIds as $objectId) {
            if ($checked >= (int)($options['limit'] ?: 100)) break;
            ...
            if (!empty($errors)) {
                ...
                $this->logSection('spellcheck', "Object $objectId: $count issues");
            }
        }

        $this->logSection('spellcheck', sprintf('Done: %d checked, %d with issues', $checked, $withErrors));
        return 0;
    }
