...
INSERT INTO naz_config (config_key, config_value, description) VALUES
...
CREATE TABLE IF NOT EXISTS naz_closure_period (
    ...
    closure_type ENUM('standard', 'extended', 'indefinite', 'ministerial') NOT NULL DEFAULT 'standard',
    ...
    review_date DATE,
    status ENUM('active', 'expired', 'extended', 'released') DEFAULT 'active',
    release_notes TEXT,
    released_by INT,
...
CREATE TABLE IF NOT EXISTS naz_researcher (
    ...
    researcher_type ENUM('local', 'foreign', 'institutional') NOT NULL DEFAULT 'local',
    title VARCHAR(20),
...
CREATE TABLE IF NOT EXISTS naz_research_permit (
    ...
    permit_type ENUM('general', 'restricted', 'special') NOT NULL DEFAULT 'general',
    ...
    approved_by INT,
    approved_date DATE,
    status ENUM('pending', 'approved', 'rejected', 'active', 'expired', 'revoked') DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS naz_records_schedule (
    ...
    disposal_action ENUM('destroy', 'transfer', 'review', 'permanent') NOT NULL,
    legal_authority TEXT COMMENT 'Legal basis for retention',
    classification ENUM('vital', 'important', 'useful', 'non-essential') DEFAULT 'useful',
    access_restriction ENUM('open', 'restricted', 'confidential', 'secret') DEFAULT 'open',
    approved_by VARCHAR(255),
    approval_date DATE,
    effective_date DATE,
    review_date DATE,
    status ENUM('draft', 'pending', 'approved', 'superseded', 'archived') DEFAULT 'draft',
    notes TEXT,
    created_by INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS naz_transfer (
    ...
    transfer_type ENUM('scheduled', 'voluntary', 'rescue', 'donation') DEFAULT 'scheduled',
    description TEXT,
    ...
    actual_date DATE,
    received_by INT,
    status ENUM('proposed', 'scheduled', 'in_transit', 'received', 'accessioned', 'rejected', 'cancelled') DEFAULT 'proposed',
    rejection_reason TEXT,
...
CREATE TABLE IF NOT EXISTS naz_transfer_item (
    ...
    access_restriction ENUM('open', 'restricted', 'confidential') DEFAULT 'open',
...
CREATE TABLE IF NOT EXISTS naz_protected_record (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    information_object_id INT NOT NULL,
    protection_type ENUM('cabinet', 'security', 'personal', 'legal', 'commercial') NOT NULL,
