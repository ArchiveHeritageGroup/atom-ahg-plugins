...
@section('title')
  <h1><i class="fas fa-lock me-2"></i>{{ __('Access Restricted') }}</h1>
@endsection
    ...
    <div class="card border-warning">
      <div class="card-header bg-warning text-dark">
        <h4 class="mb-0">
          ...
          {{ $embargoInfo['type_label'] ?? __('Access Restricted') }}
        </h4>
      </div>
      <div class="card-body text-center py-5">
        ...
        @if (!$embargoInfo['is_perpetual'] && !empty($embargoInfo['end_date']))
          <div class="alert alert-info d-inline-block mt-3">
            <i class="fas fa-calendar-alt me-2"></i>
            {{ __('This record will become available on %1%', ['%1%' => date('j F Y', strtotime($embargoInfo['end_date']))]) }}
          </div>
        @elseif ($embargoInfo['is_perpetual'])
          <div class="alert alert-secondary d-inline-block mt-3">
            <i class="fas fa-ban me-2"></i>
            {{ __('This restriction is indefinite') }}
          </div>
        @endif
        ...
        @if (sfContext::getInstance()->getUser()->isAuthenticated())
          ...
          <p class="text-muted small">
            ...
            {{ __('If you believe you should have access to this record, please contact the repository administrator.') }}
          </p>
        @endif
      </div>
    </div>
