<div class="row">
  ...
  <div class="col-md-9">
    ...
    <div class="card mt-4">
      ...
      <div class="card-body">
        <div class="row text-center">
          ...
          <div class="col">
            <h3>{{ number_format($stats->with_rights_statement ?? 0) }}</h3>
            <small class="text-muted">With Rights Statement</small>
          </div>
          <div class="col">
            <h3>{{ number_format($stats->with_creative_commons ?? 0) }}</h3>
            <small class="text-muted">With CC License</small>
          </div>
          <div class="col">
            <h3>{{ number_format($stats->with_tk_labels ?? 0) }}</h3>
    ...
    <!-- Admin Actions -->
    @if ($sf_user->isAuthenticated() && $sf_user->hasCredential('administrator'))
    <div class="card mt-4">
      <div class="card-header">
        <h5 class="mb-0">Administration</h5>
      </div>
      <div class="card-body">
        <a href="{{ url_for(['module' => 'extendedRights', 'action' => 'batch']) }}" class="btn btn-primary">
          <i class="fas fa-layer-group"></i> Batch Assign Rights
        </a>
        <a href="{{ url_for(['module' => 'extendedRights', 'action' => 'embargoes']) }}" class="btn btn-warning">
