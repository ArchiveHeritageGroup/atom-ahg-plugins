...
class ProcessCommand extends BaseCommand
{
    ...
    Processes embargo expiry operations. Should be run daily via cron (recommended: 6am).
    ...
    Examples:
      php bin/atom embargo:process                    # Process all operations
      php bin/atom embargo:process --dry-run          # Preview without changes
      php bin/atom embargo:process --notify-only      # Only send notifications
      php bin/atom embargo:process --lift-only         # Only lift expired embargoes
      php bin/atom embargo:process --warn-days=14,7,3 # Custom warning intervals

    Cron setup:
      0 6 * * * cd /usr/share/nginx/archive && php bin/atom embargo:process
    EOF;

    protected function configure(): void
    {
        $this->addOption('dry-run', null, 'Preview changes without executing');
        $this->addOption('notify-only', null, 'Only send expiry notifications');
        $this->addOption('lift-only', null, 'Only lift expired embargoes');
        $this->addOption('warn-days', null, 'Days before expiry to warn (comma-separated)', '30,7,1');
    }
    ...
    protected function handle(): int
    {
        ...
        $runAll = !$this->hasOption('notify-only') && !$this->hasOption('lift-only');
        ...
        if ($runAll || $this->hasOption('lift-only')) {
        ...
        if ($runAll || $this->hasOption('notify-only')) {
    ...
    private function liftExpiredEmbargoes(bool $dryRun): array
    {
        ...
        foreach ($expiredEmbargoes as $embargo) {
            ...
            try {
                ...
                $results['lifted']++;
                $this->success("Lifted: {$title} (embargo #{$embargo->id})");

                try {
                    $notificationService->sendLiftedNotification($embargo, 'Auto-released after expiry date');
                } catch (\Exception $e) {
                    $this->warning("Failed to send lifted notification for embargo #{$embargo->id}: " . $e->getMessage());
                }
            } catch (\Exception $e) {
        ...
        foreach ($warnDays as $days) {
            ...
            foreach ($expiringEmbargoes as $embargo) {
                ...
                try {
                    ...
                    if ($sent) {
                        $results['sent']++;
                        $this->success("Sent {$days}-day warning: {$title}");
                    } else {
                        $results['failed']++;
                        $this->warning("Failed to send warning for: {$title} (no recipients)");
                    }
                } catch (\Exception $e) {
                    $results['failed']++;
