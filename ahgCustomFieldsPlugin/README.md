...
Admin-configurable custom metadata fields for any entity type. Define fields via UI — no code changes needed.

| | |
|---|---|
| Machine name | `ahgCustomFieldsPlugin` |
| Version | 1.0.0 |
| Category | metadata |
...
cd tmp-fetch && git sparse-checkout set ahgCustomFieldsPlugin && cd ..
mv tmp-fetch/ahgCustomFieldsPlugin ./ahgCustomFieldsPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgCustomFieldsPlugin/database/install.sql
