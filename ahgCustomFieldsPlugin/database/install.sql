...
-- ---------------------------------------------------------------------------
-- Merged in from database/seeds/archaeology_profile.sql on 2026-08-18.
--
...
INSERT IGNORE INTO `menu` (`parent_id`, `name`, `path`, `lft`, `rgt`, `source_culture`, `created_at`, `updated_at`)
SELECT @browse, 'browseFindSearch', 'customFields/search', @rgt, @rgt + 1, 'en', NOW(), NOW()
  FROM DUAL WHERE @exists = 0;

INSERT IGNORE INTO `menu_i18n` (`id`, `culture`, `label`)
