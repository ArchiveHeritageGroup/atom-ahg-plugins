...
<?php slot('title'); ?>
  <div class="multiline-header d-flex flex-column mb-3">
    <h1 class="mb-0" aria-describedby="heading-label">
      <?php echo $isNew ? __('Add new rights holder') : __('Edit rights holder'); ?>
    </h1>
    <?php if (!$isNew) { ?>
...
<?php slot('content'); ?>
  ...
  <form method="post" action="<?php echo $isNew ? url_for('@rightsholder_add_override') : url_for('@rightsholder_edit_override?slug=' . $rightsHolder['slug']); ?>" id="editForm">
          ...
          <div class="accordion-body">
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="authorizedFormOfName" name="authorizedFormOfName"
                     value="<?php echo esc_specialchars($rightsHolder['authorizedFormOfName']); ?>" required>
            </div>
          </div>
        </div>
      </div>
    ...
    <ul class="actions mb-3 nav gap-2">
      <?php if (!$isNew) { ?>
        <li><?php echo link_to(__('Cancel'), '@rightsholder_view_override?slug=' . $rightsHolder['slug'], ['class' => 'btn atom-btn-outline-light', 'role' => 'button']); ?></li>
        <li><input class="btn atom-btn-outline-success" type="submit" value="<?php echo __('Save'); ?>"></li>
      <?php } else { ?>
        <li><?php echo link_to(__('Cancel'), '@rightsholder_browse_override', ['class' => 'btn atom-btn-outline-light', 'role' => 'button']); ?></li>
