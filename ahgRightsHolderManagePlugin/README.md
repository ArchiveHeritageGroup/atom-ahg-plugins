...
cd tmp-fetch && git sparse-checkout set ahgRightsHolderManagePlugin && cd ..
mv tmp-fetch/ahgRightsHolderManagePlugin ./ahgRightsHolderManagePlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgRightsHolderManagePlugin/database/install.sql
