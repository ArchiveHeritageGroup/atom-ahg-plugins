...
Field recording for archaeological, heritage and rock art sites - site records attached to authority records, with role-gated locality

| | |
|---|---|
| Machine name | `ahgSiteRecordPlugin` |
| Version | 1.0.0 |
| Category | heritage |
...
cd tmp-fetch && git sparse-checkout set ahgSiteRecordPlugin && cd ..
mv tmp-fetch/ahgSiteRecordPlugin ./ahgSiteRecordPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgSiteRecordPlugin/database/install.sql
