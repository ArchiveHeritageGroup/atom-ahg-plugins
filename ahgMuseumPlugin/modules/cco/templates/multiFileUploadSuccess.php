 ...
 */
function cco_reached_upload_limit(): bool
{
    $uploadLimit = sfConfig::get('app_upload_limit', 0);
    
    // If no limit set (0 or null), uploads are unlimited
    if (empty($uploadLimit)) {
        return false;
    }
    
    // Convert GB to bytes
    $limitBytes = $uploadLimit * 1024 * 1024 * 1024;
    
    // Calculate total size of all digital objects
 ...
 */
function cco_get_resource_title($resource): string
{
    // Try to get title from i18n table
    if (isset($resource->id)) {
        ...
        if ($ioData && $ioData->title) {
            return htmlspecialchars($ioData->title);
        }
    ...
    // Fallback to slug or identifier
    return htmlspecialchars($resource->slug ?? $resource->identifier ?? '');
}

/**
 * Build URL for resource using slug
 */
function cco_resource_url($resource, string $module, ?string $action = null): string
{
    ...
    if (isset($resource->slug) && $resource->slug) {
...
<?php slot('title'); ?>
  <div class="multiline-header d-flex flex-column mb-3">
    ...
    <span class="small" id="heading-label">
      <?php echo cco_get_resource_title($resource); ?>
    </span>
  </div>
<?php end_slot(); ?>
...
<?php slot('content'); ?>
  ...
  <noscript>
    ...
    <section class="actions mb-3">
      <a href="<?php echo cco_resource_url($resource, 'informationobject'); ?>" class="btn atom-btn-outline-light"><?php echo __('Cancel'); ?></a>
    </section>
  </noscript>

  <?php if (cco_reached_upload_limit()) { ?>
  ...
  <?php } else { ?>
    <div class="multifileupload-form"
      ...
      <?php echo $form->renderFormTag(cco_resource_url($resource, 'informationobject', 'multiFileUpload'), ['id' => 'multiFileUploadForm', 'class' => 'd-inline']); ?>
        ...
        <ul class="actions mb-3 nav gap-2">
          <li><a href="<?php echo cco_resource_url($resource, 'informationobject'); ?>" class="btn atom-btn-outline-light" role="button"><?php echo __('Cancel'); ?></a></li>
