...
CREATE TABLE IF NOT EXISTS `custom_report` (
    ...
    `status` VARCHAR(59) COMMENT 'draft, in_review, approved, published, archived' DEFAULT 'draft',
    `template_id` BIGINT UNSIGNED DEFAULT NULL,
    `data_mode` VARCHAR(26) COMMENT 'live, snapshot' DEFAULT 'live',
...
CREATE TABLE IF NOT EXISTS `report_schedule` (
    ...
    `frequency` VARCHAR(45) COMMENT 'daily, weekly, monthly, quarterly' NOT NULL,
    `day_of_week` TINYINT DEFAULT NULL,
    `day_of_month` TINYINT DEFAULT NULL,
    `time_of_day` TIME DEFAULT '08:00:00',
    `output_format` VARCHAR(26) COMMENT 'pdf, xlsx, csv' DEFAULT 'pdf',
    ...
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME ON UPDATE CURRENT_TIMESTAMP,
    `schedule_type` VARCHAR(30) COMMENT 'recurring, trigger' DEFAULT 'recurring',
...
CREATE TABLE IF NOT EXISTS `report_definition` (
    ...
    `name` VARCHAR(255) NOT NULL,
    `description` TEXT DEFAULT NULL,
    `category` VARCHAR(101) COMMENT 'collection, acquisition, access, preservation, researcher, compliance, statistics, custom' NOT NULL,
