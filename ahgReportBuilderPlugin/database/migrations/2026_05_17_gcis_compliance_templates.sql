 ...
 JSON_OBJECT(
   'sections', JSON_ARRAY(
     JSON_OBJECT(
       ...
       'content', '<p>This report summarises every audited action recorded by the system in the selected reporting period. Each row is sourced from ahg_audit_log and is uniquely identified by a UUID. The data underpins compliance with NARSSA Act 1996 §13(2)(d) and POPIA §17.</p>',
       'position', 0
     ),
     JSON_OBJECT(
 ...
 JSON_OBJECT(
   'sections', JSON_ARRAY(
     JSON_OBJECT(
       'title', 'Overview',
       'section_type', 'narrative',
       'content', '<p>User activity report covering authenticated views, share-link recipient accesses, and authentication events. Source data: ahg_audit_log + information_object_share_access.</p>',
       'position', 0
     ),
     JSON_OBJECT(
...
('GCIS Compliance: Retention Status & Lifecycle',
 ...
 JSON_OBJECT(
   'sections', JSON_ARRAY(
     ...
     JSON_OBJECT(
       'title', 'Approved transfers awaiting NARSSA package',
       'section_type', 'sql_table',
       'config', JSON_OBJECT(
