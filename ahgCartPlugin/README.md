...
Shopping cart with dual-mode support: Standard (Request to Publish) and E-Commerce with PayFast payment integration

| | |
|---|---|
| Machine name | `ahgCartPlugin` |
| Version | 2.1.1 |
| Category | ahg |
...
cd tmp-fetch && git sparse-checkout set ahgCartPlugin && cd ..
mv tmp-fetch/ahgCartPlugin ./ahgCartPlugin && rm -rf tmp-fetch

# 2. Apply the schema.
mysql -u <user> -p <database> < ahgCartPlugin/database/install.sql
