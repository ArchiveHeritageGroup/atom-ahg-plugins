...
require_once dirname(__FILE__, 4).'/lib/Services/EcommerceService.php';
...
class cartCheckoutAction extends AhgController
{
    ...
    protected function processStandardCheckout($request, $userId, $sessionId, $cartService)
    {
        ...
        if ($userId) {
            ...
            $this->redirect(['module' => 'cart', 'action' => 'thankYou']);
        }
    }
    ...
    protected function processEcommerceCheckout($request, $userId, $sessionId, $ecommerceService)
    {
        ...
        if ($result['total'] == 0) {
            ...
            $this->redirect(['module' => 'cart', 'action' => 'orderConfirmation', 'order' => $result['order_number']]);
            return;
        }

        // Redirect to payment
        $this->redirect(['module' => 'cart', 'action' => 'payment', 'order' => $result['order_number']]);
    }
}
