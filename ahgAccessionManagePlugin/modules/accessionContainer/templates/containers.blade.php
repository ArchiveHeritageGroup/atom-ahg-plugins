...
@php
  ...
  $containers = isset($containers) ? $containers : [];
  $containerItems = isset($containerItems) ? $containerItems : [];
  $containerTypes = isset($containerTypes) ? $containerTypes : [];
  $conditionStatuses = isset($conditionStatuses) ? $conditionStatuses : [];
@endphp
...
<div class="container-fluid px-0">
  ...
  @else
    <div id="containersList">
      ...
      <div class="card mb-3 container-card" data-container-id="{{ $cId }}">
        <div class="card-header d-flex justify-content-between align-items-center">
          <div>
            <i class="fas {{ $typeIcon }} me-2"></i>
            <span class="badge bg-info text-dark me-2">{{ ucfirst(str_replace('_', ' ', $container->container_type ?? 'box')) }}</span>
            <strong>{{ htmlspecialchars($container->label ?? '') }}</strong>
            @if (!empty($container->barcode))
              <code class="ms-2">{{ $container->barcode }}</code>
            @endif
          </div>
          <div class="d-flex align-items-center gap-2">
            <span class="badge bg-secondary">{{ $itemCount }} <?php echo __('items'); ?></span>
            @if (!empty($container->condition_status))
              <span class="badge bg-{{ $condBadge }}">{{ ucfirst($container->condition_status) }}</span>
            @endif
            <button type="button" class="btn btn-sm btn-outline-primary toggle-items-btn" data-container="{{ $cId }}" title="<?php echo __('Toggle items'); ?>">
              <i class="fas fa-chevron-down"></i>
            </button>
            <button type="button" class="btn btn-sm btn-outline-danger delete-container-btn" data-container="{{ $cId }}" title="<?php echo __('Delete container'); ?>">
        ...
        <div class="card-body">
          ...
          <div class="row mb-2">
            @if (!empty($container->location_name))
              <div class="col-md-3">
                <small class="text-muted"><?php echo __('Location'); ?>:</small><br>
                {{ $container->location_name }}
              </div>
            @endif
            @if (!empty($container->location_detail))
              <div class="col-md-3">
                <small class="text-muted"><?php echo __('Detail'); ?>:</small><br>
                {{ htmlspecialchars($container->location_detail) }}
              </div>
            @endif
            @if (!empty($container->dimensions))
              <div class="col-md-3">
                <small class="text-muted"><?php echo __('Dimensions'); ?>:</small><br>
                {{ htmlspecialchars($container->dimensions) }}
              </div>
            @endif
            @if (!empty($container->weight_kg))
              <div class="col-md-3">
                <small class="text-muted"><?php echo __('Weight'); ?>:</small><br>
                {{ number_format($container->weight_kg, 2) }} kg
              </div>
            @endif
          </div>

          @if (!empty($container->notes))
            <p class="text-muted small mb-2"><em>{{ htmlspecialchars($container->notes) }}</em></p>
          @endif
          ...
          <div class="container-items-section" id="containerItems{{ $cId }}" style="display:none;">
                ...
                <table class="table table-sm table-bordered mb-3">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>{{ htmlspecialchars($item->date_range ?? '-') }}</td>
                      <td>
                        @if (!empty($item->io_title))
                          <span class="text-primary">{{ htmlspecialchars($item->io_title) }}</span>
                        @else
                          <span class="text-muted">-</span>
                        @endif
                      </td>
                      <td>
                        <button type="button" class="btn btn-sm btn-outline-danger delete-item-btn" data-item="{{ $item->id }}" data-container="{{ $cId }}" title="<?php echo __('Delete'); ?>">
            ...
            <div class="bg-light p-3 rounded">
              ...
              <form class="add-item-form" data-container="{{ $cId }}">
