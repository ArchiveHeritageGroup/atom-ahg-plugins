        ...
        <div class="row mb-3">
          <div class="col-md-6">
            ...
            <input type="text" name="config[numbering_mask]" class="form-control"
                   value="{{ e($cfg['numbering_mask'] ?? '{YEAR}-{SEQ:5}') }}"
          ...
          <div class="col-md-6">
            <div class="form-check form-switch mb-3">
              <input class="form-check-input" type="checkbox" id="auto_assign_enabled"
                     ...
                     @if (!empty($cfg['auto_assign_enabled'])) checked @endif>
            ...
            <div class="form-check form-switch mb-3">
              <input class="form-check-input" type="checkbox" id="require_donor_agreement"
                     ...
                     @if (!empty($cfg['require_donor_agreement'])) checked @endif>
            ...
            <div class="form-check form-switch mb-3">
              <input class="form-check-input" type="checkbox" id="require_appraisal"
                     ...
                     @if (!empty($cfg['require_appraisal'])) checked @endif>
          ...
          <div class="col-md-6">
            <div class="mb-3">
              ...
              <select name="config[default_priority]" class="form-select">
                @php $defaultPriority = $cfg['default_priority'] ?? 'normal'; @endphp
                <option value="low" @if ($defaultPriority === 'low') selected @endif><?php echo __('Low'); ?></option>
                <option value="normal" @if ($defaultPriority === 'normal') selected @endif><?php echo __('Normal'); ?></option>
                <option value="high" @if ($defaultPriority === 'high') selected @endif><?php echo __('High'); ?></option>
                <option value="urgent" @if ($defaultPriority === 'urgent') selected @endif><?php echo __('Urgent'); ?></option>
              </select>
            </div>
            ...
            <div class="mb-3">
              ...
              <select name="config[intake_checklist_template_id]" class="form-select">
                ...
                @php $selectedChecklist = $cfg['intake_checklist_template_id'] ?? ''; @endphp
                @foreach ($checklistTemplates ?? [] as $tpl)
                  <option value="{{ $tpl->id }}" @if ($selectedChecklist == $tpl->id) selected @endif>
                    {{ e($tpl->name) }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="mb-3">
              ...
              <select name="config[appraisal_template_id]" class="form-select">
                <option value=""><?php echo __('None'); ?></option>
                @php $selectedAppraisal = $cfg['appraisal_template_id'] ?? ''; @endphp
                @foreach ($appraisalTemplates ?? [] as $tpl)
                  <option value="{{ $tpl->id }}" @if ($selectedAppraisal == $tpl->id) selected @endif>
                    {{ e($tpl->name) }}
                  </option>
                @endforeach
              </select>
          ...
          <div class="col-md-6">
            <div class="form-check form-switch mb-3">
              <input class="form-check-input" type="checkbox" id="allow_container_barcodes"
                     ...
                     @if (!empty($cfg['allow_container_barcodes'])) checked @endif>
          ...
          <div class="col-md-6">
            <div class="form-check form-switch mb-3">
              <input class="form-check-input" type="checkbox" id="rights_inheritance_enabled"
                     ...
                     @if (!empty($cfg['rights_inheritance_enabled'])) checked @endif>
