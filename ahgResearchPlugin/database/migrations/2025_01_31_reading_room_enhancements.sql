...
CREATE TABLE IF NOT EXISTS `research_reading_room_seat` (
    ...
    `seat_type` VARCHAR(77) COMMENT 'standard, accessible, computer, microfilm, oversize, quiet, group' DEFAULT 'standard',
...
CREATE TABLE IF NOT EXISTS `research_seat_assignment` (
    ...
    `status` VARCHAR(49) COMMENT 'assigned, occupied, released, no_show' DEFAULT 'assigned',
    `notes` TEXT,
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `research_equipment` (
    ...
    `equipment_type` VARCHAR(139) COMMENT 'microfilm_reader, microfiche_reader, scanner, computer, magnifier, book_cradle, light_box, camera_stand, gloves, weights, other' NOT NULL,
    ...
    `condition_status` VARCHAR(63) COMMENT 'excellent, good, fair, needs_repair, out_of_service' DEFAULT 'good',
...
CREATE TABLE IF NOT EXISTS `research_equipment_booking` (
    ...
    `purpose` TEXT,
    `status` VARCHAR(58) COMMENT 'reserved, in_use, returned, cancelled, no_show' DEFAULT 'reserved',
    `checked_out_at` DATETIME DEFAULT NULL,
    `checked_out_by` INT DEFAULT NULL,
    `returned_at` DATETIME DEFAULT NULL,
    `returned_by` INT DEFAULT NULL,
    `condition_on_return` VARCHAR(42) COMMENT 'excellent, good, fair, damaged' DEFAULT NULL,
    `return_notes` TEXT,
    `notes` TEXT,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `research_request_queue` (
    ...
    `description` TEXT,
    `queue_type` VARCHAR(63) COMMENT 'retrieval, paging, return, curatorial, reproduction' DEFAULT 'retrieval',
    ...
    `sort_direction` VARCHAR(21) COMMENT 'ASC, DESC' DEFAULT 'ASC',
...
CREATE TABLE IF NOT EXISTS `research_activity` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `activity_type` VARCHAR(97) COMMENT 'class, tour, exhibit, loan, conservation, photography, filming, event, meeting, other' NOT NULL,
    ...
    `status` VARCHAR(78) COMMENT 'requested, tentative, confirmed, in_progress, completed, cancelled' DEFAULT 'requested',
...
CREATE TABLE IF NOT EXISTS `research_activity_material` (
    ...
    `status` VARCHAR(79) COMMENT 'requested, approved, rejected, retrieved, in_use, returned, damaged' DEFAULT 'requested',
...
CREATE TABLE IF NOT EXISTS `research_activity_participant` (
    ...
    `phone` VARCHAR(50),
    `organization` VARCHAR(255),
    `role` VARCHAR(87) COMMENT 'organizer, instructor, presenter, student, visitor, assistant, staff, other' DEFAULT 'visitor',
    `dietary_requirements` TEXT,
    `accessibility_needs` TEXT,
    `registration_status` VARCHAR(70) COMMENT 'pending, confirmed, waitlist, cancelled, attended, no_show' DEFAULT 'pending',
...
CREATE TABLE IF NOT EXISTS `research_print_template` (
    ...
    `name` VARCHAR(100) NOT NULL,
    `code` VARCHAR(50) NOT NULL,
    `template_type` VARCHAR(73) COMMENT 'call_slip, paging_slip, receipt, badge, label, report, letter' NOT NULL,
    `description` TEXT,
    `template_html` LONGTEXT NOT NULL,
    `css_styles` TEXT,
    `page_size` VARCHAR(63) COMMENT 'a4, a5, letter, label_4x6, label_2x4, badge, custom' DEFAULT 'a4',
    `orientation` VARCHAR(31) COMMENT 'portrait, landscape' DEFAULT 'portrait',
