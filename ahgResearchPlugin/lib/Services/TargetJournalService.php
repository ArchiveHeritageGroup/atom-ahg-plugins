...
/**
 * TargetJournalService - target-journal directory (#114 / Heratio #1107).
 *
 ...
 *
 * AtoM port of Heratio AhgResearch\Services\ResearchTargetJournalService.
 *
 * @package ahgResearchPlugin
 * @version 1.0.0
 */
class TargetJournalService
{
    public const REFERENCE_STYLES = ['APA', 'Harvard', 'Vancouver', 'Chicago', 'MLA', 'IEEE'];

    public const TABLE = 'research_target_journal';

    // ---- CRUD ---------------------------------------------------------------

    /**
     * List directory journals with optional filters.
     *
     * @param array $filters market|q|accreditation
     *
     * @return array<int, array<string, mixed>>
     */
    public function list(array $filters = []): array
    {
        $q = DB::table(self::TABLE)->orderBy('title');
        ...
        if (!empty($filters['q'])) {
            ...
            $q->where(function ($w) use ($term) {
                $w->where('title', 'like', $term)
                    ->orWhere('subject_scope', 'like', $term)
                    ->orWhere('publisher', 'like', $term)
                    ->orWhere('accreditation', 'like', $term);
            });
        }
        ...
        return array_map(fn ($r) => (array) $r, $q->get()->all());
    }

    /**
     * Fetch a single journal as an associative array (or null).
     */
    ...
    /**
     * Create a journal; returns the new id.
     */
    public function create(array $data): int
    {
        return (int) DB::table(self::TABLE)->insertGetId($this->payload($data, true));
    }

    /**
     * Update a journal; returns true when the row exists (>= 0 affected).
     */
    public function update(int $id, array $data): bool
    {
        return DB::table(self::TABLE)->where('id', $id)->update($this->payload($data, false)) >= 0;
    }
    ...
    /**
     * Normalise a request payload into a row.
     */
    private function payload(array $d, bool $isNew): array
    {
        $issn = isset($d['issn']) ? trim((string) $d['issn']) : '';
        $p = [
     ...
     * each with an added 'match_score' key.
     *
     * Drives the auto-assemble flow: pick/suggest target -> pre-format -> validate
     * -> package.
     *
     * @return array<int, array<string, mixed>>
     */
     ...
     *
     * Representative STARTER set of DHET-accredited South African journals
     * (emphasis on GLAM / LIS / archives / heritage plus a few multidisciplinary
     * titles), not the full ~700-title list. ISSNs and exact numeric submission
     * limits are completed/verified per journal by operators via the admin UI.
     */
    public function seedDhetStarter(): int
    {
        ...
        foreach ($this->dhetStarter() as $row) {
            ...
            $existing = DB::table(self::TABLE)->where('title', $row['title'])->first();
            if ($existing) {
